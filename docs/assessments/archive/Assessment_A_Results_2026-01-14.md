...
- **Trojan Horse Artifacts**: The recent massive commit injected code without proper integration.
  - _Remediated during assessment_: `matlab_code_analyzer_gui` and `scientific_auditor.py` were found dumped inside `web_applications/unit_converter/tools` and have been moved to a proper `tools/` root directory.
