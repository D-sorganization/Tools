...
- **Security (XSS):** The application uses `textContent` for rendering user inputs (e.g., `textDiv.textContent = conversionText`), which effectively mitigates XSS risks.
  - _Note:_ The test `src/web_applications/unit_converter/tests/xss_prevention.test.js` tests a _simulation_ of a vulnerability using a local `escapeHtml` function. While good for education, it does not test the actual `app.js` logic, as `app.js` uses `textContent` and has an unused `escapeHtml` function.
