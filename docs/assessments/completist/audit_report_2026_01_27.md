...
1.  **Quality Assurance Tools**:
    *   Scripts like `src/tools/code_quality_check.py`, `scripts/quality-check.py`, and `matlab_quality_check.py` contain regex patterns (e.g., `re.compile(r"\bTODO\b")`) to *detect* these markers. These are not TODOs themselves.
