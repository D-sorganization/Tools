...
The regional-ground execution job's v1 `flight.settings` field remains a
generic bounded numeric evidence mapping. The separate versioned execution
registry narrows one exact identity, `waterloo_penner` plus
`tools-core/1.0.0`, to the keys `max_time_s`, `step_s`, and `sample_every` with
explicit numeric bounds. Its recomputation contract binds Waterloo/Penner
default coefficients, adaptive RK45, launch-relative planar transfer-surface
contact, `step_s` dense-output sampling, deterministic `sample_every`
decimation, and mandatory terminal-sample retention.

The profile boundary releases a recomputed `FlightResult` only when both the
canonical trajectory and full-result SHA-256 values match the submitted job.
It returns typed evidence for an absent profile, invalid schema, failed
recomputation, trajectory mismatch, result mismatch, or exact qualification.
Runner preflight maps those outcomes to non-sensitive stable reasons.
Cancellation is checked first; callback defects and preflight rejection retain
typed terminal stages and zero accepted trials. The authority manager may
publish only the generic terminal stage and never a partial result or raw
exception.

The current canonical job fixture does not qualify: deterministic registered
profile recomputation produces trajectory and result digests different from
its declared synthetic evidence. No ground physics is invoked and no result is
published. A future canonical fixture must be generated by the registered
profile under a pinned numerical runtime before ground execution can be
