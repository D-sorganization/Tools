# Archived Root Handoff Log — Tools (through 2026-08-15)

> **This is an archive, not the current handoff.**
> The live document is [`AGENT_HANDOFF.md`](../../AGENT_HANDOFF.md) at the repo root.
...
root handoff had instead accumulated 137 dated entries across 2708 lines.

Those entries are preserved verbatim below so no working-tree context is lost.
They are historical: statuses, branch names, and PR states in these entries were
accurate when written and many are now stale. Treat the live root handoff as
authoritative and use these only to reconstruct why something was done.
...
`.codex-worktrees` gitlinks, retains the current-main CI/PDF-renamer changes,
and adds the cumulative package-data assertion for the approved visual
baseline manifest and PNG sets. Seven explicit Qt/NumPy return narrowings close
the hosted MyPy boundary without changing runtime values or scientific logic.
...
The 18 visual references remain proposed until this consolidated protected
merge. Responsive/DPI-1.5 references, manual AT qualification, and portable
cross-platform pixel identity remain open.
...
execution head `84498e2dd42e86adcfc9507eb1d4542b04bd8f78` first and published
confidence-mesh head `0b38346ce3b56aeee620c6304ab0a27041bc4940` second. It retains strict
request/document input authority, passive localized RK4 execution and export,
bounded confidence-ellipsoid generation, PyQt6/React surface rendering,
constant-space camera bounds, schema-v3 plot persistence, and the exact
constructor-helper assertion exemption.

The sole overlapping production component preserves both accessible localized
torque source labels and the optional confidence-ellipsoid surface controls,
rendering, legend, status, and persistence. Neither parent implementation was
weakened. Production Worker/UI prescribed-profile transport, Rust parity, full
RK4 half-step torque history, cross-browser/AT evidence, approved visual
...
6/6 production Playwright scenarios. Exact Python 3.12 + Mypy 1.13 covers the
10 changed production Python files; Ruff/format, assertion policy, TypeScript,
ESLint, Vite build, docs governance, module/file-size, and diff gates pass.
