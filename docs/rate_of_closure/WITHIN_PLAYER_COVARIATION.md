...
The private TrackMan-comparable v1 corpus contains 10,169 retained source shots
but no trustworthy player identifier. It can support shot- and model-level
analysis, not within-player inference. Player analysis becomes available when a
user loads a dataset containing an explicit identity/grouping column. The UI
must state this limitation rather than treating the corpus as one known player.
