...
- Version 1.17.09 reconciles the clean consolidated release with `main`
  ...
  work already trusted there. Independent SPEC changelog histories are
  preserved.
...
- Version 1.17.07 replaces a 30-deep protected cascade with one scoped release
  ...
  changes and the complete approved Rate/swing/golf feature authority.
- Cumulative integration adds the missing exact package-data assertion for the
...
- Thirty-five historical foundation modules remain over the later 400-line
  slice policy without consolidation growth. Visual references remain proposed
  until protected merge; responsive/DPI-1.5 references, manual AT execution,
...
- Exact passive request/document/RK4/impact authority and localized result
  provenance remain intact alongside bounded confidence-ellipsoid generation,
  PyQt6/React rendering, constant-space camera bounds, strict public Python
  construction, schema-v3 persistence, and the exact assertion-helper policy.
- The overlapping React arc component preserves both accessible localized
  source labels and optional Gaussian-content surface controls, rendering,
  legend, status, and persisted plot intent.
- Integrated gates pass 275/275 localized/variation Python, 167/167 shared
  ...
  Exact Python 3.12 + Mypy 1.13 passes 10 changed production Python files;
  Ruff/format, assertion policy, TypeScript, ESLint, build, docs, size, and
  diff gates pass.
- SPEC advances to 1.16.69. Prescribed Worker/UI transport, Rust parity, full
  RK4 half-step history, cross-browser/AT evidence, approved baselines,
...
- Normal non-fast-forward integration retains approved dispersion head
  `71634bf7393c8343a53f9acaa9f4db76cb4ac8db` as first parent and published
  localized-locus/browser head `393f80e8e6b7ebcc7207136aa8a7aa47899a6eda`
  as second parent, without rebase or history rewrite.
- Both append-only documentation histories and all implementation/workflow
  changes remain present. The sole non-document conflict retained the complete
  locus persistence suite and the newer dispersion analysis block; two stale
  assertions in the locus-split analysis file now use the approved metric-
  generic accessibility labels.
- Evidence: 338 combined dispersion/PyQt/shared tests, 841 React tests, seven
  workflow-policy tests, exact Python 3.12/Mypy 1.13 over 23 cumulative source
  files, Ruff/format, TypeScript, ESLint, Vite production build, documentation
  governance, the 500-line changed-file gate, and five production-Worker
  Chromium checks.
- SPEC advances to 1.16.64. Plot-definition import UI, full ellipsoid meshes,
  cross-browser/assistive-technology E2E, protected publication, and remaining
...
## 2026-08-12 #4142 R14.3 React worker transport hardening
...
## 2026-08-12 #4142 R14.3 React Monte Carlo worker execution
