...
- Published fleet policy 92283cf3f is integrated verbatim, preserving the context navigation policy. Remote c78c99975 contributes the optional FastMCP availability probe; no peer implementation was discarded. The required context qualification step explicitly imports FastMCP and its stdio client before pytest; ordinary broad-suite runs retain optional SDK detection. Concurrent remote merge7f8c18031 and its root-handoff compaction are preserved; the context development-log summary and current qualification state are retained in SELF.
