...
## Canonical Launcher Provider #5143 / PR #5144

- Worktree: `C:/Users/diete/Repositories/Tools-impact-shaft`; branch `fix/5143-function-launcher`; integration commit `SELF`; PR#5144 remains open at publishedfc453bf8e until normal publication checks finish.
- Objective: preserve the two-file canonical launcher correction with main92283cf3f, which contains camera/calibration0a561daff and CLI/service9899c5a6a. The launcher manifest and TDD test remain byte-identical tofc453bf8e; incoming camera/CLI source remains main's implementation.
- The combined revision passes29 GUI/engine/CLI/API controls in26.57s, including the canonical launcher resolution test. All nine manual gates pass with the existing two release blockers retained. Final provider adoption still requires normal commit/push checks and protected CI/review. The prior launcher source passes16 local controls; JUnit is impact5144-main922-integration.xml.
- Known CI blockers atfc453bf8e: the original and single retry of Python3.12 tests-rate both time out90min with a club-tester worker crash and unfinished simulation-subtabs test; private Gasification checkout404 occurs before consumer tests. Complete evidence is in docs/ci-failures/impact5143-20260910.md. No further unchanged retry, timeout relaxation or guard bypass is planned.
- The capture task owns #5114 GUI lifecycle diagnosis. Session context-01a0879e-ud owns the one final gitlink/Cargo/pip/catalog alignment after a reviewed Tools descendant is available; the impact task then qualifies the exact installed wheel. UD#9916 is merged asc487265f with interim4dabe900c. Pinning camera0a alone would regress the launcher path and is not an acceptable replacement.
- No user-owned state is discarded; preserve the camera/calibration records above. docs/agent_context/catalog.json is absent on this base, so source and relevant tests provide the verified context fallback.
- Next action: qualify and publish this combined provider revision on PR#5144, leaving protected CI/review and physical/acoustic requirements open. Development-log entry: DL-#5143.

Launcher review metadata continuation at SELF: db67c2651 is published after normal hooks. Its quality gate passed, while the divergence gate found missing formal UD-PAIR text; the live body now references actual merged consumer9916/c487265f. The next action is to qualify the corrected fresh PR event. No source, test or ledger ruling changed. Context providerc80f2cf34 separately includes the same two files and owns final pin alignment.
