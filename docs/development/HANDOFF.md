...
- Next: remove repeated section-kinematics work in the coupled solver while retaining every numerical check and the60s deadline. Event-resolved work and a useful relative cutoff-loss budget remain subsequent science requirements. Last published reference source is72b2efd47b63d219c0460acb042b06dd47a02835 with normal hooks passing; Job-level inspection of Standard CI34464371054 now shows both shared shards failed at the unchanged60s zero-gap three-grid release test: jobs102829404145 (3.12) and102829404209 (3.11). The aggregate run remains queued because Rust has not started; do not infer test status from that aggregate. Full logs impact72-shared312.log and impact72-shared311.log are retained in TEMP. This source needs a performance repair before hosted qualification; the pending entry-reference commit is separate. Earliera22 run34460212232 is separate. Check the same live handle at a natural checkpoint; do not restart unchanged work merely because observation is delayed. General reversal/nonplanar/recontact, mesh/mode, protected review and final main/consumer qualification remain open. Keep#5160/#5073/#5074/#5068, UD#9700 and Affine#4253 open for the remaining numerical and physical/acoustic requirements.
