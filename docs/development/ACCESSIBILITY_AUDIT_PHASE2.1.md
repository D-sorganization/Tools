...
**P1 - HIGH (Do in Phase 2.1):** 5. Change footer text from `text-dark-500` to `text-dark-300` 6. Audit and fix any `text-dark-500` used for actionable text

**P2 - MEDIUM (Phase 2.2):** 7. Consider enhanced focus indicator colors (currently uses blue-500, which is good)
...
- **Current state:**
  ```tsx
  {
    isMobile && sidebarOpen && <div className="fixed inset-0 bg-black/50 z-40 md:hidden" onClick={() => setSidebarOpen(false)} />;
  }
  ```
