...
project's .cursorrules.md requirements. It can be run from the command line
and integrates with the project's quality control system.
...
# [lines] Maximum lines to search ahead for docstring
...
class MATLABQualityChecker:
    ...
    def run_matlab_quality_checks(self) -> dict[str, object]:
        ...
        try:
            ...
            # Note: This requires MATLAB to be installed and accessible
            # from command line
            try:
                ...
                logger.warning(
                    "Could not run MATLAB script directly: %s",
                    e,
                )
    ...
    def _run_matlab_script(self, script_path: Path) -> dict[str, object]:
        ...
        try:
            ...
            logger.info(
                "All MATLAB commands failed, falling back to static analysis",
            )
            return self._static_matlab_analysis()
        ...
        try:
            ...
            for i, line in enumerate(lines, 1):
                ...
                if not is_comment:
                    ...
                    if re.match(nesting_pattern, line_stripped):
                ...
                if re.search(r"\beval\s*\(", line_stripped):
                    issues.append(
                        f"{file_path.name} (line {i}): "
                        "Avoid using eval() - potential security risk "
                ...
                line_before_comment = line_stripped.split("%")[0]
                if (
                ...
                # Check for magic numbers (but allow common values and constants)
                ...
                # in words. Uses lookbehind/lookahead to avoid matching numbers
                # adjacent to dots or word characters. This helps flag "magic numbers"
                # in code while avoiding false positives from common patterns.
                ...
                for num in magic_numbers:
                    ...
                    if num in known_constants:
                        issues.append(
                            f"{file_path.name} (line {i}): "
                            f"Magic number {num} ({known_constants[num]}) - "
                        ...
                        if comment_idx == -1 or (
                            ...
                            issues.append(
                                f"{file_path.name} (line {i}): "
                                f"Magic number {num} should be defined as "
                                "constant with units and source",
                            )
                ...
                if re.search(r"\bexist\s*\(", line_stripped):
                    issues.append(
                        f"{file_path.name} (line {i}): "
                        "Consider using validation or try/catch instead of "
                        "exist()",
                    )

                # Check for addpath in functions (should be in startup.m
    ...
    if args.output_format == "json":
        ...
        if issues and isinstance(issues, list):
            ...
            for i, issue in enumerate(issues, 1):
                issue_str = str(issue) if issue is not None else ""
                print(f"  {i}. {issue_str}")  # noqa: T201
