...
print(f"Using {len(available_files)} test files: {[Path(f).name for f in available_files]}")
...
try:
    ...
    if processed_df is not None and not processed_df.empty:
        ...
        print(f"  Original file shape: {pd.read_csv(test_file, nrows=1).shape[1]} columns")
...
try:
    ...
    if 'processed_df' in locals() and processed_df is not None:
        ...
        try:
            ...
            if excel_files:
                print(f"PASS: Excel export successful, created {len(excel_files)} files")
            else:
