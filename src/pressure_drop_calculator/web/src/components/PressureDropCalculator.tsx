...
// See issue #608.
const CALC_API_BASE =
  import.meta.env.VITE_CALC_API_URL ?? "http://localhost:8010";
...
export function PressureDropCalculator() {
  const [activeTab, setActiveTab] = useState<"input" | "results" | "chart">(
    "input",
  );
  ...
  // Gas composition
  const [gasComp, setGasComp] = useState<GasComp>({
    N2: 78,
    O2: 21,
    CO2: 0,
    H2O: 0,
  ...
  const calculate = useCallback(async () => {
    ...
    const normComp = Object.fromEntries(
      Object.entries(gasComp).map(([k, v]) => [k, v / 100]),
    ) as GasComp;
    ...
    try {
      ...
      dP_friction =
        f * (pipeLength / diameter) * 0.5 * rho * velocity * velocity;
    }
    ...
    if (mach > 0.3)
      warnings.push("High Mach number - compressibility effects significant");
    if (velocity > erosionalVel)
      warnings.push("Velocity exceeds erosional limit!");
    if (dP_total / P_Pa > 0.1)
      warnings.push(
        "Pressure drop > 10% of inlet - consider compressible flow",
      );
  ...
  const componentBreakdown = useMemo(() => {
    ...
    return [
      { name: "Friction", value: results.frictionPressureDrop / 1000 },
      {
        name: "Elevation",
        value:
          Math.abs(results.totalPressureDrop - results.frictionPressureDrop) /
          1000,
      },
    ];
  }, [results]);
          ...
          <div className="bg-slate-800 rounded-lg p-4">
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <select
                  value={frictionMethod}
                  onChange={(e) => setFrictionMethod(e.target.value)}
                  className="w-full bg-slate-700 text-white rounded px-3 py-2"
                >
                  {FRICTION_METHODS.map((m) => (
                    <option key={m} value={m}>
                      {m}
                    </option>
                  ))}
          ...
          <div className="bg-slate-800 rounded-lg p-4">
              ...
              {(Object.keys(gasComp) as (keyof GasComp)[]).map((key) => (
                <div key={key}>
                  ...
                  <input
                    type="number"
                    value={gasComp[key]}
                    onChange={(e) => updateGas(key, +e.target.value)}
                    min="0"
                    max="100"
                    step="0.1"
                    className="w-full bg-slate-700 text-white rounded px-3 py-2"
            ...
            <div className="mt-3 text-sm text-slate-400">
              ...
              <span
                className={
                  Math.abs(
                    Object.values(gasComp).reduce((a, b) => a + b, 0) - 100,
                  ) < 1
                    ? "text-green-400"
                    : "text-yellow-400"
                }
              >
                {Object.values(gasComp)
                  .reduce((a, b) => a + b, 0)
                  .toFixed(1)}
                %
              </span>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="bg-slate-800 rounded-lg p-4">
              ...
              <div className="text-2xl font-bold text-blue-400">
                {(results.totalPressureDrop / 1000).toFixed(2)} kPa
              </div>
            </div>
            <div className="bg-slate-800 rounded-lg p-4">
              <div className="text-slate-400 text-sm">Outlet Pressure</div>
              <div className="text-2xl font-bold text-green-400">
                {(results.outletPressure / 1e5).toFixed(3)} bar
              </div>
            </div>
            <div className="bg-slate-800 rounded-lg p-4">
            ...
            <div className="bg-slate-800 rounded-lg p-4">
              ...
              <div className="text-2xl font-bold text-yellow-400">
                {results.frictionFactor.toFixed(6)}
              </div>
            </div>
          </div>
              ...
              <table className="w-full text-sm">
                <tbody>
                  {[
                    ...
                    [
                      "Erosional Velocity",
                      `${results.erosionalVelocity.toFixed(2)} m/s`,
                    ],
      ...
      {(activeTab === "results" || activeTab === "chart") && !results && (
        <div className="bg-slate-800 rounded-lg p-12 text-center">
          <p className="text-slate-400">
            No results yet. Configure inputs and click "Calculate Pressure
            Drop".
          </p>
        </div>
      )}
