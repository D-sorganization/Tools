...
class TestFeedComposition:
    ...
    def test_total_moles(self) -> None:
...
class TestFeedFromMassFractions:
    ...
    def test_pure_carbon(self) -> None:
    ...
    def test_ash_excluded(self) -> None:
        f = FeedComposition.from_mass_fractions({"C": 0.5, "Ash": 0.5})
        d = f.as_dict()
        assert "Ash" not in d
        assert f.C > 0

    def test_coal_composition(self) -> None:
    ...
    def test_empty_mass_fractions_returns_zeros(self) -> None:
        f = FeedComposition.from_mass_fractions({})
        assert f.total_moles() == 0.0

    def test_hydrogen_has_more_moles_than_carbon(self) -> None:
...
class TestOxidantConfig:
    ...
    def test_air_n2_o2_ratio(self) -> None:
    ...
    def test_air_mode_double_flow(self) -> None:
...
class TestProcessInputs:
    ...
    def test_default_all_zero_flow(self) -> None:
        pi = ProcessInputs()
    ...
    def test_all_injections_returns_list(self) -> None:
        pi = ProcessInputs()
        injections = pi.all_injections()
        assert len(injections) == 5

    def test_modify_stream_flow(self) -> None:
        pi = ProcessInputs()
        pi.steam.flow = 2.0
...
class TestBuildTotalFeed:
    ...
    def test_base_only_no_injections(self) -> None:
    ...
    def test_with_steam_injection(self) -> None:
    ...
    def test_with_pure_o2(self) -> None:
    ...
    def test_with_n2_purge(self) -> None:
        base = FeedComposition(C=1.0)
        pi = ProcessInputs()
    ...
    def test_with_ch4_injection(self) -> None:
        base = FeedComposition(C=1.0)
        pi = ProcessInputs()
    ...
    def test_with_c3h8_injection(self) -> None:
        base = FeedComposition(C=1.0)
        pi = ProcessInputs()
    ...
    def test_with_natural_gas(self) -> None:
        base = FeedComposition(C=1.0)
        pi = ProcessInputs()
    ...
    def test_multiple_injections_combine(self) -> None:
        base = FeedComposition(C=1.0, H=1.0, O=0.5)
        pi = ProcessInputs()
        pi.steam.flow = 1.0
        pi.oxidant.o2_flow = 0.5
...
class TestFeedPresets:
    ...
    def test_feed_from_preset_custom(self) -> None:
        f = feed_from_preset("Custom")
        assert f.C == 1.0
        assert f.H == 1.0
        assert f.O == 0.5

    def test_feed_from_preset_natural_gas(self) -> None:
        f = feed_from_preset("Natural Gas (CH4)")
        assert f.C == 1.0
        assert f.H == 4.0

    def test_all_presets_produce_valid_feed(self) -> None:
...
class TestCompoundElements:
    ...
    def test_h2o_composition(self) -> None:
        assert COMPOUND_ELEMENTS["H2O"] == {"H": 2, "O": 1}

    def test_o2_composition(self) -> None:
        assert COMPOUND_ELEMENTS["O2"] == {"O": 2}

    def test_n2_composition(self) -> None:
        assert COMPOUND_ELEMENTS["N2"] == {"N": 2}

    def test_ch4_composition(self) -> None:
        assert COMPOUND_ELEMENTS["CH4"] == {"C": 1, "H": 4}

    def test_c3h8_composition(self) -> None:
        assert COMPOUND_ELEMENTS["C3H8"] == {"C": 3, "H": 8}

    def test_natural_gas_composition(self) -> None:
