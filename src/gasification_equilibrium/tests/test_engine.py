...
class TestSinglePointEquilibrium:
    ...
    def test_mole_fractions_nonnegative(self, engine):
        result = engine.solve(temperature=1000, feed={"C": 1.0, "H": 1.0, "O": 1.0})
...
class TestFeedModes:
    ...
    def test_dict_feed(self, simple_engine):
        result = simple_engine.solve(
            temperature=1000, feed={"C": 1.0, "H": 2.0, "O": 1.0}
        )
        assert result.converged

    def test_mass_fraction_feed(self, simple_engine):
        result = simple_engine.solve(
            temperature=1000,
            feed_mass={
                "C": 0.75,
    ...
    def test_process_inputs_feed(self, simple_engine):
        pi = ProcessInputs()
        pi.steam.flow = 1.0
    ...
    def test_default_feed(self, simple_engine):
...
class TestLegacyParams:
    ...
    def test_steam_carbon_ratio(self, simple_engine):
    ...
    def test_oxygen_carbon_ratio(self, simple_engine):
    ...
    def test_equivalence_ratio(self, simple_engine):
        r = simple_engine.solve(
            temperature=1000,
            feed={"C": 1.0, "H": 1.0, "O": 0.5},
...
def result_has_oxygen(result):
    """Helper: check if result feed has oxygen."""
    return result.feed_elements.get("O", 0) > 0


class TestKnownEquilibria:
    """Test engine against known equilibrium behavior."""

    def test_boudouard_favors_co_at_high_t(self, simple_engine):
    ...
    def test_boudouard_favors_co2_at_low_t(self, simple_engine):
        result = simple_engine.solve(temperature=400, feed={"C": 2.0, "O": 2.0})
        comp = result.composition_dict()
        assert comp.get("CO2", 0) > comp.get("CO", 0)

    def test_methanation_favored_at_low_t(self, simple_engine):
    ...
    def test_higher_pressure_favors_ch4(self, simple_engine):
    ...
    def test_carbon_conversion_increases_with_steam(self, simple_engine):
...
class TestTemperatureSweep:
    ...
    def test_returns_correct_count(self, simple_engine):
    ...
    def test_temperatures_correct(self, simple_engine):
        results = simple_engine.temperature_sweep(
            t_start=500,
            t_end=1500,
            n_points=5,
    ...
    def test_invalid_range_raises(self, simple_engine):
        with pytest.raises(AssertionError):
...
class TestSurfaceSweep:
    ...
    def test_shape(self, simple_engine):
...
class TestContractViolations:
    ...
    def test_negative_temperature_raises(self, engine):
        with pytest.raises(AssertionError):
            engine.solve(temperature=-100, feed={"C": 1.0})

    def test_zero_temperature_raises(self, engine):
        with pytest.raises(AssertionError):
            engine.solve(temperature=0, feed={"C": 1.0})

    def test_negative_pressure_raises(self, engine):
        with pytest.raises(AssertionError):
            engine.solve(temperature=1000, pressure=-1, feed={"C": 1.0})

    def test_zero_pressure_raises(self, engine):
        with pytest.raises(AssertionError):
...
class TestFeedPresets:
    ...
    def test_all_presets_solve(self, engine):
    ...
    def test_bituminous_coal_with_steam(self, engine):
...
class TestWarmStart:
    ...
    def test_warm_start_converges(self, simple_engine):
