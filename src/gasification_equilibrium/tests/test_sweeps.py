...
class TestTemperatureSweep:
    ...
    def test_temperatures_are_correct(self, engine):
        results = temperature_sweep(
    ...
    def test_most_converged(self, engine):
        results = temperature_sweep(
    ...
    def test_h2_increases_with_temperature(self, engine):
    ...
    def test_invalid_range_raises(self, engine):
        with pytest.raises(AssertionError):
            temperature_sweep(engine, 1500, 500, 10, feed={"C": 1.0})

    def test_minimum_points_raises(self, engine):
        with pytest.raises(AssertionError):
            temperature_sweep(engine, 500, 1500, 1, feed={"C": 1.0})

    def test_two_points_minimum(self, engine):
        results = temperature_sweep(
            engine, 500, 1500, 2, feed={"C": 1.0, "H": 1.0, "O": 0.5}
        )
        assert len(results) == 2

    def test_results_have_correct_species(self, engine):
        results = temperature_sweep(
...
class TestSurfaceSweep:
    ...
    def test_param_name_stored(self, engine):
        data = surface_sweep(
            engine,
            (600, 1200),
            "pressure",
    ...
    def test_species_stored(self, engine):
        data = surface_sweep(
            engine,
            (600, 1200),
            "steam_carbon_ratio",
    ...
    def test_temperatures_range(self, engine):
        data = surface_sweep(
            engine,
            (600, 1200),
            "steam_carbon_ratio",
    ...
    def test_compositions_nonnegative(self, engine):
        data = surface_sweep(
            engine,
            (600, 1200),
            "steam_carbon_ratio",
