...
class TestElementMatrix:
    ...
    def test_build_balance_vector(self, small_matrix):
    ...
    def test_build_balance_vector_missing_elements(self, small_matrix):
    ...
    def test_species_index_found(self, small_matrix):
        idx = small_matrix.species_index("H2")
        assert idx >= 0
        assert small_matrix.species_keys[idx] == "H2"

    def test_species_index_not_found(self, small_matrix):
        idx = small_matrix.species_index("NONEXISTENT")
        assert idx == -1

    def test_element_matrix_h2_row(self, small_matrix):
...
class TestComputeGibbsGradient:
    ...
    def test_gradient_shape(self, small_matrix):
        n = np.ones(small_matrix.n_species) * 0.1
        grad = compute_gibbs_gradient(n, 1000, P_REF, small_matrix)
        assert grad.shape == (small_matrix.n_species,)

    def test_gradient_finite(self, small_matrix):
        n = np.ones(small_matrix.n_species) * 0.1
        grad = compute_gibbs_gradient(n, 1000, P_REF, small_matrix)
        assert np.all(np.isfinite(grad))

    def test_gradient_numerical_consistency(self, small_matrix):
...
class TestInitialGuess:
    ...
    def test_all_positive(self, small_matrix):
        b = small_matrix.build_balance_vector({"C": 1.0, "H": 2.0, "O": 1.0})
        n0 = initial_guess(small_matrix, b)
        assert np.all(n0 > 0)

    def test_handles_zero_feed(self, small_matrix):
...
class TestSolveEquilibrium:
    ...
    def test_converges_1000k(self, small_matrix):
    ...
    def test_converges_500k(self, small_matrix):
        result = solve_equilibrium(
            T=500,
            P=P_REF,
    ...
    def test_converges_1500k(self, small_matrix):
        result = solve_equilibrium(
            T=1500,
            P=P_REF,
    ...
    def test_element_balance_conserved(self, small_matrix):
        feed = {"C": 1.0, "H": 2.0, "O": 1.0}
        result = solve_equilibrium(
    ...
    def test_moles_nonnegative(self, small_matrix):
        result = solve_equilibrium(
            T=1000,
            P=P_REF,
    ...
    def test_result_has_fields(self, small_matrix):
        result = solve_equilibrium(
            T=1000,
            P=P_REF,
            feed_elements={"C": 1.0, "H": 1.0, "O": 0.5},
    ...
    def test_warm_start(self, small_matrix):
    ...
    def test_negative_temperature_raises(self, small_matrix):
        with pytest.raises(AssertionError):
            solve_equilibrium(
                T=-100, P=P_REF, feed_elements={"C": 1.0}, matrix=small_matrix
            )

    def test_zero_temperature_raises(self, small_matrix):
        with pytest.raises(AssertionError):
            solve_equilibrium(
                T=0, P=P_REF, feed_elements={"C": 1.0}, matrix=small_matrix
            )

    def test_negative_pressure_raises(self, small_matrix):
        with pytest.raises(AssertionError):
            solve_equilibrium(
                T=1000, P=-1, feed_elements={"C": 1.0}, matrix=small_matrix
            )

    def test_zero_pressure_raises(self, small_matrix):
        with pytest.raises(AssertionError):
            solve_equilibrium(
                T=1000, P=0, feed_elements={"C": 1.0}, matrix=small_matrix
            )

    def test_high_pressure_convergence(self, small_matrix):
        result = solve_equilibrium(
            T=800,
            P=P_REF * 10,
            feed_elements={"C": 1.0, "H": 2.0, "O": 1.0},
...
class TestKnownEquilibria:
    ...
    def test_boudouard_favors_co_at_high_t(self, matrix):
    ...
    def test_boudouard_favors_co2_at_low_t(self, matrix):
    ...
    def test_methanation_favored_at_low_t(self, matrix):
    ...
    def test_le_chatelier_pressure(self, matrix):
    ...
    def test_steam_gasification_produces_syngas(self, matrix):
