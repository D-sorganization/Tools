...
class TestSpeciesDatabase:
    ...
    def test_all_coefficients_have_7_elements(self):
        for key, sp in SPECIES_DB.items():
            assert (
                len(sp["coeff_low"]) == 7
...
class TestNASAPolynomials:
    ...
    def test_cp_over_r_positive_at_298(self):
    ...
    def test_coefficient_selection_low_range(self):
...
class TestHelperFunctions:
    ...
    def test_get_gas_species_excludes_solids(self):
        gases = get_gas_species()
        for g in gases:
            assert SPECIES_DB[g]["phase"] == "gas"

    def test_get_gas_species_includes_c3h8_and_ar(self):
        gases = get_gas_species()
        assert "C3H8" in gases
        assert "Ar" in gases

    def test_get_all_species_includes_solids(self):
        all_sp = get_all_species()
        phases = {SPECIES_DB[k]["phase"] for k in all_sp}
        assert "solid" in phases
        assert "gas" in phases

    def test_get_elements_in_system(self):
...
class TestConstants:
    ...
    def test_p_ref_value(self):
        assert abs(P_REF - 101325.0) < 1.0

    def test_t_ref_value(self):
        assert abs(T_REF - 298.15) < 0.1

    def test_heating_values_positive(self):
    ...
    def test_atomic_weights_has_all_elements(self):
