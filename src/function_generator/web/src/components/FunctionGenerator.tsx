...
export function FunctionGenerator() {
  ...
  const layerSignals = useMemo(() => {
    ...
    // ⚡ Bolt Optimization: Replace Array.from() with a pre-allocated array and for loop
    // to avoid iterability checks and closure execution per element.
    const time = new Array<number>(n);
    for (let i = 0; i < n; i++) time[i] = i / sampleRate;
