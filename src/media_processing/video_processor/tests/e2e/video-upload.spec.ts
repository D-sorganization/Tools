...
test.describe("Video Upload Workflow", () => {
  ...
  test("should display the landing page correctly", async ({ page }) => {
    ...
    // Check main heading
    await expect(
      page.getByRole("heading", { name: /Golf Swing Video Analyzer/i }),
    ).toBeVisible();

    // Check description
    await expect(
      page.getByText(/Upload and analyze your golf swing videos/i),
    ).toBeVisible();
  ...
  test("should display video uploader when no video is loaded", async ({
    page,
  }) => {
  ...
  test("should handle video file upload", async ({ page }) => {
    ...
    test.skip(
      !require("fs").existsSync(testVideoPath),
      "Test video fixture not found",
    );
  ...
  test("should validate file type", async ({ page }) => {
    ...
    test.skip(
      !require("fs").existsSync(invalidFilePath),
      "Test image fixture not found",
    );
    ...
    await expect(page.getByText(/invalid.*type/i)).toBeVisible({
      timeout: 5000,
    });
  });
  ...
  test("should display video player after successful upload", async ({
    page,
  }) => {
