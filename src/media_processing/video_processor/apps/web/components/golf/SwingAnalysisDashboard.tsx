...
export default function SwingAnalysisDashboard({
  ...
  const handlePoseDetected = useCallback(
    (landmarks: Landmark[]) => {
      ...
      const frame: PoseFrame = {
        ...
        landmarks,
        confidence:
          landmarks.reduce((acc, l) => acc + (l.visibility || 0), 0) /
          landmarks.length,
      };
  ...
  const runAnalysis = useCallback(async () => {
    if (poseFrames.length < 10) {
      setError(
        "Need at least 10 pose frames. Please play the video to capture poses.",
      );
      return;
    }
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500">
            ...
            {analysis &&
              ` • Overall Score: ${Math.round(analysis.scores.overall)}`}
          </p>
        </div>
        <div className="flex items-center space-x-3">
          ...
          <button
            ...
            disabled={
              disabled || isAnalyzing || (!analysis && poseFrames.length < 10)
            }
      ...
      {analysis ? (
        ...
        <div className="text-center py-12 bg-gray-50 rounded-lg">
          ...
          <p className="text-sm text-gray-500 max-w-md mx-auto">
            Enable pose detection and play through your swing video. Once enough
            frames are captured, click "Analyze Swing" to generate a
            comprehensive analysis report.
          </p>
          <div className="mt-6 flex items-center justify-center space-x-4 text-sm text-gray-500">
            <div className="flex items-center space-x-2">
              <div
                className={`w-3 h-3 rounded-full ${poseFrames.length >= 10 ? "bg-green-500" : "bg-gray-300"}`}
              />
