...
describe("Sanitization Functions", () => {
  ...
  describe("sanitizeHTML", () => {
    ...
    it("should strip event handler attributes", () => {
      expect(sanitizeHTML('<div onclick="alert(1)">Test</div>')).toBe(
        "<div>Test</div>",
      );
      expect(sanitizeHTML('<a href="#" onmouseover="alert(1)">link</a>')).toBe(
        '<a href="#">link</a>',
      );
    });
  ...
  describe("sanitizeColor", () => {
    ...
    it("should accept RGB colors", () => {
      ...
      expect(sanitizeColor("rgba(255, 0, 0, 0.5)")).toBe(
        "rgba(255, 0, 0, 0.5)",
      );
    });
