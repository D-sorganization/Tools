...
describe("Video Validation", () => {
  ...
  describe("isFileSizeValid", () => {
    it("should return true for valid file sizes", () => {
      ...
      expect(isFileSizeValid(createMockFile("test.mp4", 1024 * 1024))).toBe(
        true,
      );
      expect(isFileSizeValid(createMockFile("test.mp4", MAX_FILE_SIZE))).toBe(
        true,
      );
    });
    ...
    it("should return false for oversized files", () => {
      expect(
        isFileSizeValid(createMockFile("test.mp4", MAX_FILE_SIZE + 1)),
      ).toBe(false);
    });
  });
  ...
  describe("isFileTypeAllowed", () => {
    it("should return true for allowed MIME types", () => {
      expect(
        isFileTypeAllowed(createMockFile("test.mp4", 1024, "video/mp4")),
      ).toBe(true);
      expect(
        isFileTypeAllowed(createMockFile("test.webm", 1024, "video/webm")),
      ).toBe(true);
      expect(
        isFileTypeAllowed(createMockFile("test.ogg", 1024, "video/ogg")),
      ).toBe(true);
    });

    it("should return false for disallowed MIME types", () => {
      expect(
        isFileTypeAllowed(createMockFile("test.txt", 1024, "text/plain")),
      ).toBe(false);
      expect(
