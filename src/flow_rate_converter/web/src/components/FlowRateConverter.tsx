...
const VOLUMETRIC_TO_M3_S: Record<string, number> = {
  ...
  "ft3/min": 0.028316846592 / 60,
  "ft3/h": 0.028316846592 / 3600,
...
function convertFallback(
  ...
  if (!table || !table[fromUnit] || !table[toUnit]) {
    throw new Error(
      `Unknown unit or category: ${fromUnit} -> ${toUnit} (${category})`,
    );
  }
      ...
      {result && (
        <div className="bg-green-900/30 border border-green-700 rounded-lg p-6 text-center">
          ...
          <p className="text-slate-500 text-sm mt-3">
            {value.toLocaleString()} {result.fromUnit} ={" "}
            {result.result.toLocaleString(undefined, {
              maximumSignificantDigits: 8,
            })}{" "}
            {result.toUnit}
