...
def ground_result_json(result: GroundSimulationResult) -> str:
    ...
    if type(result) is not GroundSimulationResult:
        raise TypeError("result must use the exact GroundSimulationResult type")
    # `str(...)` is for the type checker: the changed-file MyPy gate runs with
    # `--follow-imports=skip`, so `GroundSimulationResult` degrades to `Any` and
    # `to_json()` would leave this `-> str` function untyped. The exact-type
    # check above already guarantees the runtime value.
