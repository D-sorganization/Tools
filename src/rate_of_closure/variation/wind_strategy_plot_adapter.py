...
def _spin_axis(launch: LaunchConditions) -> tuple[float, float, float]:
    if launch.spin_axis is not None:
        # Unpacked rather than returned directly: the changed-file mypy gate
        # runs with `--follow-imports=skip`, so `LaunchConditions.spin_axis`
        # resolves to `Any` and would leave this return untyped. Naming the
        # three components also pins the arity the annotation promises.
        axis_x, axis_y, axis_z = launch.spin_axis
        return (float(axis_x), float(axis_y), float(axis_z))
