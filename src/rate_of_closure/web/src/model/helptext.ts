...
export const HELP_TEXTS: Record<string, HelpEntry> = {
  ...
  "Ground Playback": {
    title: "How to Use This Page",
    paragraphs: [
      "Import one strict flight-to-ground-result/v1 JSON record generated by the Python reference executor. The browser is a result viewer and does not execute ground physics. It rejects request files, execution envelopes, unknown fields, invalid transitions, and oversized documents while retaining the last valid result. Review carry, total or observed-end distance, warnings, calibration, provenance, trajectory samples, and the exact event ledger.",
      "Use Play, Pause, exact-frame steps, the absolute-time scrubber, phase jumps, speed, and Loop. Drag the canvas to orbit, wheel to zoom, or reset the view. Phase boundaries hold the preceding exact state until the next sample rather than inventing motion. A partial result labels its last point Observed end. Result v1 names the surface but does not embed surface geometry, so the locked-scale view shows neutral target-frame axes instead of claiming an exact terrain plane.",
    ],
  },
