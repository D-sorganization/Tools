...
import { exportViewWorkspace, importViewWorkspace, type ViewWorkspace } from "../model/viewWorkspace";
...
export function useWorkspaceFiles(options: WorkspaceFileOptions): WorkspaceFileController {
  const input = useRef<HTMLInputElement>(null);
  const mode = useRef<PickerMode>("workspace");
  const [baseline, setBaseline] = useState(() => fingerprint(options.initialSnapshot));
  const [status, setStatus] = useState("Workspace ready");
  const [error, setError] = useState<string | null>(null);
  const currentFingerprint = useMemo(() => fingerprint(options.snapshot), [options.snapshot]);
  const dirty = currentFingerprint !== baseline;

  const confirmDiscard = useCallback((action: string): boolean =>
    !dirty || window.confirm(`Discard unsaved workspace changes and ${action}?`), [dirty]);

  const reset = useCallback((label: string) => {
  ...
  const handleCommand = useCallback((command: AppCommandId): boolean => {
    setError(null);
    ...
    if (command === APP_COMMAND_ID.fileSaveWorkspaceAs) {
      void import("../model/workspaceSession").then(({ createWorkspaceDocument }) => {
        downloadText(
          createWorkspaceDocument(options.snapshot, metadata("Rate workspace")),
          "rate-of-closure.roc-workspace.json",
        );
        setBaseline(currentFingerprint);
        setStatus("Workspace copy downloaded");
      }).catch((caught: unknown) => {
        setError(caught instanceof Error ? caught.message : "Workspace download failed");
      });
      return true;
    }
    if (command === APP_COMMAND_ID.fileExportWorkspace) {
      try {
        downloadText(exportViewWorkspace(options.snapshot.viewWorkspace), "layout.roc-view.json");
        setStatus("View layout downloaded");
      } catch (caught) {
        setError(caught instanceof Error ? caught.message : "View export failed");
      }
      return true;
    }
    return command === APP_COMMAND_ID.fileSaveWorkspace ||
    ...
    void readFileText(file).then((text) => {
      try {
        ...
        void import("../model/workspaceSession").then(({ parseWorkspaceDocument }) => {
          ...
          setError(caught instanceof Error ? caught.message : "Workspace file is invalid");
        });
      } catch (caught) {
        setError(caught instanceof Error ? caught.message : "Workspace file is invalid");
      }
    }).catch((caught: unknown) => {
      setError(caught instanceof Error ? caught.message : "Workspace file read failed");
    });
  }, [confirmDiscard, options]);
