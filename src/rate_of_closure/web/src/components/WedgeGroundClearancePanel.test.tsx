...
describe("WedgeGroundClearancePanel", () => {
  it("makes sequence, clearance, provenance, and limits visible", () => {
    render(<WedgeGroundClearancePanel run={runSimulation(input)} scenario={scenario}
      club={getClub("Pitching Wedge")} />);
  ...
  it("does not present wedge-only claims for other club families", () => {
    const { container } = render(<WedgeGroundClearancePanel run={runSimulation(input)}
      scenario={scenario} club={getClub("Driver 10.5°")} />);
