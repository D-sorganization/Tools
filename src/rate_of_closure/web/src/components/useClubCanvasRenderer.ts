...
import {
  SHAFT_LEN, add, apply, headParts, project, rodrigues, type Vec3,
...
const STEPS = 48;
const COLORS = {
  face: getChartColor(0), body: "#8b949e", shaft: getChartColor(7),
  vRef: getChartColor(1), vPoint: getChartColor(3),
  impact: getChartColor(6), cog: getChartColor(2),
};
...
export function useClubCanvasRenderer({
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      const dpr = Number.isFinite(rawDpr) ? Math.max(1, Math.min(2.5, rawDpr)) : 1;
      const rect = canvas.getBoundingClientRect();
      const cssWidth = Number.isFinite(rect.width)
        ? Math.max(1, Math.min(1600, rect.width)) : 1;
      const desiredWidth = cssWidth * dpr;
      const desiredHeight = desiredWidth * 0.68;
      const pixelScale = Math.min(
        1, 2048 / desiredWidth, 2048 / desiredHeight,
      ...
      const { width: w, height: h } = staging;
        ...
        let shaftEnd: Vec3 | null = source.kind === "imported" ? null : parts.shaftEnd;
        ...
        if (hosel && shaftEnd) line([place(hosel), place(shaftEnd)], COLORS.shaft, 2.5);
        ...
        if (showCg) {
          ...
          const cgModel: Vec3 =
            generated && source.geometricCentroid
              ? add(source.geometricCentroid, shift) : [0, 0, 0];
      ...
      if (!visible) throw new Error("club canvas rendering context is unavailable");
    ...
    const safeDraw = (advance = false): boolean => {
      try {
        draw();
        if (advance) phaseRef.current = (phaseRef.current + speed / STEPS) % 1.0;
        onRenderError(null);
        return true;
      } catch (error) {
    ...
    const observer = typeof ResizeObserver === "undefined"
      ? null : new ResizeObserver(() => { safeDraw(false); });
