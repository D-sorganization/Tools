...
function CovariationWorkspace({ rows, savedSettings, onSettingsChange }: {
  rows: LaunchMonitorRow[];
  savedSettings?: CovariationUiSettings;
  onSettingsChange?: (settings: CovariationUiSettings) => void;
}) {
  const controller = useCovariationController(rows, { savedSettings, onSettingsChange });
  ...
  return <section aria-label="Within-player covariation" className="space-y-4">
    ...
    <LaunchMonitorCovariationControls controller={controller} />
