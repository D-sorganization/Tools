...
export function GroundPlayback3D({ timeline }: Props) {
  ...
  const visualPoints = useMemo(
    () => selectGroundPlaybackPath(timeline.result),
    [timeline],
  );
  const endpointLegend = timeline.endLabel === "Rest"
    ? "Total / rest"
    : timeline.endLabel === "Observed end"
      ? "Observed total / end"
      : timeline.endLabel;
  ...
  }), [timeline, visualPoints, frame, camera]);
  ...
  return (
    <section className="space-y-3" aria-label="Ground trajectory playback">
      <div className="flex flex-wrap items-center gap-2 text-xs">
        ...
        <button type="button" onClick={() => step(-1)} aria-label="Previous exact ground frame"
          className="rounded border border-slate-700 px-2 py-1">−1 frame</button>
        ...
        <button type="button" onClick={() => step(1)} aria-label="Next exact ground frame"
          className="rounded border border-slate-700 px-2 py-1">+1 frame</button>
        {PHASES.map((phase) => {
          const target = timeline.phaseTime(phase);
          return target === null ? null : (
            <button key={phase} type="button" onClick={() => jump(target)}
        ...
        <label className="flex items-center gap-1">Speed
          <select value={speed} onChange={(event) => setSpeed(Number(event.target.value))}
            aria-label="Ground playback speed" className="rounded border border-slate-700 bg-slate-900 px-1 py-1">
            {SPEEDS.map((value) => <option key={value} value={value}>{value}×</option>)}
          </select>
        </label>
        <label className="flex items-center gap-1">
          <input type="checkbox" checked={loop} onChange={(event) => setLoop(event.target.checked)}
            aria-label="Loop ground playback" />Loop
        </label>
        <button type="button" onClick={() => setCamera(INITIAL_CAMERA)} aria-label="Reset ground 3D view"
      ...
      <canvas ref={canvasRef} width={FLIGHT_PLAYBACK_LOGICAL_SIZE.width}
        ...
        onPointerMove={(event) => {
          ...
          dragRef.current = { pointerId: event.pointerId, x: event.clientX, y: event.clientY };
          setCamera((current) => ({ ...current, yawRad: current.yawRad + deltaX * 0.008,
        ...
        onWheel={(event) => {
          ...
          setCamera((current) => ({ ...current, zoom: Math.max(0.4, Math.min(4, current.zoom * factor)) }));
        }} />
      <div className="flex flex-wrap gap-3 text-xs text-slate-300" aria-label="Ground phase legend">
        {(["Impact", "Bounce", "Skid", "Roll", "Rest"] as const).map((label, index) => (
          <span key={label} className="rounded border px-2 py-0.5"
            style={{ borderColor: ["#ef476f", "#a78bfa", "#f59e0b", "#34d399", "#60a5fa"][index] }}>
            {label}
          </span>
        ))}
        <span>Carry / first contact</span><span>{endpointLegend}</span>
      </div>
