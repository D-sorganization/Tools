...
export function LaunchMonitorPerformanceWorkspace({ rows, sourceName }: Props) {
  ...
  return <section aria-label="Launch monitor performance analytics" className="space-y-4 rounded-xl border border-slate-800 bg-slate-900/60 p-4">
    ...
    <details title="Show formulas, provenance, and availability rules"><summary>Calculations and backing-data rules</summary><p className="text-xs text-slate-300">Negative lateral is yards left; positive is yards right. RMS = √mean(lateral²). Radial target error = hypot(target − carry, lateral) in yards and is not strokes gained. True SG = E(before) − 1 − E(after) and requires cited expected-stroke state. Session cumulative means equally weight attested sessions.</p></details>
    ...
    <div className="flex flex-wrap gap-3">{select("Expected strokes before column", before, setBefore)}{select("Expected strokes after column", after, setAfter)}<label>Baseline source URL<input aria-label="Strokes gained baseline source URL" title="Citable HTTP(S) source defining expected-stroke state." value={baseline} onChange={(event) => setBaseline(event.target.value)} className={`${field} ml-2`} /></label><button type="button" disabled={!before || !after || !baseline} onClick={runStrokes} title="Calculate true source-backed strokes gained." className={button}>Calculate Strokes Gained</button></div>
    {!strokes ? <p>Strokes gained unavailable: provide expected-stroke columns and a cited baseline.</p> : <p>Mean strokes gained: {strokes.mean.toFixed(3)} strokes · <a href={strokes.sourceUrl}>baseline source</a></p>}
