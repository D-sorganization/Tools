...
describe("VariationPanel analysis execution policy", () => {
  ...
  it("fails closed instead of sending ground inputs to scalar flight physics", async () => {
    ...
    await user.upload(
      screen.getByLabelText("Import variation plan JSON"),
      new File([planToJson(plan)], "ground-plan.json", {
        type: "application/json",
      }),
    );
    ...
    expect(
      screen.getByRole("status", { name: "Variation status" }),
  ...
  it("runs the explicit ground-mode wedge forgiveness objective", async () => {
    ...
    await user.click(
      screen.getByRole("checkbox", { name: "Analyze wedge chip forgiveness" }),
    );

    await user.click(screen.getByRole("button", { name: "Run Variation Study" }));

    expect(
      screen.getByRole("heading", { name: /conditional chip-shot forgiveness/i }),
    ).toBeInTheDocument();
    expect(screen.getByText(/Contact Cohorts — All 2 Trials/i)).toBeInTheDocument();
    expect(
      screen.getByRole("heading", { name: /forgiveness metric scatter/i }),
    ).toBeInTheDocument();
    expect(screen.getByText(/turf ranking disabled/i)).toBeInTheDocument();
    expect(
      screen.getByText(/illustrative reduced single-point/i),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: "Forgiveness Study JSON" }),
    ).toBeEnabled();
    expect(
      screen.getByRole("button", { name: "Forgiveness Trials CSV" }),
    ).toBeEnabled();
  });
