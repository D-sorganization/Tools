...
def _variation_payload(request: GroundRegionalVariationRequest) -> dict[str, Any]:
    # `dict(...)` is for the type checker: the changed-file MyPy gate runs
    # `--follow-imports=skip`, so `strict_json_object` resolves to `Any` and its
    # result would leave this `-> dict[str, Any]` function untyped. It already
    # returns a strict mapping, so copying it is cheap and loses nothing.
