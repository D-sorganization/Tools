...
PR `#4298` remains on `feat/4284-camera-snap-tracking` over declared base
`feat/4199-wind-workflow` (`51033696634c624f7c92c3822d98a66875ff90a0`).
Exact camera child `35d04ee4bb4d74045594d3ef0b274e2b09cc5a3b` was
normally merged first with native preset-axis polish
`37fa7f86dcdd7bf4225da34286e39f16837caba6` to form intermediate
`da9ffd820f1596c398d137c84a29bd77e96e5ec5`; refreshed flight-ground
composite `8137621d7c77d9fa6941f5cd51d3fd161ebf4300` is merged second.
Stale candidate `0eb576315498d4576b553e1ccc631345d9f6b23e` is not a parent.
The validated split kinetics implementation resolves the only production
conflict while camera controls/native polish, ground transfer, the
`CameraCommandId | None` guard, and capability serializer narrowing remain
intact. SPEC `1.16.26` records the local composite; publication, exact-head
protected CI, review, and release remain open.

Final local qualification passed 1,109 Python/PyQt/flight/ground tests;
TypeScript, zero-warning ESLint, 111 React files and 675 tests, four Playwright
camera tests at desktop and constrained HiDPI sizes, and the 194-module build;
Rust format, 137 `tools-core` tests, 12 `swing-core` tests, and warning-denied
Clippy; pinned Ruff 0.14.10 (51 paths), pinned MyPy 1.13 (37 production paths),
Bandit (37 production paths, no medium/high findings), module budgets,
changed-file policy/test contracts, manifest/schema, docs-governance, and
whitespace gates. PyQt ran offscreen; native rendered evidence remains the
previously recorded review rather than a new visual-release claim.
...
## 2026-08-11 Flight-ground transfer receives refreshed ground parent

Open PR `#4288` keeps actual branch `feat/4269-flight-ground-transfer` over
`feat/4268-ground-contract`. Exact published transfer child
`3e6e418f98856162420dcb3f5f13dedd48d40439` is the first parent and exact
refreshed ground-contract parent
`7a6f31b6cb36e05f903c591e207480d36f491c4d` is merged second normally. The
Python/TypeScript/Rust/PyO3/WASM transfer, signed terminal angular state,
physical contact evidence, fixtures, and cross-runtime contracts remain
child-owned. Historical #4283 remains merged at reviewed head
`9bbb98e16e435a0d4c74153b909f2ebfefbbce7a` and merge SHA
`c1827bbdc50a6e11cc475db2636b4e47a4c15416`; live dependency heads remain
separate. SPEC advances uniquely to `1.16.25`. Publication, protected CI,
review, approval, and release remain open. Qualification passes 302 focused
Python/PyQt tests with six optional wheel skips covered by 137 passing
`tools-core` Rust tests, plus Rust fmt/Clippy; 100 React tests across 12 files,
type-check/lint/audit/build; Ruff on 13 exact-parent Python paths; MyPy on nine
production paths; Bandit; and all manifest/schema, docs, size, changed-policy,
minimum-test, exact-base assertion, and diff gates.

## 2026-08-11 #4288 receives exact reconciled #4285 parent

Live PR evidence confirms #4288 uses branch
`feat/4269-flight-ground-transfer` over `feat/4268-ground-contract`; no
`feat/4270-flight-ground-transfer` ref exists. A normal child-first merge
combines exact published transfer child
`c39297d37a17d5d8d3520ed62c5563cdcc609cab` with exact published strict-ground
parent `d36551cead67e2544cf185584bff4d070ee663b3` without renaming a branch,
rebasing, retargeting, force-pushing, or rewriting history. All implementation,
test, and handoff paths merge automatically; only `SPEC.md` and manifest
carrier-head text conflict, and every history is retained. Qualified Python,
TypeScript, Rust, PyO3, and WASM transfer remains unchanged while inheriting the
current strict-ground and upstream repair ancestry. `SPEC.md` advances uniquely
to `1.14.26`; the manifest records exact #4285/#4288 open heads while preserving
historical merged #4283 identity, incomplete-physics limitations, and unreleased
claims. Pinned Ruff 0.14.10 and diff hygiene pass, as do 138 focused Python/PyQt
tests, strict manifest validation and eight contract tests, Rust format and 137
Rust tests, 25 React tests across three files, and docs governance. This merge
is local only; publication, protected CI, review, approval, and release remain
open.
...
## 2026-08-11 Ground contract receives refreshed live capability carrier

Open PR `#4285` keeps `feat/4268-ground-contract` over
`feat/4197-capability-observer`. Exact published ground-contract child
`d36551cead67e2544cf185584bff4d070ee663b3` is the first parent and exact
refreshed live capability carrier
`0179abbdd8c0166f125d87e4d6ab47aeca15df40` is merged second normally. The
strict `flight-to-ground/v1` contract, fixture, migrations, adapters,
dependency, and fail-closed no-physics boundary remain authoritative. The
parent contributes refreshed #4279-#4283 ancestry and its typed observation-
export boundary without implementation conflict. Historical PR #4283 remains
merged at head `9bbb98e16e435a0d4c74153b909f2ebfefbbce7a` and merge SHA
`c1827bbdc50a6e11cc475db2636b4e47a4c15416`. SPEC advances uniquely to
`1.16.24`. Qualification passes 256 Python/PyQt tests, 75 React tests in nine
files plus type-check/lint/build/audit, 12 Rust tests plus fmt/Clippy, pinned
Ruff on 27 Python paths, pinned MyPy on 20 production paths, Bandit, strict
manifest/schema, documentation, module-size, changed-policy, minimum-test,
exact-base assertion, and diff gates. Publication, protected CI, review,
approval, #4288 propagation, and release remain open.

## 2026-08-11 #4285 receives exact live #4283 carrier

Open PR `#4285` retains branch `feat/4268-ground-contract` and base
`feat/4197-capability-observer`. A normal child-first merge combines exact
published ground-contract child `584e4e53a7990053a346938c4e45538786887ec7`
with exact published live carrier parent
`6d10afb184ff79b11be0bdeb6bc2722b3c7d8912` without rebasing, retargeting,
force-pushing, or rewriting history. All implementation and test paths merge
automatically; conflicts are limited to both append-only handoffs, `SPEC.md`,
and manifest carrier-head text, and every history is retained. The strict
`flight-to-ground/v1` contract and fail-closed unimplemented-physics boundary
remain unchanged while inheriting the current capability/wind stack. `SPEC.md`
advances uniquely to `1.14.25`; the manifest records exact open #4282/#4285
heads while preserving historical merged #4283 identity and unreleased claims.
Pinned Ruff 0.14.10 and diff hygiene pass, as do 176 focused Python/PyQt tests,
strict manifest validation and eight contract tests, 12 `swing-core` Rust tests,
77 React tests across 12 files, and docs governance. This merge is local only;
publication, protected CI, review, approval, #4288 propagation, and release
remain open.
...
Historical PR `#4283` remains merged at head
`9bbb98e16e435a0d4c74153b909f2ebfefbbce7a` and merge SHA
`c1827bbdc50a6e11cc475db2636b4e47a4c15416`. Its preserved live branch starts
from exact carrier child `6d10afb184ff79b11be0bdeb6bc2722b3c7d8912`
and normally merges refreshed workflow parent
`51033696634c624f7c92c3822d98a66875ff90a0` second. Capability behavior and
history remain authoritative; implementation/tests merge automatically while
the refreshed #4279–#4282 ancestry is inherited. SPEC advances uniquely to
`1.16.23`, and the manifest keeps historical #4283 identity unchanged. The
capability-observation JSON exporter explicitly narrows its serializer result
to its declared `str` contract. Focused qualification passes 145 Python/PyQt
tests, 75 React tests across nine files, React type-check/lint/build, Ruff,
MyPy, Bandit, manifest/schema, documentation, size, minimum-test, and diff
gates. This is a local carrier update, not a reopened PR or protected release.
...
preserved branch `feat/4197-capability-observer` remains the dependency carrier
for open draft PR `#4285`. A normal child-first merge combines exact published
carrier child `d0b827667e61f0583e1dad0b3cbbca6819624d3c` with exact published
parent `8c6105d865abe130a68cfa35ffe6afb7eef850c8` without reopening a PR,
rebasing, retargeting, force-pushing, or rewriting history. All implementation
and test paths merge automatically; only both append-only handoffs and
`SPEC.md` conflict textually, and every history is retained. Capability-observer
functionality remains present while inheriting the current wind workflow and
five-file GUI/solver repair. `SPEC.md` advances uniquely to `1.14.24`; the
manifest records exact #4282 while preserving #4283's historical merged
identity, commit-bound evidence, and unreleased status. Pinned Ruff 0.14.10 and
diff hygiene pass, as do 110 focused Python/PyQt tests, strict manifest
validation and eight contract tests, 77 React tests across 12 files, and docs
governance. This carrier merge is local only; publication, protected CI, #4285
propagation, unresolved-thread checks, and release remain open.
...
## 2026-08-11 Wind workflow receives refreshed scalar-adapter parent
...
parent `73b7f6b45063ec71fa300a39d2185d3f935187b7` second. All implementation and
tests merge automatically; the responsive PyQt6/React workflow remains
authoritative while refreshed scalar/variation/workspace ancestry is inherited.
SPEC advances uniquely to `1.16.22`; the strict manifest records current
published carrier heads and remains unreleased. This local candidate changes no
remote or PR topology. Protected exact-head CI, independent review, approval,
downstream propagation, and release remain open. Local evidence passes pinned
Ruff on 57 Python files, pinned MyPy on 38 production files, Bandit, 150
Python/PyQt tests, 56 React tests, TypeScript, zero-warning ESLint, the
188-module build, strict manifest/schema, documentation, file-size,
changed-Python, minimum-test, and diff gates.

## 2026-08-11 #4282 receives exact reconciled #4281 parent

PR `#4282` retains branch `feat/4199-wind-workflow` and base
`feat/4199-wind-scalar-adapter`. A normal merge combines exact published child
`3faf9e9493c9ba37168d9f65ab10cafeedc2a72f` with exact published parent
`e934ba563a79bf95ee892fea8593972c81299eeb`. All implementation and test paths
merge automatically; only the append-only root handoff and `SPEC.md` conflict
textually, and both histories are retained. Responsive PyQt6/React wind
execution, progress/cancellation, controls, scatter, persistence/export,
capability integration, and release authority remain unchanged while inheriting
the current scalar-adapter ancestry. `SPEC.md` advances uniquely to `1.14.23`.
The campaign manifest advances to 2026-08-11 and records the exact published
#4281/#4282 heads while retaining commit-bound evidence and unreleased status.
Pinned Ruff 0.14.10 check/format passes on all 61 child and inherited Python
paths, as do diff hygiene, 124 focused Python/PyQt tests, strict manifest
validation and eight manifest contract tests, and 45 React tests across eight
files. The merge is local only; publication, protected exact-head CI,
independent review, unresolved-thread checks, dependency order, and release
remain open.
...
## 2026-08-11 Wind scalar adapter receives refreshed variation parent

PR `#4281` keeps branch `feat/4199-wind-scalar-adapter` and base
`feat/4144-variation-export-continuation`. Exact published wind child
`e934ba563a79bf95ee892fea8593972c81299eeb` is merged first and exact refreshed
variation-export parent `7fb5d7f489db49742b7bc82ef009570ad2502456`
second. Implementation and tests merge automatically: the child wind adapter
and scalar-ensemble contracts remain authoritative while current #4280
variation/workspace ancestry is inherited. Only append-only handoffs and SPEC
conflict; every record is retained under unique SPEC `1.16.21`. No campaign
manifest exists on either side of this pre-manifest stack. This local candidate
changes no remote or PR topology; protected CI, review, approval, dependency
propagation, and release remain open. Local evidence passes pinned Ruff 0.14.10
on five Python files, pinned MyPy 1.13 and Bandit on three production files,
101 focused Python/PyQt tests, 18 React tests, TypeScript, zero-warning ESLint,
the 166-module production build, documentation governance, 500/400-line
budgets, changed-Python policy, and diff hygiene.
...
published child `9321c1d2e091b8c7e5a4a83aa9ad726290e7fb5a` with exact published
parent `9b45bd5beca38370c1d541f8c488ef0edad08517`. All implementation and test
paths merge automatically; only both append-only handoffs and `SPEC.md`
conflict textually, and every history is retained. The Python/TypeScript
`scalar-ensemble/v1` and wind-strategy adapter behavior is unchanged while the
parent supplies current variation/export, workspace, kinetics, solver, layout,
and Qt typing repairs. `SPEC.md` advances monotonically and uniquely to
`1.14.22`; the later campaign manifest is absent from this pre-manifest stack.
Pinned Ruff 0.14.10 check/format and diff hygiene pass, as do 21 wind
scalar-adapter Python tests, 91 variation/workspace Python tests, 232 inherited
workspace/kinetics/impact Python tests, and 52 React tests across 13 files. The
merge is local only; publication, protected exact-head CI, independent review,
unresolved-thread checks, dependency order, and release remain open.
