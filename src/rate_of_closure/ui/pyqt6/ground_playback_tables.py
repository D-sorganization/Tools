...
MAX_VISIBLE_GROUND_ROWS = 256


@dataclass(frozen=True)
class GroundEvidenceWindow:
    """A bounded visible row window with exact retention disclosure."""
...
def evidence_window(
    rows: Iterable[Sequence[object]], *, total: int
) -> GroundEvidenceWindow:
    """Consume at most the UI row budget while reporting the exact total."""
    if total < 0:
        raise ValueError("total must be nonnegative")
    visible = tuple(
        tuple(row) for row in islice(rows, min(total, MAX_VISIBLE_GROUND_ROWS))
    )
    disclosure = None
    if total > len(visible):
        disclosure = (
            f"Showing first {len(visible)} of {total} validated rows; "
            "full result retained."
        )
    return GroundEvidenceWindow(visible, disclosure)
...
def _set_evidence_rows(
    table: QTableWidget, rows: Iterable[Sequence[object]], *, total: int
) -> None:
    window = evidence_window(rows, total=total)
    _set_rows(table, window.rows)
    if window.disclosure is None:
        table.setAccessibleDescription(f"Showing all {total} validated rows.")
        return
    disclosure_row = len(window.rows)
    table.setRowCount(disclosure_row + 1)
    table.setSpan(disclosure_row, 0, 1, table.columnCount())
    table.setItem(disclosure_row, 0, QTableWidgetItem(window.disclosure))
    table.setAccessibleDescription(window.disclosure)
    table.setToolTip(window.disclosure)
