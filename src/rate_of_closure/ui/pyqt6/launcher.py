...
def _launch_config(
    morris_client: Any, durable_ensemble_client: Any = None
) -> LaunchConfig:
    ...
    return LaunchConfig(
        ...
        window_kwargs={
            "morris_client": morris_client,
            "durable_ensemble_client": durable_ensemble_client,
        },
    )
...
def launch_rate_pyqt6() -> int:
    ...
    durable_client: DurableEnsembleAuthorityClient | None = None
    try:
        ...
        durable_client = DurableEnsembleAuthorityClient(
            runtime.base_url,
            runtime.authorization_headers,
        )
    ...
    try:
        return int(launch_pyqt6_app(_launch_config(client, durable_client)))
    finally:
