...
(Cheetham's AMM datasets and successors) report it:

* ``omega_plane`` — rotation about the swing-plane normal: the primary
  "swinging" rotation carrying the head along its arc.
* ``omega_shaft`` — rotation about the shaft axis: the closing/rolling
  release component. Positive closes the face for a right-handed golfer.

Frame convention (right-handed golfer):
    +Y toward the target, +Z up, +X toward the golfer's trail side
    (right of the target line). Negative path deviation = LEFT.

The reference point travels dead at the target with zero attack angle by
construction — every output is a *deviation from the reported delivery*,
which is exactly the quantity the geometric-center-vs-impact-point
question asks about. Published rate figures vary by study and player;
defaults here are representative mid-tour values and every number is an
...
class ImpactScenario:
    ...
    Args:
        ...
        omega_plane_dps: Angular velocity about the swing-plane normal,
            degrees per second. Positive swings the head toward the target.
        omega_shaft_dps: Angular velocity about the shaft axis, degrees
            per second. Positive closes the face (right-handed golfer).
        lie_angle_deg: Shaft angle from horizontal at impact, degrees.
            90 puts the shaft vertical (isolates pure horizontal closure).
        com_to_face_mm: Distance from the reference point forward to the
            face center, millimetres. Driver heads: roughly 25-40.
    ...
    omega_plane_dps: float = 2200.0
    omega_shaft_dps: float = 1700.0
    lie_angle_deg: float = 58.0
    com_to_face_mm: float = 35.0
...
class ImpactResult:
    ...
    Attributes:
        ...
        path_deviation_deg: Horizontal angle between the impact point's
            path and the reference path. Negative = left.
...
def _frame(lie_angle_deg: float) -> tuple[np.ndarray, np.ndarray]:
    ...
    The shaft points from the head up toward the hands: toward the golfer
    (-X) and up (+Z). The swing plane contains the shaft and the target
    line; its normal is ``unit(v_hat x shaft)``.
    """
    lie = math.radians(lie_angle_deg)
    shaft = np.array([-math.cos(lie), 0.0, math.sin(lie)])
...
def solve(scenario: ImpactScenario) -> ImpactResult:
    ...
    v_ref = np.array([0.0, scenario.clubhead_speed_mph / MPH_PER_MPS, 0.0])
    ...
    path_dev = math.degrees(math.atan2(v_point[0], v_point[1]))
    aoa_dev = math.degrees(math.atan2(v_point[2], math.hypot(v_point[0], v_point[1])))

    # The face normal is +Y at square impact. Its horizontal swing rate is
    # the vertical omega component; its loft-gain rate is the +X component.
    contact_s = scenario.contact_duration_us * 1e-6
    closure = math.degrees(omega[2]) * contact_s
    loft_gain = math.degrees(omega[0]) * contact_s
    ...
    result = ImpactResult(
        ...
        all(math.isfinite(getattr(result, f.name)) for f in fields(result)[:8]),
