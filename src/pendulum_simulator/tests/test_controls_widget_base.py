...
class DummyControls(ControlsWidgetBase):
    ...
    def _build_model_sections(self, layout): pass
    def _apply_preset(self, name): pass
    def get_params(self): return {}
    def _get_joint_names(self): return ["JointA", "JointB"]
...
def test_playback_methods(qapp):
    ...
    play_emitted = False
    def on_play(checked): nonlocal play_emitted; play_emitted = checked
...
def test_uai_or_parse(qapp, monkeypatch):
    ...
    monkeypatch.setitem(sys.modules, "upstream_drift_tools.ui.widgets.unit_aware_input", mock_module)
