...
export function makeGolferParams(p: GolferParams): GolferParams {
  ...
  assertPositive(p.m_r_upper, "m_r_upper");
  assertPositive(p.m_r_fore, "m_r_fore");
  assertPositive(p.m_l_upper, "m_l_upper");
  assertPositive(p.m_l_fore, "m_l_fore");
  assertPositive(p.m_club, "m_club");
  assertNonNeg(p.m_clubhead, "m_clubhead");

  assertPositive(p.L_hub, "L_hub");
  assertPositive(p.L_r_upper, "L_r_upper");
  assertPositive(p.L_r_fore, "L_r_fore");
  assertPositive(p.L_l_upper, "L_l_upper");
  assertPositive(p.L_l_fore, "L_l_fore");
  assertPositive(p.L_club, "L_club");

  assertNonNeg(p.d_rs, "d_rs");
  assertNonNeg(p.d_ls, "d_ls");
  assertNonNeg(p.grip_right, "grip_right");
  assertNonNeg(p.grip_left, "grip_left");
  assertNonNeg(p.g, "g");
...
export function constraintJacobian(
  ...
  const Jrh = [
    [
      p.d_rs * -Math.sin(theta_hub) +
        p.L_r_upper * Math.cos(theta_rs) +
        p.L_r_fore * Math.cos(theta_re),
      p.L_r_upper * Math.cos(theta_rs) + p.L_r_fore * Math.cos(theta_re),
      p.L_r_fore * Math.cos(theta_re),
      p.L_r_fore * Math.cos(theta_rh),
      0,
      0,
      0,
      0,
    ...
    [
      p.d_rs * Math.cos(theta_hub) +
        p.L_r_upper * Math.sin(theta_rs) +
        p.L_r_fore * Math.sin(theta_re),
      p.L_r_upper * Math.sin(theta_rs) + p.L_r_fore * Math.sin(theta_re),
      p.L_r_fore * Math.sin(theta_re),
      p.L_r_fore * Math.sin(theta_rh),
      0,
      0,
      0,
      0,
  ...
  const Jlh = [
    [
      -p.d_ls * -Math.sin(theta_hub) +
        p.L_l_upper * Math.cos(theta_ls) +
        p.L_l_fore * Math.cos(theta_le),
      0,
      0,
      0,
      p.L_l_upper * Math.cos(theta_ls) + p.L_l_fore * Math.cos(theta_le),
      p.L_l_fore * Math.cos(theta_le),
      p.L_l_fore * Math.cos(theta_lh),
      0,
    ],
    [
      -p.d_ls * Math.cos(theta_hub) +
        p.L_l_upper * Math.sin(theta_ls) +
        p.L_l_fore * Math.sin(theta_le),
      0,
      0,
      0,
      p.L_l_upper * Math.sin(theta_ls) + p.L_l_fore * Math.sin(theta_le),
      p.L_l_fore * Math.sin(theta_le),
      p.L_l_fore * Math.sin(theta_lh),
      0,
    ],
  ];
  ...
  for (let i = 0; i < 8; i++) {
    dclub_x_q[i] =
      Jrh[0][i] - p.grip_right * Math.cos(theta_club) * (i === 7 ? 1 : 0);
    dclub_y_q[i] =
      Jrh[1][i] - p.grip_right * -Math.sin(theta_club) * (i === 7 ? 1 : 0);
  }
...
export function massMatrix_golfer(
  ...
  M[0][0] *=
    p.m_hub + p.m_r_upper + p.m_r_fore + p.m_l_upper + p.m_l_fore + p.m_club;
  for (let i = 1; i < 8; i++) {
...
export function equationsOfMotion_golfer(
  ...
  // Friction
  const [tf0, tf1, tf2, tf3, tf4, tf5, tf6] = frictionTorqueVector_golfer(
    qdot,
    p,
  );
  ...
  const constraint_tau = phi.map((ph) => -penalty_gain * ph) as [
    number,
    number,
    number,
    number,
...
function rk4Step_golfer(
  ...
  const f = (s: StateGolfer, ti: number): StateGolfer =>
    equationsOfMotion_golfer(s, ti, p, tf);
