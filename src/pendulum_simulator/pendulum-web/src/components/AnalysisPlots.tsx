...
export const AnalysisPlots: React.FC<AnalysisPlotsProps> = ({
  result,
  units,
  detailPlot,
}) => {
  ...
  const angVelData = useMemo(
    () =>
      indices.map((i) => ({
        t: +t[i].toFixed(3),
        "Arms ω": +angularSpeedFromSI(states[i][2], units.angularSpeed).toFixed(
          2,
        ),
        "Shaft ω": +angularSpeedFromSI(
          states[i][2] + states[i][3],
          units.angularSpeed,
        ).toFixed(2),
        "Wrist rel ω": +angularSpeedFromSI(
          states[i][3],
          units.angularSpeed,
        ).toFixed(2),
      })),
  ...
  const energyData = useMemo(
    () =>
      indices.map((i) => ({
        t: +t[i].toFixed(3),
        KE: +energyFromSI(
          kineticEnergy(states[i], params),
          units.energy,
        ).toFixed(4),
        PE: +energyFromSI(
          potentialEnergy(states[i], params),
          units.energy,
        ).toFixed(4),
        Total: +energyFromSI(
          totalEnergy(states[i], params),
          units.energy,
        ).toFixed(4),
      })),
  ...
  const torqueData = useMemo(
    () =>
      indices.map((i) => {
        const [tf1, tf2] = frictionTorqueVector(
          states[i][2],
          states[i][3],
          params,
        );
  ...
  const baseForceData = useMemo(
    () =>
      indices.map((i) => {
        const qdd = computeAccelerations(
          states[i],
          t[i],
          params,
          torqueFunc,
...
function renderDetailPlot(plotId: string, data: PlotData): React.ReactElement {
  ...
  const plotConfigs: Record<
    string,
    {
      title: string;
      data: Record<string, number>[];
      lines: { key: string; color: string; dash?: string }[];
    }
  > = {
    angles: {
  ...
  if (!cfg)
    return <div style={{ color: "#ff5566" }}>Unknown plot: {plotId}</div>;
