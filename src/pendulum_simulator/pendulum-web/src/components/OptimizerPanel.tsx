  ...
  return (
    <div className="optimizer-panel">
      ...
      <p className="optimizer-desc">
        Maximize horizontal tip velocity at bottom of arc by optimizing
        polynomial torque coefficients.
      </p>
        ...
        <div className="optimizer-progress">
          ...
          {progress && (
            <div className="progress-info">
              ...
              <span>
                Best:{" "}
                {speedFromSI(progress.bestHorizontalSpeed, units.speed).toFixed(
                  2,
                )}{" "}
                {units.speed}
              </span>
      ...
      {result && !running && (
        <div className="optimizer-result">
          ...
          <div className="result-speed">
            Max horizontal tip speed:{" "}
            <strong>
              {speedFromSI(result.bestHorizontalSpeed, units.speed).toFixed(2)}{" "}
              {units.speed}
            </strong>
          </div>
          <div className="result-coeffs">
            <div>
              Shoulder: [
              {result.coeffsShoulder.map((c) => c.toFixed(3)).join(", ")}]
            </div>
            <div>
              Wrist: [{result.coeffsWrist.map((c) => c.toFixed(3)).join(", ")}]
            </div>
          </div>
