...
export function isGolferSimulationResult(
  result: unknown,
): result is SimulationResult_golfer {
  return hasParams(result) && "m_hub" in result.params;
}

export function isTripleSimulationResult(
  result: unknown,
): result is SimulationResult3 {
  return hasParams(result) && "m3" in result.params;
}

export function isDoubleSimulationResult(
  result: unknown,
