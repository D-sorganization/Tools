...
const SPEED_TO_SI: Record<SpeedUnit, number> = {
    'm/s': 1, 'ft/s': 0.3048, 'mph': 0.44704,
};

const ANGULAR_SPEED_TO_SI: Record<AngularSpeedUnit, number> = {
    'rad/s': 1, 'deg/s': Math.PI / 180, 'rpm': 2 * Math.PI / 60,
};
...
export function convertAngularSpeed(value: number, from: AngularSpeedUnit, to: AngularSpeedUnit): number {
    return fromSI(toSI(value, ANGULAR_SPEED_TO_SI[from]), ANGULAR_SPEED_TO_SI[to]);
}
...
export function convertAccel(value: number, from: AccelUnit, to: AccelUnit): number {
...
export function angularSpeedFromSI(siValue: number, unit: AngularSpeedUnit): number {
...
export const ANGULAR_SPEED_UNITS: AngularSpeedUnit[] = ['rad/s', 'deg/s', 'rpm'];
