...
| Replay regression corpus (#5222)                          | `tests/data/`, `tests/test_replay_corpus.py` | 7 short clips in manifest; 5 putts within tolerance, 2 negative controls assert no putt; marked slow |
| Accuracy validation harness & protocol (#5221)            | `validate.py`, `docs/putting_launch_monitor/validation.md` | `validate` subcommand logs CSV and running errors (+/-3% speed, +/-1 deg HLA); 7 unit tests |

Tests: `src/putting_launch_monitor/tests` (63 fast + 7 slow = 70) and `tests/camera` (8), all
...
2. **#5221 accuracy validation** — validation harness built (`validate.py`,
   CLI `validate`, CSV + running error stats, 7 unit tests) and rig evidence
   page documented (`docs/putting_launch_monitor/validation.md`). Ready for
   operator bay rolling session.
...
4. **#5222 replay corpus** (PR #5256) — 7 short clips under `tests/data/` with
   `manifest.json` as `slow` regression tests through `VideoFileSource`.
