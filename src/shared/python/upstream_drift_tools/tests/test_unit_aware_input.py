...
def mock_prefs(monkeypatch):
    ...
    monkeypatch.setattr(prefs, "get_preferred_unit", lambda cat: "m" if cat == "length" else "custom")
    monkeypatch.setattr(prefs, "convert_to_si", lambda val, cat, unit: val * 1000 if unit == "km" else val)
    monkeypatch.setattr(prefs, "convert_from_si", lambda val, cat, unit: val / 1000 if unit == "km" else val)

    with patch("upstream_drift_tools.ui.widgets.unit_aware_input.get_unit_preferences_manager", return_value=prefs):
        yield prefs

def test_unit_aware_input_init(qapp, mock_prefs):
...
def test_unit_aware_input_value_changed(qapp, mock_prefs):
...
def test_unit_aware_input_unit_changed(qapp, mock_prefs):
    widget = UnitAwareInput(category="length")
...
def test_unit_aware_input_setters(qapp, mock_prefs):
    widget = UnitAwareInput(category="length")
...
def test_unit_aware_input_set_value(qapp, mock_prefs):
    widget = UnitAwareInput(category="length")
...
def test_unit_aware_display_init(qapp, mock_prefs):
...
def test_unit_aware_display_set_value_si(qapp, mock_prefs):
...
def test_unit_aware_display_unit_changed(qapp, mock_prefs):
    disp = UnitAwareDisplay(category="length")
...
def test_preference_changed_input(qapp, mock_prefs):
    widget = UnitAwareInput(category="length")
...
def test_preference_changed_display(qapp, mock_prefs):
    disp = UnitAwareDisplay(category="length")
