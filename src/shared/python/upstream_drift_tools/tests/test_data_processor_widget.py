...
def test_initialization(empty_widget):
...
def test_setup_state_management(qapp):
    widget = DataProcessorWidget()
...
def test_open_file_success(empty_widget):
...
def test_open_file_failure(empty_widget):
    with patch("PyQt6.QtWidgets.QFileDialog.getOpenFileName", return_value=("test.csv", "")):
        with patch.object(empty_widget.engine, "load_file") as mock_load:
            res = MagicMock()
            res.success = False
...
def test_save_file_existing_error(populated_widget):
    populated_widget.current_file = "test.csv"
    with patch.object(populated_widget.engine, "export_data") as mock_exp:
        res = MagicMock()
        res.success = False
...
def test_undo_redo_reset(populated_widget):
...
def test_update_table_pagination(populated_widget):
...
def test_refresh_statistics_empty(empty_widget):
    # Should do nothing
    empty_widget.refresh_statistics()
    assert empty_widget.row_count_label.text() == "0"

def test_update_column_stats(populated_widget):
...
def test_on_data_loaded_modified(empty_widget):
