...
class TestExceptionHierarchy:
    def test_data_processing_error_is_exception(self) -> Any:
        err = DataProcessingError("base error")
        assert isinstance(err, Exception)

    def test_data_not_loaded_is_data_processing_error(self) -> Any:
        err = DataNotLoadedError("no data")
        assert isinstance(err, DataProcessingError)

    def test_column_not_found_is_data_processing_error(self) -> Any:
        err = ColumnNotFoundError("col")
        assert isinstance(err, DataProcessingError)

    def test_file_io_error_is_data_processing_error(self) -> Any:
        err = FileIOError("bad file")
        assert isinstance(err, DataProcessingError)

    def test_transformation_error_is_data_processing_error(self) -> Any:
        err = TransformationError("bad transform")
        assert isinstance(err, DataProcessingError)

    def test_filter_error_is_data_processing_error(self) -> Any:
        err = FilterError("bad filter")
        assert isinstance(err, DataProcessingError)

    def test_fit_error_is_data_processing_error(self) -> Any:
        err = FitError("fit failed")
        assert isinstance(err, DataProcessingError)

    def test_unsupported_operation_is_data_processing_error(self) -> Any:
        err = UnsupportedOperationError("not supported")
        assert isinstance(err, DataProcessingError)


class TestColumnNotFoundError:
    def test_message_without_available(self) -> Any:
        err = ColumnNotFoundError("my_col")
        assert "my_col" in str(err)
        assert err.column == "my_col"
        assert err.available == []

    def test_message_with_available_columns(self) -> Any:
    ...
    def test_message_with_empty_available(self) -> Any:
    ...
    def test_message_with_none_available(self) -> Any:
