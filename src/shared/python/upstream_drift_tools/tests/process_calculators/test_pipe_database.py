...
class TestGetRoughness:
    ...
    def test_all_materials_resolve_in_meters(self) -> Any:
...
class TestGetPipeSpec:
    def test_4_inch_schedule_40(self) -> Any:
    ...
    def test_custom_material_stored(self) -> Any:
    ...
    def test_unknown_size_raises(self) -> Any:
        """Lines 310-313: unknown key → ValueError."""
        with pytest.raises(ValueError):
            get_pipe_spec("99", "40")

    def test_known_schedule_xxthin(self) -> Any:
        """NPS 1/2 Schedule 5S."""
        spec = get_pipe_spec("1/2", "5S")
        assert abs(spec.outer_diameter - 21.3) < 0.01

    def test_dimensions_are_positive(self) -> Any:
...
class TestListAvailableSizes:
    def test_returns_list_of_strings(self) -> Any:
        """Lines 328-333: returns sorted list."""
        sizes = list_available_sizes()
        assert isinstance(sizes, list)
        assert len(sizes) > 0

    def test_contains_expected_sizes(self) -> Any:
        sizes = list_available_sizes()
        assert "2" in sizes
        assert "4" in sizes
        assert "6" in sizes
    ...
    def test_smallest_size_is_half_inch(self) -> Any:
    ...
    def test_no_duplicates(self) -> Any:
        sizes = list_available_sizes()
...
class TestListSchedulesForSize:
    def test_4_inch_has_standard_schedules(self) -> Any:
    ...
    def test_unknown_size_returns_empty(self) -> Any:
        """If size doesn't exist, returns empty list."""
        schedules = list_schedules_for_size("99")
        assert schedules == []

    def test_returns_all_available_schedules(self) -> Any:
...
class TestCreateCustomPipe:
    def test_defaults_to_commercial_steel(self) -> Any:
