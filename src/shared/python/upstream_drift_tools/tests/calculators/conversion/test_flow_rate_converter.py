...
class TestMassToMass:
    def test_kg_h_to_lb_hr(self) -> Any:
    ...
    def test_kg_s_to_kg_s_identity(self) -> Any:
        result = mass_to_mass(5.0, "kg/s", "kg/s")
        assert abs(result - 5.0) < 1e-10

    def test_lb_hr_to_kg_h(self) -> Any:
    ...
    def test_unknown_from_unit_raises(self) -> Any:
        with pytest.raises(ValueError, match="Unknown mass flow unit"):
            mass_to_mass(1.0, "bad_unit", "kg/s")

    def test_unknown_to_unit_raises(self) -> Any:
        with pytest.raises(ValueError, match="Unknown mass flow unit"):
...
class TestMolarToMolar:
    def test_kmol_h_to_mol_s(self) -> Any:
...
class TestMassToMolar:
    def test_air_100_kg_h_to_kmol_h(self) -> Any:
    ...
    def test_non_positive_mw_raises(self) -> Any:
        with pytest.raises(ValueError):
            mass_to_molar(1.0, "kg/s", 0.0, "mol/s")

    def test_unknown_mass_unit_raises(self) -> Any:
        with pytest.raises(ValueError, match="mass flow"):
            mass_to_molar(1.0, "bad_mass", 29.0, "mol/s")

    def test_unknown_molar_unit_raises(self) -> Any:
...
class TestMassToVolumetricActual:
    def test_100_kg_h_at_1p2(self) -> Any:
...
class TestMassToStandardVolumetric:
    def test_100_kg_h_ch4_stp(self) -> Any:
        # CH4 MW = 16 kg/kmol
...
class TestScfmAcfm:
    def test_scfm_to_acfm_at_standard_conditions(self) -> Any:
    ...
    def test_scfm_to_acfm_higher_temp(self) -> Any:
        # Higher actual temp → larger ACFM
        acfm = scfm_to_acfm(1000.0, 600.0, 101325.0, "SCFM")
        assert acfm > 1000.0

    def test_acfm_to_scfm_roundtrip(self) -> Any:
    ...
    def test_non_positive_temp_raises(self) -> Any:
        with pytest.raises(ValueError, match="positive"):
            scfm_to_acfm(1000.0, -10.0, 101325.0)

    def test_non_positive_pressure_raises(self) -> Any:
        with pytest.raises(ValueError, match="positive"):
...
class TestConvertFlowRateToMass:
    ...
    def test_inf_value_raises(self) -> Any:
        with pytest.raises(ValueError, match="finite"):
...
class TestInternalHelpers:
    def test_standard_density_stp_air(self) -> Any:
    ...
    def test_from_kg_per_s_identity(self) -> Any:
        result = _from_kg_per_s(1.0, "kg/s")
        assert abs(result - 1.0) < 1e-12

    def test_from_kg_per_s_to_kg_h(self) -> Any:
        # 1 kg/s = 3600 kg/h
