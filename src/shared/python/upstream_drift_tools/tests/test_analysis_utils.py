...
class TestEvaluateOutput:
    ...
    def test_successful_calculation_returns_output_value(self) -> Any:
    ...
    def test_hhv_injected_when_positive(self) -> Any:
    ...
    def test_hhv_not_injected_when_zero(self) -> Any:
    ...
    def test_overrides_applied_to_params(self) -> Any:
    ...
    def test_no_overrides(self) -> Any:
    ...
    def test_missing_output_variable_returns_zero(self) -> Any:
    ...
    def test_engine_raises_value_error_returns_zeros(self) -> Any:
        self.engine.calculate.side_effect = ValueError("invalid params")
        value, state, composition = evaluate_output(self.engine, {}, 0.0, "out")
        assert value == pytest.approx(0.0)

    def test_engine_raises_zero_division_returns_zeros(self) -> Any:
        self.engine.calculate.side_effect = ZeroDivisionError("division by zero")
        value, state, composition = evaluate_output(self.engine, {}, 0.0, "out")
        assert value == pytest.approx(0.0)

    def test_engine_raises_overflow_returns_zeros(self) -> Any:
        self.engine.calculate.side_effect = OverflowError("overflow")
        value, state, composition = evaluate_output(self.engine, {}, 0.0, "out")
        assert value == pytest.approx(0.0)

    def test_engine_returns_non_dict_returns_zeros(self) -> Any:
    ...
    def test_engine_returns_list_returns_zeros(self) -> Any:
        self.engine.calculate.return_value = [1, 2, 3]
        value, _, _ = evaluate_output(self.engine, {}, 0.0, "out")
        assert value == pytest.approx(0.0)

    def test_base_params_not_mutated(self) -> Any:
