...
class UnitConversionService:
    ...
    def __init__(self, enable_validation: bool = True) -> None:
        ...
        if not (enable_validation is not None): raise ValueError(f"Assertion failed: { enable_validation is not None }, "enable_validation must be provided"")
    ...
    def convert(
        ...
        if not (value is not None): raise ValueError(f"Assertion failed: { value is not None }, "value must be provided"")
    ...
    def _normalize_unit(self, unit: str) -> str:
        ...
        if not (unit is not None): raise ValueError(f"Assertion failed: { unit is not None }, "unit must be provided"")
    ...
    def _gas_flow_to_standard_m3h(
        ...
        if from_unit == "ACFM":
            if not (temperature is not None): raise ValueError(f"Assertion failed: { temperature is not None }")
            if not (pressure is not None): raise ValueError(f"Assertion failed: { pressure is not None }")
    ...
    def _user_unit_warnings(
        ...
        if not (from_unit is not None): raise ValueError(f"Assertion failed: { from_unit is not None }, "from_unit must be provided"")
        warnings: list[str] = []
        seen: set[str] = set()
    ...
    def _tar_to_mg_nm3(
        ...
        if from_key == "ppm_mass":
            if not (molecular_weight is not None): raise ValueError(f"Assertion failed: { molecular_weight is not None }")
    ...
    def compressibility_factor(
        ...
        if not (gas_type is not None): raise ValueError(f"Assertion failed: { gas_type is not None }, "gas_type must be provided"")
