...
T3 #5072 publishes finite-grip response at `28c45eb15`. Stationary arbitrary-node grips now supply consistent preload curvature, all-node equilibrium and full M/G/C/K operators, with gyroscopic transport and damping separate. The existing clamped solver shares the bounded iteration and preserves its root constraint. All 38 focused coupled/clamped/grip tests and 646 Linux golf/API tests pass (two optional CAD skips; one BLAS thread and unchanged 60-second limit). `GRIPPED_CHAIN.md` derives the coupling and independent rod/rotation/world-balance/energy controls. All nine final manual gates pass; published at eeea63b47 with all normal hooks passing. Stability, full grip-supported FRF/bandwidth and time evolution with anchor/frame work remain open, followed by impact/acoustic/physical qualification. KIT metadata is not calibrated payload or golfer evidence.
