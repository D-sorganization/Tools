...
T3 #5072 adds tensile FEM, passive grip, stationary shaft/head, rotating transport, clamped roots, consistent SE(3) kinetics and deformed-frame inertia; 554 golf/API tests pass (two optional CAD skips), including 147 focused contracts. `DEFORMED_FRAME_INERTIA.md` derives rotating residual/Jacobian/gyroscopic terms; `LOADED_INERTIA.md` derives section mass and its energy identity. Both reuse the physical-body kernels. `LOADED_STATE_REVIEW.md` records strain-bounded roots, reactions and the moving-material Newton Jacobian. Stability remains unqualified. `docs/development/impact-acoustics/ROTATING_TRANSPORT.md`, `SHAFT_LINEAR_SYSTEM.md`, `GRIP_IMPEDANCE.md` and `SHAFT_PRESTRESS.md` retain prior derivations. Complete rotating loaded equilibrium, moving work and FRF/convergence remain open; `PROGRESS.md` retains the full matrix.
