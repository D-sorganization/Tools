...
T3 #5072 publishes finite-grip response at `28c45eb15` through all normal hooks. It shares existing Gram-factor energy algebra and returns both physical reactions with observer-specific powers. All 37 focused grip tests and 634 Linux golf/API tests pass (two optional CAD skips; one BLAS thread and unchanged 60-second limit). `FINITE_GRIP_RESPONSE.md` derives the law and independent energy-gradient/accelerating-observer controls. The local API retains its small-rotation semantics. Loaded root balance/tangent, time evolution with anchor/frame work, stability and full bandwidth/impact/acoustic qualification remain open. All nine final manual gates pass. `DATA_CANDIDATES.md` catalogs measured KIT boundary candidates without payload inspection or fitting. No calibrated hand mass or sound prediction is inferred.
