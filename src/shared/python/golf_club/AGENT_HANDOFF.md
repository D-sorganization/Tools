...
T3 #5072 adds tensile FEM, passive grip, stationary shaft/head, rotating transport, clamped roots and consistent SE(3) kinetic quadrature; 543 golf/API tests pass (two optional CAD skips), including 136 focused contracts. `LOADED_INERTIA.md` derives section mass, material transport and the energy identity, reusing existing physical-body kernels. `LOADED_STATE_REVIEW.md` records strain-bounded roots, reactions and the moving-material Newton Jacobian. Stability remains unqualified. `docs/development/impact-acoustics/ROTATING_TRANSPORT.md`, `SHAFT_LINEAR_SYSTEM.md`, `GRIP_IMPEDANCE.md` and `SHAFT_PRESTRESS.md` retain prior derivations. Complete rotating loaded equilibrium, moving work and FRF/convergence remain open; `PROGRESS.md` retains the full matrix.
