...
T3 #5072 publishes frozen tip response at `f37e37bd2`. New private moving-anchor kinematics pass eight independent controls plus 23 existing grip tests; broad Linux golf/API regression passes 628 tests with two optional CAD skips, one BLAS thread and the unchanged 60-second limit. `MOVING_GRIP_KINEMATICS.md` records the earlier timeout and derives actual separation, rotation rates, acceleration transport and dual power ports using existing shaft Jacobians. The existing local grip API remains a small-rotation law; nonlinear constitutive extension, root balance/tangent, moving boundary work in time evolution, stability and full bandwidth/impact/acoustic qualification remain open. `PROGRESS.md` retains the full requirement/evidence matrix and prior loaded-operator, head-attachment, spectrum and response checkpoints. No measured hand mass or sound prediction is inferred.
