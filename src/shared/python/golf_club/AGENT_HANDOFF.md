...
T3 #5072 publishes rigid attachment at `42f229951`. Private tip response passes 41 harmonic/spectral Windows tests and nine API tests; 620 Linux golf/API tests and all nine manual gates pass (two optional CAD skips, three optional-plugin warnings). `FROZEN_TIP_RESPONSE.md` derives point ports, support transfer and coefficient-cancellation refusal. T3 also adds rigid nodal attachment through existing full-inertia quadrature: 12 final Windows tests and 601 Linux golf/API tests pass, with two optional CAD skips. `LOADED_BODY_ATTACHMENT.md` derives offset-head mechanics and principal-axis rod/tip-mass convergence. Frozen spectra are published at `cbc43b659`. T3 adds private frozen spectra (`FROZEN_SPECTRA.md`): 22 final Windows tests retain growing/defective modes and check both state/quadratic residuals; stability stays unqualified. Final broader Linux checkpoint: 589 passes, two CAD skips. It also adds tensile FEM, passive grip, stationary shaft/head, rotating transport, clamped roots, consistent SE(3) kinetics and deformed-frame inertia; 567 Linux golf/API tests pass (two optional CAD skips); seven new Windows operator tests pass. `LOADED_DYNAMIC_OPERATORS.md` adds current-geometry M/G/K, physical energy/Coriolis controls and guided axial frequency convergence, and records the Windows report-sweep timeout. `ROTATING_LOADED_ROOTS.md` adds consistent loaded-chain roots, continuum extension/support and complete tangent checks. `DEFORMED_FRAME_INERTIA.md` derives rotating residual/Jacobian/gyroscopic terms; `LOADED_INERTIA.md` derives section mass and its energy identity. Both reuse the physical-body kernels. `LOADED_STATE_REVIEW.md` records strain-bounded roots, reactions and the moving-material Newton Jacobian. Stability remains unqualified. `docs/development/impact-acoustics/ROTATING_TRANSPORT.md`, `SHAFT_LINEAR_SYSTEM.md`, `GRIP_IMPEDANCE.md` and `SHAFT_PRESTRESS.md` retain prior derivations. Complete stability, moving work, passive grip and FRF/convergence remain open; `PROGRESS.md` retains the full matrix.
