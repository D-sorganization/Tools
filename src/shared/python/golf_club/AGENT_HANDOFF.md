...
T3 #5072 adds tensile FEM, passive grip, stationary spatial shaft/head, rotating transport and private clamped-chain equilibrium candidates; 524 golf/API tests pass (two optional CAD skips), including 117 focused derivative/equilibrium contracts. `LOADED_STATE_REVIEW.md` derives section/load work, complete chain curvature and the moving-material Newton Jacobian. Explicit strain bounds, support reactions and refusal of unfinished states are verified; stability remains unqualified. `docs/development/impact-acoustics/ROTATING_TRANSPORT.md`, `SHAFT_LINEAR_SYSTEM.md`, `GRIP_IMPEDANCE.md` and `SHAFT_PRESTRESS.md` record derivations and limits. Loaded inertia, moving work and FRF/convergence remain open; `PROGRESS.md` retains the full program matrix.
