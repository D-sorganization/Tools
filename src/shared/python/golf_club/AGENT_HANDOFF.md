...
T3 #5072: finite-grip response published at `12bcf3d83` (665 Linux golf/API passes). New private spectra retain separate G/C, all-node balance and frame/support metadata; 49 spectral controls pass, including independent two-node/continuum damped poles and damping-induced instability. Shared eigensolver diagnostics preserve the legacy contract. See `GRIPPED_SPECTRUM.md`; 692 Linux golf/API tests and nine final manual/type/lint gates pass; published at `97d46055c` through normal hooks with remote SHA verified. New autonomous ODE decay assessment passes 39 focused and 731 Linux golf/API tests (two optional CAD skips, three plugin warnings); hook-style mypy and Ruff pass. Shared plant validation, explicit Lyapunov/error margins and nonnormal transient controls are documented in AUTONOMOUS_DECAY.md. All nine final structural/manual gates and repository Ruff pass; published at 58f33e403 through all normal hooks, with remote SHA verified. The old inventory classifier still misses this calculation; integrate #5103 before combined delivery. Merge 96ba67aa6 integrates main 287767dfa without changing golf implementation/tests; 772 combined golf/signal/API tests pass (two optional CAD skips), with eight non-inventory manual gates passing; inventory, pinned Ruff and all merge hooks pass; published at 5e4d82314 through all hooks after 173 affected regressions and the Qt-absent control. New private affine response retains residual forcing; 97 Windows and 106 Linux/API controls pass; all nine final gates and repository Ruff pass; normal publication pending; see AFFINE_RESPONSE.md. Next operating-model work retains residual forcing. Driven/nonlinear stability, bandwidth and physical/acoustic work remain.
