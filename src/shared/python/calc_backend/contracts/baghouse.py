...
class BaghouseRequest(BaseModel):
    ...
    composition: dict[str, float] = Field(
        ..., description="Gas composition as mole fractions"
    )
    ...
    drum_volume_m3: float = Field(
        default=2.0, gt=0, description="Collection drum volume [m3]"
    )
    solid_density_kg_m3: float = Field(
        default=500.0, gt=0, description="Density of collected solids [kg/m3]"
    )
    bag_area_ft2: float = Field(
        default=5000.0, gt=0, description="Total bag filter area [ft2]"
    )
