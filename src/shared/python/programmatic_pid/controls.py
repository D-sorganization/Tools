...
def orthogonal_control_route(
    ...
    if corridor_y is not None:
        ...
        return dedupe_points([(sx, sy), (sx, detour_y), (ex, detour_y), (ex, ey)])  # type: ignore[no-any-return]  # noqa: E501
    offset_band = (route_index % 5) - 2
    center_x = sx + (ex - sx) * 0.5 + offset_band * max(to_float(spread), 0.5)
    return dedupe_points([(sx, sy), (center_x, sy), (center_x, ey), (ex, ey)])  # type: ignore[no-any-return]  # noqa: E501
