...
def test_bounce_cancellation_is_typed_and_retains_request_identity(
    ...
    settled = _settled_prefix(request)
    # Derive the termination times from the prefix instead of hardcoding the
    # request's separation time. `RepeatedBounceResult` requires
    # `termination.time_s` to equal the final trajectory point and
    # `elapsed_time_s` to equal the span since the first event, so the literals
    # this test used raised ValueError before it could assert anything about
    # cancellation typing — which is all it is meant to check.
    final_time_s = settled.trajectory[-1].time_s
    elapsed_s = final_time_s - settled.events[0].time_s if settled.events else 0.0
    prefix = replace(
        settled,
        handoff_state=None,
