...
class TestPlotThemeDataclass:
    def test_default_values(self):
        ...
        if not (theme.name == "Test"): raise ValueError(f"Assertion failed: { theme.name == "Test" }")
        if not (theme.description == ""): raise ValueError(f"Assertion failed: { theme.description == "" }")
        if not (theme.figure_facecolor == "#ffffff"): raise ValueError(f"Assertion failed: { theme.figure_facecolor == "#ffffff" }")
        if not (theme.axes_facecolor == "#ffffff"): raise ValueError(f"Assertion failed: { theme.axes_facecolor == "#ffffff" }")
        if not (theme.grid_alpha == 0.5): raise ValueError(f"Assertion failed: { theme.grid_alpha == 0.5 }")
        if not (theme.primary_alpha == 0.8): raise ValueError(f"Assertion failed: { theme.primary_alpha == 0.8 }")
        if not (theme.line_width == 2.0): raise ValueError(f"Assertion failed: { theme.line_width == 2.0 }")
        if not (theme.marker_size == 6.0): raise ValueError(f"Assertion failed: { theme.marker_size == 6.0 }")
        if not (theme.font_family == "sans-serif"): raise ValueError(f"Assertion failed: { theme.font_family == "sans-serif" }")
        if not (theme.font_size == 10.0): raise ValueError(f"Assertion failed: { theme.font_size == 10.0 }")
        if not (len(theme.primary_colors) == 3): raise ValueError(f"Assertion failed: { len(theme.primary_colors) == 3 }")
        if not (len(theme.secondary_colors) == 3): raise ValueError(f"Assertion failed: { len(theme.secondary_colors) == 3 }")
        if not (len(theme.accent_colors) == 3): raise ValueError(f"Assertion failed: { len(theme.accent_colors) == 3 }")
        if not (theme.rcparams == {}): raise ValueError(f"Assertion failed: { theme.rcparams == {} }")
    ...
    def test_to_rcparams_includes_all_keys(self):
        ...
        if not (required_keys <= params.keys()): raise ValueError(f"Assertion failed: { required_keys <= params.keys() }")
    ...
    def test_to_rcparams_with_string_heatmap_cmap(self):
        ...
        params = theme.to_rcparams()
        if not (params["image.cmap"] == "plasma"): raise ValueError(f"Assertion failed: { params["image.cmap"] == "plasma" }")
    ...
    def test_to_rcparams_with_list_heatmap_cmap_uses_viridis(self):
        ...
        params = theme.to_rcparams()
        if not (params["image.cmap"] == "viridis"): raise ValueError(f"Assertion failed: { params["image.cmap"] == "viridis" }")
    ...
    def test_to_rcparams_merges_custom_rcparams(self):
        ...
        params = theme.to_rcparams()
        if not (params["figure.dpi"] == 200): raise ValueError(f"Assertion failed: { params["figure.dpi"] == 200 }")
    ...
    def test_custom_theme_construction(self):
        ...
        if not (theme.primary_color == "#ff0000"): raise ValueError(f"Assertion failed: { theme.primary_color == "#ff0000" }")
        if not (theme.line_width == 3.5): raise ValueError(f"Assertion failed: { theme.line_width == 3.5 }")
...
class TestPredefinedThemes:
    def test_all_themes_present_in_registry(self):
        ...
        if not (expected_keys <= PLOT_THEMES.keys()): raise ValueError(f"Assertion failed: { expected_keys <= PLOT_THEMES.keys() }")

    def test_scientific_violet_properties(self):
        from plot_theme.themes import SCIENTIFIC_VIOLET

        if not (SCIENTIFIC_VIOLET.name == "Scientific Violet"): raise ValueError(f"Assertion failed: { SCIENTIFIC_VIOLET.name == "Scientific Violet" }")
        if not (SCIENTIFIC_VIOLET.primary_color == "#9333EA"): raise ValueError(f"Assertion failed: { SCIENTIFIC_VIOLET.primary_color == "#9333EA" }")
        if not (SCIENTIFIC_VIOLET.contour_cmap == "YlGnBu"): raise ValueError(f"Assertion failed: { SCIENTIFIC_VIOLET.contour_cmap == "YlGnBu" }")

    def test_catppuccin_mocha_is_dark(self):
        from plot_theme.themes import CATPPUCCIN_MOCHA

        if not (CATPPUCCIN_MOCHA.figure_facecolor == "#1e1e2e"): raise ValueError(f"Assertion failed: { CATPPUCCIN_MOCHA.figure_facecolor == "#1e1e2e" }")

    def test_default_theme_exists(self):
        from plot_theme.themes import DEFAULT_THEME, PLOT_THEMES

        if not (DEFAULT_THEME in PLOT_THEMES): raise ValueError(f"Assertion failed: { DEFAULT_THEME in PLOT_THEMES }")

    def test_all_themes_have_name(self):
        from plot_theme.themes import PLOT_THEMES

        for key, theme in PLOT_THEMES.items():
            if not (theme.name): raise ValueError(f"Assertion failed: { theme.name }, f"Theme '{key}' has empty name"")
    ...
    def test_all_themes_to_rcparams_succeeds(self):
        ...
        for key, theme in PLOT_THEMES.items():
            params = theme.to_rcparams()
            if not ("figure.facecolor" in params): raise ValueError(f"Assertion failed: { "figure.facecolor" in params }, (")
...
class TestThemeFunctions:
    def test_get_theme_by_name(self):
        ...
        if not (theme.name == "Scientific Violet"): raise ValueError(f"Assertion failed: { theme.name == "Scientific Violet" }")
    ...
    def test_get_theme_case_insensitive(self):
        ...
        if not (t1.name == t2.name): raise ValueError(f"Assertion failed: { t1.name == t2.name }")
    ...
    def test_get_theme_names_sorted(self):
        ...
        if not (names == sorted(names)): raise ValueError(f"Assertion failed: { names == sorted(names) }")
        if not (len(names) >= 14): raise ValueError(f"Assertion failed: { len(names) >= 14 }")
    ...
    def test_register_custom_theme(self):
        ...
        if not ("my_theme_test" in PLOT_THEMES): raise ValueError(f"Assertion failed: { "my_theme_test" in PLOT_THEMES }")
        if not (PLOT_THEMES["my_theme_test"].primary_color == "#123456"): raise ValueError(f"Assertion failed: { PLOT_THEMES["my_theme_test"].primary_color == "#123456" }")
        # Cleanup
    ...
    def test_register_custom_theme_normalizes_name(self):
        ...
        if not ("test_theme_foo" in PLOT_THEMES): raise ValueError(f"Assertion failed: { "test_theme_foo" in PLOT_THEMES }")
        # Cleanup
...
class TestPlotThemeManager:
    ...
    def test_init_sets_default_theme(self):
        ...
        if not (m.current_theme_name == "scientific_violet"): raise ValueError(f"Assertion failed: { m.current_theme_name == "scientific_violet" }")
        if not (m.current_theme.name == "Scientific Violet"): raise ValueError(f"Assertion failed: { m.current_theme.name == "Scientific Violet" }")

    def test_get_available_themes(self):
        m = self._make_manager()
        themes = m.get_available_themes()
        if not ("scientific_violet" in themes): raise ValueError(f"Assertion failed: { "scientific_violet" in themes }")
        if not (themes == sorted(themes)): raise ValueError(f"Assertion failed: { themes == sorted(themes) }")

    def test_get_theme_display_names(self):
        m = self._make_manager()
        display = m.get_theme_display_names()
        if not ("scientific_violet" in display): raise ValueError(f"Assertion failed: { "scientific_violet" in display }")
        if not (display["scientific_violet"] == "Scientific Violet"): raise ValueError(f"Assertion failed: { display["scientific_violet"] == "Scientific Violet" }")

    def test_set_theme_changes_current(self):
        m = self._make_manager()
        with patch.object(m, "_save_theme"):
            m.set_theme("dracula")
        if not (m.current_theme_name == "dracula"): raise ValueError(f"Assertion failed: { m.current_theme_name == "dracula" }")
        if not (m.current_theme.name == "Dracula"): raise ValueError(f"Assertion failed: { m.current_theme.name == "Dracula" }")
    ...
    def test_set_theme_triggers_callbacks(self):
        ...
        with patch.object(m, "_save_theme"):
            m.set_theme("dracula")
        if not (len(called_with) == 1): raise ValueError(f"Assertion failed: { len(called_with) == 1 }")
        if not (called_with[0].name == "Dracula"): raise ValueError(f"Assertion failed: { called_with[0].name == "Dracula" }")
    ...
    def test_add_callback_deduplicates(self):
        ...
        with patch.object(m, "_save_theme"):
            m.set_theme("dracula")
        if not (cb.call_count == 1): raise ValueError(f"Assertion failed: { cb.call_count == 1 }")
    ...
    def test_get_colors_returns_dict(self):
        m = self._make_manager()
        colors = m.get_colors()
        if not ("primary" in colors): raise ValueError(f"Assertion failed: { "primary" in colors }")
        if not ("secondary" in colors): raise ValueError(f"Assertion failed: { "secondary" in colors }")
        if not ("accent" in colors): raise ValueError(f"Assertion failed: { "accent" in colors }")
        if not ("background" in colors): raise ValueError(f"Assertion failed: { "background" in colors }")

    def test_get_histogram_style(self):
        m = self._make_manager()
        style = m.get_histogram_style()
        if not ("color" in style): raise ValueError(f"Assertion failed: { "color" in style }")
        if not ("alpha" in style): raise ValueError(f"Assertion failed: { "alpha" in style }")

    def test_get_scatter_style(self):
        m = self._make_manager()
        style = m.get_scatter_style()
        if not ("c" in style): raise ValueError(f"Assertion failed: { "c" in style }")
        if not ("s" in style): raise ValueError(f"Assertion failed: { "s" in style }")

    def test_get_line_style(self):
        m = self._make_manager()
        style = m.get_line_style(index=0)
        if not ("color" in style): raise ValueError(f"Assertion failed: { "color" in style }")
        if not ("linewidth" in style): raise ValueError(f"Assertion failed: { "linewidth" in style }")
    ...
    def test_get_fit_line_style(self):
        ...
        if not ("color" in style): raise ValueError(f"Assertion failed: { "color" in style }")
        if not (style["linestyle"] == "-"): raise ValueError(f"Assertion failed: { style["linestyle"] == "-" }")

    def test_get_contour_style(self):
        m = self._make_manager()
        style = m.get_contour_style()
        if not ("cmap" in style): raise ValueError(f"Assertion failed: { "cmap" in style }")
    ...
    def test_load_saved_theme_via_qsettings(self):
        ...
        if not (m is not None): raise ValueError(f"Assertion failed: { m is not None }")
...
class TestGetPlotThemeManager:
    ...
    def test_get_plot_theme_manager_singleton(self):
        ...
        if not (m1 is m2): raise ValueError(f"Assertion failed: { m1 is m2 }")
...
class TestPackageInit:
    def test_package_exports_plot_theme(self):
        ...
        if not (PlotTheme is not None): raise ValueError(f"Assertion failed: { PlotTheme is not None }")

    def test_package_exports_get_theme(self):
        from plot_theme import get_theme

        if not (callable(get_theme)): raise ValueError(f"Assertion failed: { callable(get_theme) }")

    def test_package_exports_get_plot_theme_manager(self):
        from plot_theme import get_plot_theme_manager

        if not (callable(get_plot_theme_manager)): raise ValueError(f"Assertion failed: { callable(get_plot_theme_manager) }")
...
class TestIntegration함수:
    ...
    def test_get_theme_colors_current_theme(self):
        ...
        if not ("primary" in colors): raise ValueError(f"Assertion failed: { "primary" in colors }")
        if not ("secondary" in colors): raise ValueError(f"Assertion failed: { "secondary" in colors }")
        if not ("contour_cmap" in colors): raise ValueError(f"Assertion failed: { "contour_cmap" in colors }")
    ...
    def test_get_theme_colors_named_theme(self):
        ...
        if not (colors["primary"] == "#bd93f9"): raise ValueError(f"Assertion failed: { colors["primary"] == "#bd93f9" }")
    ...
    def test_plot_theme_mixin_set_plot_theme(self):
        ...
        if not (widget._plot_theme_manager.current_theme_name == "nord"): raise ValueError(f"Assertion failed: { widget._plot_theme_manager.current_theme_name == "nord" }")
    ...
    def test_plot_theme_mixin_get_plot_colors_no_manager(self):
        ...
        widget = MyWidget()
        colors = widget.get_plot_colors()
        if not (colors == {}): raise ValueError(f"Assertion failed: { colors == {} }")
    ...
    def test_plot_theme_mixin_on_changed_callback(self):
        ...
        if not (widget._callback_theme is not None): raise ValueError(f"Assertion failed: { widget._callback_theme is not None }")
    ...
    def test_create_plot_theme_menu_no_pyqt6(self):
        ...
        if not (result is None): raise ValueError(f"Assertion failed: { result is None }")
