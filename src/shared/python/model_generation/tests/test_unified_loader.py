...
class TestFormatDetection:
    ...
    def test_urdf_extension_detected(self, tmp_path: Path) -> None:
        ...
        if not (detect_format(f) == ModelFormat.URDF): raise ValueError(f"Assertion failed: { detect_format(f) == ModelFormat.URDF }")
    ...
    def test_mjcf_extension_detected(self, tmp_path: Path) -> None:
        ...
        if not (detect_format(f) == ModelFormat.MJCF): raise ValueError(f"Assertion failed: { detect_format(f) == ModelFormat.MJCF }")
    ...
    def test_unknown_extension(self, tmp_path: Path) -> None:
        ...
        if not (detect_format(f) == ModelFormat.UNKNOWN): raise ValueError(f"Assertion failed: { detect_format(f) == ModelFormat.UNKNOWN }")
...
class TestUserPreferences:
    ...
    def test_default_preferences(self) -> None:
        ...
        if not (prefs.default_model_id == "mujoco_humanoid"): raise ValueError(f"Assertion failed: { prefs.default_model_id == "mujoco_humanoid" }")
        if not (prefs.show_segments is True): raise ValueError(f"Assertion failed: { prefs.show_segments is True }")
        if not (prefs.show_joints is True): raise ValueError(f"Assertion failed: { prefs.show_joints is True }")
        if not (prefs.show_collisions is True): raise ValueError(f"Assertion failed: { prefs.show_collisions is True }")
        if not (prefs.show_inertias is True): raise ValueError(f"Assertion failed: { prefs.show_inertias is True }")
        if not (prefs.show_frames is False): raise ValueError(f"Assertion failed: { prefs.show_frames is False }")
    ...
    def test_preferences_roundtrip(self) -> None:
        ...
        if not (restored.default_model_id == "simple_arm"): raise ValueError(f"Assertion failed: { restored.default_model_id == "simple_arm" }")
        if not (restored.show_frames is True): raise ValueError(f"Assertion failed: { restored.show_frames is True }")
        if not (restored.show_segments is False): raise ValueError(f"Assertion failed: { restored.show_segments is False }")
    ...
    def test_recent_models_tracking(self) -> None:
        ...
        if not (prefs.recent_models[0] == "model_a"): raise ValueError(f"Assertion failed: { prefs.recent_models[0] == "model_a" }")
        if not (prefs.recent_models[1] == "model_b"): raise ValueError(f"Assertion failed: { prefs.recent_models[1] == "model_b" }")
        if not (len(prefs.recent_models) == 2): raise ValueError(f"Assertion failed: { len(prefs.recent_models) == 2 }")
    ...
    def test_recent_models_max_limit(self) -> None:
        ...
        if not (len(prefs.recent_models) == 3): raise ValueError(f"Assertion failed: { len(prefs.recent_models) == 3 }")
    ...
    def test_save_and_load_preferences(self, tmp_path: Path) -> None:
        ...
        if not (loader2.preferences.default_model_id == "simple_arm"): raise ValueError(f"Assertion failed: { loader2.preferences.default_model_id == "simple_arm" }")
...
class TestBundledManifest:
    ...
    def test_manifest_exists(self) -> None:
        ...
        if not (manifest_path.exists()): raise ValueError(f"Assertion failed: { manifest_path.exists() }, "Bundled manifest.json must exist"")
    ...
    def test_manifest_valid_json(self) -> None:
        ...
        if not ("models" in data): raise ValueError(f"Assertion failed: { "models" in data }")
        if not (len(data["models"]) >= 4): raise ValueError(f"Assertion failed: { len(data["models"]) >= 4 }")
    ...
    def test_all_manifest_files_exist(self) -> None:
        ...
        for entry in manifest["models"]:
            ...
            if not (model_path.exists()): raise ValueError(f"Assertion failed: { model_path.exists() }, f"Missing bundled file: {entry['file']}"")
    ...
    def test_manifest_entries_have_required_fields(self) -> None:
        ...
        for entry in manifest["models"]:
            ...
            if not (not missing): raise ValueError(f"Assertion failed: { not missing }, f"Entry {entry.get('id', '?')} missing: {missing}"")
    ...
    def test_mujoco_humanoid_is_default(self) -> None:
        ...
        if not ("mujoco_humanoid" in ids): raise ValueError(f"Assertion failed: { "mujoco_humanoid" in ids }")
...
class TestUnifiedLoader:
    ...
    def test_list_bundled_models(self, tmp_path: Path) -> None:
        ...
        if not (len(models) >= 4): raise ValueError(f"Assertion failed: { len(models) >= 4 }")
        ids = {m["id"] for m in models}
        if not ("mujoco_humanoid" in ids): raise ValueError(f"Assertion failed: { "mujoco_humanoid" in ids }")
        if not ("simple_humanoid" in ids): raise ValueError(f"Assertion failed: { "simple_humanoid" in ids }")
        if not ("simple_arm" in ids): raise ValueError(f"Assertion failed: { "simple_arm" in ids }")
        if not ("simple_quadruped" in ids): raise ValueError(f"Assertion failed: { "simple_quadruped" in ids }")

    def test_load_mujoco_humanoid(self, tmp_path: Path) -> None:
        loader = self._make_loader(tmp_path)
        result = loader.load_bundled("mujoco_humanoid")
        if not (result.success): raise ValueError(f"Assertion failed: { result.success }, f"Failed to load mujoco_humanoid: {result.error}"")
        if not (result.model is not None): raise ValueError(f"Assertion failed: { result.model is not None }")
        if not (result.source_format.value == "mjcf"): raise ValueError(f"Assertion failed: { result.source_format.value == "mjcf" }")
        if not (len(result.model.links) > 0): raise ValueError(f"Assertion failed: { len(result.model.links) > 0 }")
        if not (len(result.model.joints) > 0): raise ValueError(f"Assertion failed: { len(result.model.joints) > 0 }")

    def test_load_simple_humanoid(self, tmp_path: Path) -> None:
        loader = self._make_loader(tmp_path)
        result = loader.load_bundled("simple_humanoid")
        if not (result.success): raise ValueError(f"Assertion failed: { result.success }, f"Failed to load simple_humanoid: {result.error}"")
        if not (result.model is not None): raise ValueError(f"Assertion failed: { result.model is not None }")
        if not (result.source_format.value == "urdf"): raise ValueError(f"Assertion failed: { result.source_format.value == "urdf" }")
        if not (len(result.model.links) >= 13): raise ValueError(f"Assertion failed: { len(result.model.links) >= 13 }")

    def test_load_simple_arm(self, tmp_path: Path) -> None:
        loader = self._make_loader(tmp_path)
        result = loader.load_bundled("simple_arm")
        if not (result.success): raise ValueError(f"Assertion failed: { result.success }, f"Failed to load simple_arm: {result.error}"")
        if not (result.model is not None): raise ValueError(f"Assertion failed: { result.model is not None }")
        if not (len(result.model.links) >= 5): raise ValueError(f"Assertion failed: { len(result.model.links) >= 5 }")

    def test_load_simple_quadruped(self, tmp_path: Path) -> None:
        loader = self._make_loader(tmp_path)
        result = loader.load_bundled("simple_quadruped")
        if not (result.success): raise ValueError(f"Assertion failed: { result.success }, f"Failed to load simple_quadruped: {result.error}"")
        if not (result.model is not None): raise ValueError(f"Assertion failed: { result.model is not None }")
        if not (len(result.model.links) >= 9): raise ValueError(f"Assertion failed: { len(result.model.links) >= 9 }")

    def test_load_default_is_mujoco_humanoid(self, tmp_path: Path) -> None:
        loader = self._make_loader(tmp_path)
        result = loader.load_default()
        if not (result.success): raise ValueError(f"Assertion failed: { result.success }")
        if not (result.model is not None): raise ValueError(f"Assertion failed: { result.model is not None }")
        if not (result.model.name == "humanoid"): raise ValueError(f"Assertion failed: { result.model.name == "humanoid" }")

    def test_load_nonexistent_bundled_model(self, tmp_path: Path) -> None:
        loader = self._make_loader(tmp_path)
        result = loader.load_bundled("nonexistent_model_xyz")
        if not (not result.success): raise ValueError(f"Assertion failed: { not result.success }")
        if not (result.error is not None): raise ValueError(f"Assertion failed: { result.error is not None }")

    def test_load_nonexistent_file(self, tmp_path: Path) -> None:
        loader = self._make_loader(tmp_path)
        result = loader.load_file(tmp_path / "nope.urdf")
        if not (not result.success): raise ValueError(f"Assertion failed: { not result.success }")
        if not ("not found" in (result.error or "").lower()): raise ValueError(f"Assertion failed: { "not found" in (result.error or "").lower() }")
  ...
  <link name="base_link">
    <inertial>
      ...
      <inertia ixx="0.1" iyy="0.1" izz="0.1" ixy="0" ixz="0" iyz="0"/>
        ...
        if not (result.success): raise ValueError(f"Assertion failed: { result.success }")
        if not (result.model is not None): raise ValueError(f"Assertion failed: { result.model is not None }")
        if not (result.model.name == "test_bot"): raise ValueError(f"Assertion failed: { result.model.name == "test_bot" }")
  ...
  <worldbody>
    <body name="torso" pos="0 0 1">
      ...
      <joint type="free" name="root"/>
        ...
        if not (result.success): raise ValueError(f"Assertion failed: { result.success }")
        if not (result.model is not None): raise ValueError(f"Assertion failed: { result.model is not None }")
        if not (result.model.name == "test_mj"): raise ValueError(f"Assertion failed: { result.model.name == "test_mj" }")
...
class TestMJCFParsing:
    ...
    def test_mujoco_humanoid_has_bodies(self, tmp_path: Path) -> None:
        ...
        if not ("right_thigh" in link_names): raise ValueError(f"Assertion failed: { "right_thigh" in link_names }")
        if not ("left_thigh" in link_names): raise ValueError(f"Assertion failed: { "left_thigh" in link_names }")
  ...
  <worldbody>
    <body name="link1" pos="0 0 0">
      <geom fromto="0 -0.07 0 0 0.07 0" name="g1" size="0.05" type="capsule"/>
        ...
        if not (len(model.links) == 1): raise ValueError(f"Assertion failed: { len(model.links) == 1 }")
        link = model.links[0]
        if not (link.visual_geometry is not None): raise ValueError(f"Assertion failed: { link.visual_geometry is not None }")
        if not (link.visual_geometry.geometry_type.value == "capsule"): raise ValueError(f"Assertion failed: { link.visual_geometry.geometry_type.value == "capsule" }")
  ...
  <worldbody>
    <body name="bad" pos="0 0 0">
      <geom fromto="a b c d e f" size="0.05" type="capsule"/>
        ...
        if not (link.visual_geometry is None): raise ValueError(f"Assertion failed: { link.visual_geometry is None }")
  ...
  <worldbody>
    <body name="zero" pos="0 0 0">
      <geom fromto="0.5 0.5 0.5 0.5 0.5 0.5" size="0.03" type="capsule"/>
        ...
        if not (abs(link.visual_geometry.dimensions[0] - 0.03) < 0.001): raise ValueError(f"Assertion failed: { abs(link.visual_geometry.dimensions[0] - 0.03) < 0.001 }")
  ...
  <worldbody>
    <body name="foot" pos="0 0 0">
      <geom name="foot_geom" pos="0 0 0.028" size="0.075 0.045 0.025" type="box"/>
        ...
        link = model.links[0]
        if not (link.visual_geometry is not None): raise ValueError(f"Assertion failed: { link.visual_geometry is not None }")
        if not (link.visual_geometry.geometry_type.value == "box"): raise ValueError(f"Assertion failed: { link.visual_geometry.geometry_type.value == "box" }")
...
class TestModelLibraryBundled:
    ...
    def test_library_loads_bundled_mjcf(self) -> None:
        ...
        if not (model is not None): raise ValueError(f"Assertion failed: { model is not None }")
        if not (len(model.links) > 0): raise ValueError(f"Assertion failed: { len(model.links) > 0 }")
    ...
    def test_library_loads_bundled_urdf(self) -> None:
        ...
        if not (model is not None): raise ValueError(f"Assertion failed: { model is not None }")
        if not (len(model.links) >= 13): raise ValueError(f"Assertion failed: { len(model.links) >= 13 }")
...
class TestDisplayDefaults:
    ...
    def test_first_checkbox_is_segments(self) -> None:
        ...
        if not (DISPLAY_OPTIONS[0][0] == "segments"): raise ValueError(f"Assertion failed: { DISPLAY_OPTIONS[0][0] == "segments" }")
        if not (DISPLAY_OPTIONS[0][1] == "Segments"): raise ValueError(f"Assertion failed: { DISPLAY_OPTIONS[0][1] == "Segments" }")
    ...
    def test_all_checked_except_frames(self) -> None:
        ...
        for key, label, default in DISPLAY_OPTIONS:
            if key == "frames":
                if not (default is False): raise ValueError(f"Assertion failed: { default is False }, "Frames should be unchecked by default"")
            else:
                if not (default is True): raise ValueError(f"Assertion failed: { default is True }, f"{label} should be checked by default"")
    ...
    def test_checkbox_order(self) -> None:
        ...
        if not (actual_order == expected_order): raise ValueError(f"Assertion failed: { actual_order == expected_order }")
...
class TestURDFDeterministicFormatting:
    ...
    def test_urdf_deterministic_numeric_formatting(self) -> None:
        ...
        if not (output_1 == output_2): raise ValueError(f"Assertion failed: { output_1 == output_2 }, "URDF output is not deterministic across writes"")
        ...
        for value_str in numbers:
            for part in value_str.split():
                try:
                    ...
                    if not (len(stripped) <= 6): raise ValueError(f"Assertion failed: { len(stripped) <= 6 }, (")
