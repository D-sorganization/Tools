...
def validate_inertia_tensor(inertia_matrix: NDArray[np.float64]) -> list[str]:
    ...
    inertia_arr = np.asarray(inertia_matrix)

    if inertia_arr.shape != (3, 3):
        errors.append(f"Inertia must be 3x3, got {inertia_arr.shape}")
        return errors

    # Check symmetry
    if not np.allclose(inertia_arr, inertia_arr.T, rtol=1e-6):
        errors.append("Inertia tensor is not symmetric")

    # Check positive diagonal
    if np.any(np.diag(inertia_arr) <= 0):
        errors.append("Diagonal elements must be positive")

    # Check positive definite
    try:
        np.linalg.cholesky(inertia_arr)
    except np.linalg.LinAlgError:
        errors.append("Inertia tensor is not positive definite")

    # Check triangle inequality
    ixx, iyy, izz = inertia_arr[0, 0], inertia_arr[1, 1], inertia_arr[2, 2]
