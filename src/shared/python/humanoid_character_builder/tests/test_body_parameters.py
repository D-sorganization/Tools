...
class TestVector3:
    ...
    def test_as_tuple(self) -> Any:
        v = Vector3(1.0, 2.0, 3.0)
        assert v.as_tuple() == (1.0, 2.0, 3.0)

    def test_uniform(self) -> Any:
        v = Vector3.uniform(0.5)
        assert v.x == 0.5
        assert v.y == 0.5
        assert v.z == 0.5

    def test_from_tuple(self) -> Any:
...
class TestBodyParameters:
    ...
    def test_gender_factor(self) -> Any:
...
class TestSegmentParameters:
    ...
    def test_has_overrides(self) -> Any:
        seg = SegmentParameters()
