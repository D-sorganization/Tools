...
@dataclass(frozen=True)
class TokenUsage:
    """Normalized token usage across all AI provider adapters.

    Provider responses use heterogeneous keys (``prompt_tokens`` vs
    ``input_tokens``) and shapes. ``TokenUsage`` is the single, strongly
    typed representation every adapter must return on
    :pyattr:`AgentResponse.usage` (issue #2763).

    Attributes:
        input_tokens: Tokens consumed by the prompt / input side.
    ...
    def to_dict(self) -> dict[str, int]:
        """Return the legacy dict shape for serialization / back-compat.
...
class AgentResponse:
    ...
    Attributes:
        ...
        usage: Token usage statistics (normalized across providers).
    ...
    def usage_dict(self) -> dict[str, int]:
        """Backward-compatible dict view of usage.

        Preserves the pre-#2763 contract for any external consumer that
        still expects ``response.usage_dict["input_tokens"]`` style access.
