...
def _handle_violation(
    ...
    Reads from ``_ContractState.level`` rather than the module-level alias
    so that runtime calls to ``set_contract_level()`` are always respected,
    even by modules that previously imported the old value (#2217).
    """
