...
class TestLoadDataframe:
    def test_load_dataframe_sets_df(self, sample_df: pd.DataFrame) -> Any:
        dp = DataProcessor()
        dp.load_dataframe(sample_df, name="test")
        assert dp.dataframe is not None
        assert len(dp.dataframe) == 100

    def test_load_dataframe_records_history(self, sample_df: pd.DataFrame) -> Any:
        dp = DataProcessor()
        dp.load_dataframe(sample_df, name="my_data")
        assert len(dp.history) == 1
        assert "my_data" in dp.history[0]

    def test_load_dataframe_invalid_type_raises(self) -> Any:
        dp = DataProcessor()
        with pytest.raises(PreconditionError):
            dp.load_dataframe("not a dataframe")  # type: ignore[arg-type]

    def test_load_dataframe_empty_name_raises(self, sample_df: pd.DataFrame) -> Any:
        dp = DataProcessor()
        with pytest.raises(PreconditionError):
...
class TestInfoProperty:
    def test_info_without_data(self) -> Any:
        dp = DataProcessor()
        info = dp.info
        assert info.num_rows == 0
        assert info.name == ""

    def test_info_with_data(self, dp: DataProcessor) -> Any:
        info = dp.info
    ...
    def test_info_untitled_when_no_source_path(self, sample_df: pd.DataFrame) -> Any:
        proc = DataProcessor()
        proc.load_dataframe(sample_df, name="x")
        # source_path is "" so name is "untitled"
        assert proc.info.name == "untitled"

    def test_dataframe_setter(self, sample_df: pd.DataFrame) -> Any:
        dp = DataProcessor()
        dp.dataframe = sample_df
        assert len(dp.dataframe) == 100

    def test_dataframe_getter_raises_when_empty(self) -> Any:
        dp = DataProcessor()
        with pytest.raises(RuntimeError, match="No data loaded"):
            _ = dp.dataframe

    def test_history_returns_copy(self, dp: DataProcessor) -> Any:
        hist = dp.history
        hist.append("extra")
...
class TestLoadFile:
    def test_load_csv(self, tmp_path: Path, sample_df: pd.DataFrame) -> Any:
    ...
    def test_load_tsv(self, tmp_path: Path, sample_df: pd.DataFrame) -> Any:
    ...
    def test_load_txt(self, tmp_path: Path, sample_df: pd.DataFrame) -> Any:
    ...
    def test_load_parquet(self, tmp_path: Path, sample_df: pd.DataFrame) -> Any:
    ...
    def test_load_xlsx(self, tmp_path: Path, sample_df: pd.DataFrame) -> Any:
    ...
    def test_load_unsupported_format_raises(self, tmp_path: Path) -> Any:
    ...
    def test_load_dat_fallback(self, tmp_path: Path) -> Any:
    ...
    def test_load_records_history(self, tmp_path: Path, sample_df: pd.DataFrame) -> Any:
        csv_file = tmp_path / "data.csv"
        sample_df.to_csv(csv_file, index=False)
        dp = DataProcessor()
        dp.load(csv_file)
    ...
    def test_load_returns_self(self, tmp_path: Path, sample_df: pd.DataFrame) -> Any:
        csv_file = tmp_path / "data.csv"
        sample_df.to_csv(csv_file, index=False)
        dp = DataProcessor()
...
class TestTrimTime:
    def test_trim_reduces_rows(self, dp: DataProcessor) -> Any:
        original = len(dp.dataframe)
        dp.trim_time(0.2, 0.8)
        assert len(dp.dataframe) < original

    def test_trim_with_explicit_column(self, dp: DataProcessor) -> Any:
        dp.trim_time(0.0, 0.5, time_column="time")
        assert dp.dataframe["time"].max() <= 0.5

    def test_trim_auto_detects_column(self, dp: DataProcessor) -> Any:
        dp.trim_time(0.0, 0.5)
        assert dp.dataframe["time"].max() <= 0.5

    def test_trim_invalid_start_raises(self, dp: DataProcessor) -> Any:
        with pytest.raises(PreconditionError):
            dp.trim_time("x", 0.5)  # type: ignore[arg-type]

    def test_trim_end_less_than_start_raises(self, dp: DataProcessor) -> Any:
        with pytest.raises(PreconditionError):
            dp.trim_time(0.8, 0.2)

    def test_trim_records_history(self, dp: DataProcessor) -> Any:
        dp.trim_time(0.2, 0.8)
        assert any("Trimmed" in h for h in dp.history)

    def test_trim_fallback_first_column_when_no_time(self) -> Any:
...
class TestResample:
    def test_resample_changes_length(self, dp: DataProcessor) -> Any:
    ...
    def test_resample_invalid_rate_raises(self, dp: DataProcessor) -> Any:
        with pytest.raises(PreconditionError):
            dp.resample(target_rate=-1.0)

    def test_resample_records_history(self, dp: DataProcessor) -> Any:
        with patch.dict("sys.modules", {"data_processor.core.signal_processing": None}):
            dp.resample(target_rate=50.0)
        assert any("Resampled" in h for h in dp.history)

    def test_resample_with_explicit_time_column(self, dp: DataProcessor) -> Any:
        with patch.dict("sys.modules", {"data_processor.core.signal_processing": None}):
...
class TestApplyFilter:
    ...
    def test_supported_filter_types_constant(self) -> Any:
...
class TestApplyFormula:
    def test_apply_formula_creates_column(self, dp: DataProcessor) -> Any:
        dp.apply_formula("speed", "x + y")
        assert "speed" in dp.dataframe.columns

    def test_apply_formula_records_history(self, dp: DataProcessor) -> Any:
        dp.apply_formula("speed", "x + y")
        assert any("speed" in h for h in dp.history)

    def test_apply_formula_invalid_column_name_raises(self, dp: DataProcessor) -> Any:
        with pytest.raises(PreconditionError):
            dp.apply_formula("", "x + y")

    def test_apply_formula_empty_expression_raises(self, dp: DataProcessor) -> Any:
        with pytest.raises(PreconditionError):
...
class TestTransformOps:
    def test_drop_columns(self, dp: DataProcessor) -> Any:
        dp.drop_columns(["x"])
        assert "x" not in dp.dataframe.columns

    def test_drop_columns_nonexistent_is_ignored(self, dp: DataProcessor) -> Any:
        dp.drop_columns(["nonexistent"])
        assert len(dp.dataframe.columns) == 3  # no change

    def test_drop_columns_invalid_raises(self, dp: DataProcessor) -> Any:
        with pytest.raises(PreconditionError):
            dp.drop_columns([])

    def test_rename_columns(self, dp: DataProcessor) -> Any:
        dp.rename_columns({"x": "x_renamed"})
        assert "x_renamed" in dp.dataframe.columns

    def test_rename_columns_empty_raises(self, dp: DataProcessor) -> Any:
        with pytest.raises(PreconditionError):
            dp.rename_columns({})

    def test_sort_ascending(self, dp: DataProcessor) -> Any:
        dp.sort("x", ascending=True)
        x_vals = dp.dataframe["x"].values
        assert all(x_vals[i] <= x_vals[i + 1] for i in range(len(x_vals) - 1))

    def test_sort_descending(self, dp: DataProcessor) -> Any:
        dp.sort("x", ascending=False)
        x_vals = dp.dataframe["x"].values
        assert all(x_vals[i] >= x_vals[i + 1] for i in range(len(x_vals) - 1))

    def test_sort_invalid_column_raises(self, dp: DataProcessor) -> Any:
        with pytest.raises(PreconditionError):
            dp.sort("")

    def test_sort_invalid_ascending_raises(self, dp: DataProcessor) -> Any:
        with pytest.raises(PreconditionError):
            dp.sort("x", ascending="yes")  # type: ignore[arg-type]

    def test_dropna_no_columns(self, sample_df: pd.DataFrame) -> Any:
    ...
    def test_dropna_with_columns(self, sample_df: pd.DataFrame) -> Any:
        sample_df.loc[0, "x"] = float("nan")
        dp = DataProcessor()
        dp.load_dataframe(sample_df, name="test")
        dp.dropna(columns=["x"])
...
class TestAnalysis:
    def test_describe_returns_stats(self, dp: DataProcessor) -> Any:
        result = dp.describe()
    ...
    def test_correlate_returns_dataframe(self, dp: DataProcessor) -> Any:
        result = dp.correlate()
        assert isinstance(result, pd.DataFrame)
        assert "x" in result.columns
        assert "y" in result.columns

    def test_correlate_invalid_method_raises(self, dp: DataProcessor) -> Any:
        with pytest.raises(PreconditionError):
            dp.correlate(method="")

    def test_detect_outliers_returns_mask(self, dp: DataProcessor) -> Any:
        mask = dp.detect_outliers()
        assert isinstance(mask, pd.DataFrame)
        assert len(mask) > 0

    def test_detect_outliers_with_explicit_columns(self, dp: DataProcessor) -> Any:
        mask = dp.detect_outliers(columns=["x"])
        assert "x" in mask.columns

    def test_detect_outliers_with_threshold(self, dp: DataProcessor) -> Any:
...
class TestExport:
    def test_export_csv(self, dp: DataProcessor, tmp_path: Path) -> Any:
    ...
    def test_export_parquet(self, dp: DataProcessor, tmp_path: Path) -> Any:
        out = tmp_path / "out.parquet"
        dp.export(out)
        assert out.exists()

    def test_export_json(self, dp: DataProcessor, tmp_path: Path) -> Any:
    ...
    def test_export_xlsx(self, dp: DataProcessor, tmp_path: Path) -> Any:
    ...
    def test_export_records_history(self, dp: DataProcessor, tmp_path: Path) -> Any:
        out = tmp_path / "out.csv"
        dp.export(out)
...
class TestMethodChaining:
    def test_full_chain(self, tmp_path: Path, sample_df: pd.DataFrame) -> Any:
        csv_file = tmp_path / "data.csv"
        sample_df.to_csv(csv_file, index=False)
