"""Strict durable camera preferences shared by 3D viewports.

Only deliberate user preferences are serialized. A moving subject target and
manual-tracking suspension are transient runtime state and must never enter
...
from .models import CameraCommandId, CameraState, FaceOnSide
...
class CameraPreferences:
    ...
    def from_document(cls, value: object) -> CameraPreferences:
        """Parse an exact current document and reject future or malformed formats."""
...
def default_camera_preferences() -> CameraPreferences:
    """Return default preferences: neutral impact and tracked 2x moving views."""
