"""UI-neutral camera commands and mathematical models for 3D viewports.

The canonical world coordinate frame is:
- ``x``: downrange / target line
- ``y``: vertical / up
- ``z``: lateral / right of target

Renderers adapt these view directions to their respective projection APIs.
"""
...
class FaceOnSide(StrEnum):
    """Physical side of the target line from which face-on camera observes."""
...
def moving_subject_camera_state() -> CameraState:
    """Return the initial state suited for moving subjects (2x zoom, tracking)."""
    return CameraState(
        zoom=2.0,
...
def camera_preset(command_id: CameraCommandId, side: FaceOnSide) -> CameraPreset:
    """Return canonical view direction and up vector for a named view command."""
...
def apply_manual_override(state: CameraState) -> CameraState:
    """Suspend an enabled tracker after an intentional manual orbit or pan."""
