"""Accessible Qt camera controls and viewport mixin for 3D canvases.

Provides DRY control widgets for snap presets, deliberate face-on side,
subject tracking, auto-fit clearance, and recenter recovery.
"""
...
from .preferences import CameraPreference, apply_camera_preference
...
# PyQt6-dependent imports - guarded for headless environments
try:
...
if _PYQT6_AVAILABLE:
    ...
    class CameraControls(QWidget):
        """Standard camera control bar with view presets, side choice, and tracking."""
        ...
        def __init__(
            ...
            for command_id, label, tooltip in _BUTTON_SPECS:
                ...
                button.clicked.connect(
                    lambda _checked, value=command_id: command(value)
                )
            ...
            self.face_side.setToolTip(
                "Choose the physical side of the target line; "
                "handedness is never inferred."
            )
    ...
    CameraControls = None  # type: ignore[misc,assignment]
...
class CameraViewportMixin:
    ...
    def _initialize_camera(
        ...
        if _PYQT6_AVAILABLE and CameraControls is not None:
            ...
            self._camera_controls_widget = None
        return self._camera_controls_widget

    def camera_controls(self) -> Any:
    ...
    def restore_camera_preference(self, preference: CameraPreference) -> None:
        ...
        if self._camera_controls_widget is not None:
            self._camera_controls_widget.sync(self._camera_state)
    ...
    def _apply_camera_axis_visibility(self, axes: Any) -> None:
...
__all__ = ["CameraControls", "CameraViewport", "CameraViewportMixin"]
