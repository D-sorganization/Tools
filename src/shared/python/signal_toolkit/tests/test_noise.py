...
class TestConstants:
    def test_default_line_frequency(self) -> Any:
        assert DEFAULT_LINE_FREQUENCY_HZ == 60.0

    def test_harmonic_constants(self) -> Any:
...
class TestNoiseGenerator:
    def test_seeded_reproducibility(self, t: np.ndarray) -> Any:
    ...
    def test_white_noise_length(self, t: np.ndarray) -> Any:
...
class TestAddNoiseToSignal:
    def test_with_explicit_amplitude(self, sine_signal: Signal) -> Any:
    ...
    def test_with_snr_db(self, sine_signal: Signal) -> Any:
        noisy = add_noise_to_signal(sine_signal, snr_db=20.0, seed=42)
        assert noisy.metadata["snr_db"] == 20.0

    def test_default_amplitude(self, sine_signal: Signal) -> Any:
    ...
    def test_noise_is_added(self, sine_signal: Signal) -> Any:
...
class TestGenerateDisturbanceProfile:
    def test_step_disturbance(self, t: np.ndarray) -> Any:
    ...
    def test_step_default_params(self, t: np.ndarray) -> Any:
    ...
    def test_pulse_default_params(self, t: np.ndarray) -> Any:
        sig = generate_disturbance_profile(t, "pulse")
        assert len(sig.values) == len(t)

    def test_ramp_disturbance(self, t: np.ndarray) -> Any:
        sig = generate_disturbance_profile(
    ...
    def test_ramp_default_params(self, t: np.ndarray) -> Any:
...
class TestDisturbanceSimulator:
    def test_empty_simulator_generates_zeros(self, t: np.ndarray) -> Any:
        sim = DisturbanceSimulator(seed=0)
        sig = sim.generate(t)
        assert np.all(sig.values == 0)
        assert sig.name == "combined_disturbance"

    def test_add_noise_chaining(self, t: np.ndarray) -> Any:
        sim = DisturbanceSimulator(seed=0)
        result = sim.add_noise(NoiseType.WHITE, amplitude=0.1)
        assert result is sim  # Returns self

    def test_add_step_chaining(self, t: np.ndarray) -> Any:
        sim = DisturbanceSimulator(seed=0)
        result = sim.add_step(0.5, magnitude=1.0)
        assert result is sim

    def test_add_pulse_chaining(self, t: np.ndarray) -> Any:
        sim = DisturbanceSimulator(seed=0)
        result = sim.add_pulse(0.3, 0.1)
        assert result is sim

    def test_add_periodic_chaining(self, t: np.ndarray) -> Any:
        sim = DisturbanceSimulator(seed=0)
        result = sim.add_periodic(60.0, 0.1)
        assert result is sim

    def test_combined_disturbance_generation(self, t: np.ndarray) -> Any:
    ...
    def test_apply_to_signal(self, sine_signal: Signal) -> Any:
        sim = DisturbanceSimulator(seed=0)
        sim.add_noise(NoiseType.WHITE, amplitude=0.1)
    ...
    def test_apply_to_signal_preserves_time(self, sine_signal: Signal) -> Any:
        sim = DisturbanceSimulator(seed=0)
        sim.add_step(0.5)
        disturbed = sim.apply_to_signal(sine_signal)
        np.testing.assert_array_equal(disturbed.time, sine_signal.time)

    def test_metadata_components_count(self, t: np.ndarray) -> Any:
        sim = DisturbanceSimulator(seed=0)
        sim.add_noise()
        sim.add_step(0.3)
        sig = sim.generate(t)
    ...
    def test_add_noise_default_type(self, t: np.ndarray) -> Any:
        sim = DisturbanceSimulator(seed=0)
...
class TestNoiseGeneratorElseFallback:
    def test_generate_with_unknown_noise_type_uses_white(self, t: np.ndarray) -> Any:
