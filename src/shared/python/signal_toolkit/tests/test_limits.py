...
class TestSaturationMode:
    def test_all_modes_exist(self) -> Any:
...
class TestApplySaturationValues:
    ...
    def test_zero_range_returns_center(self) -> Any:
...
class TestApplySaturation:
    def test_hard_saturation(self, ramp_signal: Signal) -> Any:
...
class TestPrivateClipHelpers:
    def test_soft_clip_linear_region(self) -> Any:
    ...
    def test_soft_clip_saturated_region(self) -> Any:
        x = np.array([-5.0, 5.0])
        result = _soft_clip(x, k=1.0)
        assert np.all(np.abs(result) <= 1.0)

    def test_cubic_clip_within_unit(self) -> Any:
        x = np.linspace(-0.9, 0.9, 20)
        result = _cubic_clip(x, k=1.0)
        assert np.all(np.abs(result) <= 1.0)

    def test_cubic_clip_outside_unit(self) -> Any:
        x = np.array([-5.0, 5.0])
        result = _cubic_clip(x, k=1.0)
        assert np.all(np.abs(result) <= 1.0)

    def test_exponential_clip_normalizer_near_zero(self) -> Any:
...
class TestApplyRateLimiter:
    def test_rate_limiter_limits_changes(self, ramp_signal: Signal) -> Any:
    ...
    def test_rate_limiter_smooth(self, ramp_signal: Signal) -> Any:
        result = apply_rate_limiter(ramp_signal, max_rate=0.5, smooth_transition=True)
        assert len(result.values) == len(ramp_signal.values)

    def test_rate_limiter_preserves_slow_signals(self, t: np.ndarray) -> Any:
    ...
    def test_rate_limiter_negative_rate_limiting(self, t: np.ndarray) -> Any:
    ...
    def test_rate_limiter_units_preserved(self, ramp_signal: Signal) -> Any:
...
class TestApplyDeadband:
    def test_deadband_hard_zero_zone(self, t: np.ndarray) -> Any:
    ...
    def test_deadband_smooth_mode(self, t: np.ndarray) -> Any:
    ...
    def test_deadband_preserves_time(self, sine_signal: Signal) -> Any:
...
class TestApplyHysteresis:
    def test_hysteresis_transitions(self, t: np.ndarray) -> Any:
    ...
    def test_hysteresis_initial_state_high(self, t: np.ndarray) -> Any:
    ...
    def test_hysteresis_smooth(self, t: np.ndarray) -> Any:
    ...
    def test_hysteresis_units_preserved(self, sine_signal: Signal) -> Any:
...
class TestApplyBacklash:
    def test_backlash_no_backlash(self, ramp_signal: Signal) -> Any:
    ...
    def test_backlash_with_width(self, ramp_signal: Signal) -> Any:
        result = apply_backlash(ramp_signal, backlash_width=0.5, smooth=False)
        assert len(result.values) == len(ramp_signal.values)

    def test_backlash_smooth_mode(self, ramp_signal: Signal) -> Any:
        result = apply_backlash(ramp_signal, backlash_width=0.2, smooth=True, smoothness=5.0)
        assert len(result.values) == len(ramp_signal.values)

    def test_backlash_negative_width_raises(self, sine_signal: Signal) -> Any:
        with pytest.raises(ValueError, match="non-negative"):
            apply_backlash(sine_signal, backlash_width=-0.1)

    def test_backlash_non_positive_smoothness_raises(self, sine_signal: Signal) -> Any:
        with pytest.raises(ValueError, match="positive"):
            apply_backlash(sine_signal, backlash_width=0.1, smoothness=0.0)

    def test_backlash_output_decreasing_ramp(self, t: np.ndarray) -> Any:
...
class TestCreateSaturationFunction:
    def test_returns_callable(self) -> Any:
        fn = create_saturation_function(-1.0, 1.0, SaturationMode.TANH)
        assert callable(fn)

    def test_function_applies_saturation(self) -> Any:
    ...
    def test_default_mode_is_tanh(self) -> Any:
