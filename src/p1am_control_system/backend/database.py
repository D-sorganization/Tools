...
def init_db() -> None:
    ...
    try:
        ...
        # Order matters: the #4004 migration runs first so a legacy row keeps
        # main's documented backfill (``live`` — what the old code recorded),
        # then the provenance migration adds the remaining SignalFrame columns
        # and leaves the existing ``quality`` column alone.
