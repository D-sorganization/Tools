...
It also owns the three seams that keep a scan honest and cheap:
...
* ``SignalFrameFactory`` — describes the scan *per sample* (quality, diagnostic
  reason, sequence, source and source time) so the HMI can say why a trace is
  held rather than just that it is. The frame and the ``DataSource`` are
  complementary: the source classifies the scan and decides what the control
  path may touch, the frame carries the detail the operator sees.
"""
...
def _health_payload(frame: SignalFrame | None) -> dict[str, object]:
    """Summarise the frame for the HMI's comms-health badge."""
    if frame is None:
        return {
...
def _build_scan_frame(
    factory: SignalFrameFactory,
    *,
    source: DataSource,
    fresh_tags: dict[str, float] | None,
    display_tags: dict[str, float],
    plc_connected: bool,
) -> SignalFrame | None:
    """Describe this scan per sample, or None when there is nothing to describe.

    Complements :func:`_resolve_source` rather than duplicating it: the
    ``DataSource`` classifies the scan and decides what the control path may
    touch, while the frame carries the per-sample quality, diagnostic reason,
    sequence and source time the HMI shows beside a held or simulated trace.

    Returns None on a fault scan (no data at all) — ``_health_payload`` renders
    that as ``bad``/``no_data`` — and on an empty tag mapping, which the frame
    factory rejects outright.
    """
    if fresh_tags:
        if source is DataSource.SIMULATED:
            return factory.simulated(
                fresh_tags,
                source="plc.driver" if plc_connected else "synthetic.simulator",
            )
        return factory.good(fresh_tags, source="plc.driver")
    if not display_tags:
        return None
    if source is DataSource.HELD:
        # Connected but this read hiccuped (common when the Pi is under load and
        # the Modbus read trips its timeout). The last good values are shown so
        # the trace does not flicker to ~0, marked stale so nobody mistakes them
        # for a fresh measurement.
        return factory.stale(display_tags, source="plc.driver", reason="read_timeout")
    if source is DataSource.SIMULATED:
        # Simulator driver whose read failed: still bench data, but stale.
        return factory.stale(
            display_tags,
...
async def _poll_once(
    ...
    * ``fault`` — no data at all. Same suppression, plus the write-seam
    ...
    Args:
        ...
        signal_frames: Optional ``SignalFrameFactory`` owning the scan sequence
            and source times. Its frame populates ``tag_samples`` /
            ``comms_health`` on the payload; it never decides what the control
            path may touch — ``data_source`` does.
    ...
    # Per-sample description of the same scan. It never overrides ``source`` —
    # what the control path is allowed to touch is decided above and only there.
    frame = _build_scan_frame(
        frame_factory,
        source=source,
