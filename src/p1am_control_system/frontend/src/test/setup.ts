...
// function". Install a conforming Storage so every suite gets real persistence.
// This file only ever runs under the test runner, so it installs
// unconditionally rather than sniffing the existing value: a fresh in-memory
// Storage per test file is the isolation we want anyway, and merely reading
// Node's lazy `localStorage` to probe it would fire that warning in every
// worker.
