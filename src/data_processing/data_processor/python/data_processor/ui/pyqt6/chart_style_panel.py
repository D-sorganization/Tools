...
if PYQT6_AVAILABLE:
    ...
    class ChartStylePanel(QWidget):
        ...
        def _setup_ui(self) -> None:
            ...
            layout.addWidget(self._create_display_mode_group())
            layout.addWidget(self._create_trendline_group())
            layout.addWidget(self._create_axis_group())
            layout.addWidget(self._create_legend_group())
            layout.addStretch()

        def _create_display_mode_group(self) -> QGroupBox:
            """Create display mode controls (line style, markers, color)."""
            ...
            self._line_style_combo.addItems(["solid", "dashed", "dotted", "dashdot"])
            ...
            form.addRow("Marker Size:", self._marker_size_spin)
        ...
        def _create_trendline_group(self) -> QGroupBox:
            """Create trendline controls (type, degree, equation display)."""
            group = QGroupBox("Trendline")
            form = QFormLayout(group)
        ...
        def _create_axis_group(self) -> QGroupBox:
            ...
            form.addRow("", self._x_log_check)

            self._y_log_check = QCheckBox("Log Scale Y")
            form.addRow("", self._y_log_check)
        ...
        def _create_legend_group(self) -> QGroupBox:
            """Create legend controls (visibility, position)."""
            group = QGroupBox("Legend")
            form = QFormLayout(group)

            self._legend_visible_check = QCheckBox("Show Legend")
            self._legend_visible_check.setChecked(True)
            form.addRow("", self._legend_visible_check)

            self._legend_pos_combo = QComboBox()
            self._legend_pos_combo.addItems(["right", "left", "top", "bottom", "none"])
            form.addRow("Position:", self._legend_pos_combo)
