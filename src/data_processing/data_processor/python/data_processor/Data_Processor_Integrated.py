...
class IntegratedCSVProcessorApp(OriginalCSVProcessorApp):
    ...
    def create_help_tab(self, tab):
        ...
        # Comprehensive help content loaded from external file
        try:
            # Try to locate the help file relative to this script or repo root
            current_dir = Path(__file__).resolve().parent
            
            # Candidates for help file location
            candidates = [
                # Repo root resources (if running from source)
                current_dir.parents[4] / "resources" / "integrated_app_help.md",
                # Local resources (if packaged)
                current_dir / "resources" / "integrated_app_help.md",
                # Relative to CWD
                Path("resources/integrated_app_help.md"),
            ]
            
            help_content = "Help documentation could not be loaded."
            
            for path in candidates:
                if path.exists():
                    try:
                        help_content = path.read_text(encoding="utf-8")
                        break
                    except Exception:
                        continue
            else:
                 # Fallback if file not found
                 help_content = """# Integrated Data Processor
                 
Documentation file 'resources/integrated_app_help.md' not found.
Please ensure the resources directory is present.
"""
        except Exception as e:
            help_content = f"Error loading help documentation: {e}"
