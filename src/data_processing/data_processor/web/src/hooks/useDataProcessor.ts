...
function medianFilter(values: number[], kernelSize: number): number[] {
  // Bolt: Optimize median filter to avoid O(N*K log K) garbage collection by reusing a single typed array buffer.
  // Using Float64Array.subarray().sort() provides a ~5x performance speedup.
  const len = values.length;
  const result = new Array<number>(len);
  const halfKernel = Math.floor(kernelSize / 2);
  const windowArray = new Float64Array(halfKernel * 2 + 1);
  ...
  for (let i = 0; i < len; i++) {
    ...
    for (let j = 0; j < windowLen; j++) {
      windowArray[j] = values[start + j];
    }

    const activeWindow = windowArray.subarray(0, windowLen);
    activeWindow.sort();

    result[i] = activeWindow[Math.floor(windowLen / 2)];
  }
