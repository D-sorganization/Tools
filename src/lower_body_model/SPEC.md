...
- **Typed state snapshots**: `LowerBodySimulator.current_qpos` and `set_target_from_current()` return copied NumPy arrays from MuJoCo's live `qpos` buffer so callers receive stable, typed pose snapshots while the simulator keeps ownership of mutable engine state.
