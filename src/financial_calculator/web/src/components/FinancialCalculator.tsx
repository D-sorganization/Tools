...
function FinancialCalculator() {
  ...
  const calculate = () => {
    ...
    const variableCosts = annualFeedstock * (feedstockCost + laborCost + utilitiesCost + maintenanceCost + 10)
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-lg p-4" style={{ backgroundColor: colors.mantle, border: `1px solid ${colors.surface1}` }}>
            ...
            <div className="space-y-3">
              <InputField label="Feedstock" value={feedstockCost} onChange={setFeedstockCost} min={0} max={5000} />
              <InputField label="Labor" value={laborCost} onChange={setLaborCost} min={0} max={1000} />
              <InputField label="Utilities" value={utilitiesCost} onChange={setUtilitiesCost} min={0} max={1000} />
              <InputField label="Maintenance" value={maintenanceCost} onChange={setMaintenanceCost} min={0} max={500} />
            </div>
          </div>
          ...
          <div className="rounded-lg p-4" style={{ backgroundColor: colors.mantle, border: `1px solid ${colors.surface1}` }}>
            ...
            <div className="space-y-3">
              <InputField label="Fixed Labor" value={fixedLabor} onChange={setFixedLabor} min={0} max={10000000} step={10000} />
              <InputField label="Insurance" value={insurance} onChange={setInsurance} min={0} max={1000000} step={10000} />
            </div>
          </div>
          ...
          <div className="rounded-lg p-4" style={{ backgroundColor: colors.mantle, border: `1px solid ${colors.surface1}` }}>
            ...
            <div className="space-y-3">
              <InputField label="Total Capital ($)" value={capital} onChange={setCapital} min={0} max={1000000000} step={100000} />
              <InputField label="Debt Ratio (%)" value={debtRatio} onChange={setDebtRatio} min={0} max={100} />
              <InputField label="Interest Rate (%)" value={interestRate} onChange={setInterestRate} min={0} max={30} step={0.5} />
              <InputField label="Depreciation (years)" value={depreciationYears} onChange={setDepreciationYears} min={1} max={40} />
              <InputField label="Tax Rate (%)" value={taxRate} onChange={setTaxRate} min={0} max={50} />
            </div>
          </div>
        ...
        <div className="space-y-6">
          ...
          {results && (
            <div className="grid grid-cols-2 gap-4">
              <MetricCard label="Annual Feedstock" value={`${formatNumber(results.annualFeedstockTons)} tons`} color={colors.blue} />
              <MetricCard label="Total Revenue" value={formatCurrency(results.totalRevenue)} color={colors.green} />
              <MetricCard label="Total Costs" value={formatCurrency(results.totalCosts)} color={colors.red} />
              <MetricCard label="Net Income" value={formatCurrency(results.netIncome)} color={colors.yellow} />
              <MetricCard label="EBITDA" value={formatCurrency(results.ebitda)} color={colors.peach} />
              <MetricCard label="Return on Equity" value={`${results.roe.toFixed(1)}%`} color={colors.mauve} />
              <MetricCard label="Payback Period" value={`${results.paybackYears.toFixed(1)} years`} color={colors.teal} />
            </div>
          )}
