...
def test_export_vessel_step_creates_file_and_manifest(tmp_path) -> None:
...
def test_export_vessel_step_convenience(tmp_path) -> None:
    path = export_vessel_step(output_dir=tmp_path, stem="vessel_step")
    assert path.exists()
    assert path.suffix == ".step"


def test_export_vessel_stl_creates_file(tmp_path) -> None:
    path = export_vessel_stl(output_dir=tmp_path, stem="vessel_stl")
    assert path.exists()
    assert path.stat().st_size > 0


def test_export_vessel_brep_creates_file(tmp_path) -> None:
    path = export_vessel_brep(output_dir=tmp_path, stem="vessel_brep")
    assert path.exists()
    assert path.stat().st_size > 0


def test_export_vessel_gltf_creates_file(tmp_path) -> None:
    path = export_vessel_gltf(output_dir=tmp_path, stem="vessel_gltf")
    assert path.exists()
    assert path.stat().st_size > 0


def test_export_multiple_formats_at_once(tmp_path) -> None:
...
def test_export_unsupported_format_raises(tmp_path) -> None:
