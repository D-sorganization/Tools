...
export function RotationConverter() {
  ...
  const handleCalculate = useCallback(async () => {
    ...
    try {
      ...
      const message =
        err instanceof Error ? err.message : "Failed to convert rotation";
      setError(message);
    }
      ...
      <div className="bg-slate-800 rounded-lg p-6 space-y-4 text-white">
        ...
        {!results ? (
          <div className="text-slate-400 text-sm italic">
            Provide an initial rotation and calculate to see all output formats
            here.
          </div>
        ) : (
          <div className="space-y-4">
            ...
            <div className="bg-slate-700/50 p-3 rounded">
              <p className="text-xs text-slate-400 mb-1">
                Euler Angles ({results.euler_convention})
              </p>
              <p className="font-mono text-sm">
            ...
            <div className="bg-slate-700/50 p-3 rounded">
              ...
              <p className="font-mono text-sm">
                Axis: [
                {results.axis_angle.axis.map((n) => n.toFixed(4)).join(", ")}]
              </p>
              <p className="font-mono text-sm">
