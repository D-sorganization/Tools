...
- T3 #5072: frozen spectra published at `cbc43b659`; private rigid nodal attachment reuses full body inertia and existing quadrature. Twelve final Windows tests pass, including principal-axis rod/tip-mass convergence; `LOADED_BODY_ATTACHMENT.md` records the corrected reference assumption. Final Linux golf/API run: 601 passes, two optional CAD skips and three optional-plugin warnings. Stability, moving work, loaded grip and full FRF remain open; PR not created.
...
- T2 CI: prior UD consumer passes; Gasification checkout still fails on renderer `b8c6e6013`. #5090 has repeatable reviewed references and a passing production workflow. Rust #5097 merged at `d9dec3602`; standard CI attempt 2 passes, while separate optional/private checks are not certified. UpstreamDrift #9784 merged; authority #9787 / PR #9804 publishes `d1563dffa` with actual native evidence and every normal hook passing. Secret metadata 403 leaves credential state unknown; configuration requested without secret values.
...
- T5 ingestion PR #5084 merged at `86a725c6c`; reuse `swing_sim.vibroacoustics`. Exact-commit boundary probes and limits are in `VIBROACOUSTICS_INGESTION_REVIEW.md`. T6 #5083 merged at `cfca06449`; #5075 records confirmed strict-wire/availability/evidence gaps. T3-T6 qualification, exact-pin studies, physical/blinded gates and final synthesis remain required.
