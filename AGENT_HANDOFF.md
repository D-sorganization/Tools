...
| #4775 | Actuation & swing realism (pendulum_simulator). Open. Established that the two-link fixed-hub model cannot reach measured golfer hand speed at all: releasing the club requires reversing hub torque. Distributed club inertia ruled out analytically. Next step is a moving hub via `physics_triple.py`, not more two-link tuning. |
| #4766 | Swing Objective Comparison (pendulum_simulator). Open. S1--S6 landed; UpstreamDrift tile on `feat/swing-objective-lab-tile`. New objective layer only, no physics re-derived. Check `is_degenerate` before quoting objective agreement.                                                                                             |
