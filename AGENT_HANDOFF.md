...
- T3 #5072 publishes tip force/torque response at `f37e37bd2`. New finite-pose moving-anchor kinematics reuse existing rotation Jacobians and retain dual work ports; 628 Linux golf/API tests pass with one BLAS thread and the unchanged 60-second limit (two optional CAD skips). `MOVING_GRIP_KINEMATICS.md` records the earlier timeout and derivation. Nonlinear grip, loaded balance/tangent, time evolution and stability remain open; PR not created.
