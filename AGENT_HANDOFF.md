...
| #4800 | Putting simulator parity. Open. P1 (#4808) delivered the full 2-D stroke/impact solve with the explicit head-MOI hook; P2 (#4809) delivered green-surface heightfields, 2-D surface roll, Holmes/Penner capture, and the `swing_sim.green_surface/1` wire (planar limit bit-identical, TS twins); P3 (#4810) delivered the `golf_club.putter_head/1` wire, mesh-MOI PutterSpec v2 with the club-library fallback, and quasi-static twist response (TS twins). P4-P9 remain; P9 adapts the P2 wire to UpstreamDrift's `putting_green`.                              |
