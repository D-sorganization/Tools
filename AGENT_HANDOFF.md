...
- T3 #5072: `feat/5072-prestressed-shaft`, based on T2 `2f975d06e`; kinetic checkpoint `22cfc8df9` published with all hooks. Deformed-frame checkpoint `0f3e591db` is published. Current `SELF` composes rotating loaded-chain roots with the existing solver; 560 golf/API tests pass (two optional CAD skips), including continuum extension/support and complete tangent checks. See `ROTATING_LOADED_ROOTS.md`. Stability, moving work, passive grip and FRF qualification remain open. PR not created.
