...
- T3 #5072: Lie RK4 checkpoint b5e0ec32a53cb49e280e5e934dff0713daad4a1a is published through all normal hooks, remote verified. Issue comment 5607144248 records delivery. Evidence: 77 Windows and 1,157 Linux passes; two optional CAD skips and three plugin warnings. All nine governance gates pass. T3 remains open.
...
- T3 lease: codex/session impact-acoustics-01a07d8a-shaft5072 through 2026-09-09T19:30:11Z. Versioned distributed coefficient input now reuses existing section/inertia kernels, strict duplicate-key and mass parsing, and exact artifact-byte checks. Missing-module and scalar-coercion REDs are repaired; 39 new Windows tests pass in 7.16 s. Existing API records are unchanged; shaft_model_data adds a new public wire surface. Actual three-source mypy passes. Final regression passes all 73 tests on Windows (14.59 s console) and Linux (8.50 s), no skips; Linux reports three unavailable-plugin configuration warnings. Actual three-source/five-file mypy, root Ruff (3,851 files), unchanged existing API records and all nine governance gates pass. Exact archive/JUnit evidence is in DISTRIBUTED_MODEL_INPUT_RESULTS.json. Normal publication remains. See DISTRIBUTED_MODEL_INPUTS.md and PROGRESS.md.
