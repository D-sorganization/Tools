...
- T3 #5072 publishes finite-grip response at `28c45eb15`. Stationary arbitrary-node grips now couple through consistent preload tangents, an all-node solve and full M/G/C/K operators; 646 Linux golf/API tests pass (two optional CAD skips), plus 38 focused controls. `GRIPPED_CHAIN.md` records the derivation and limits. All nine final manual gates pass. Published at eeea63b47 with all normal hooks passing. Stability, full grip-supported FRF and time evolution remain; PR not created. KIT measurement metadata is not golf validation.
...
- T2 prior UD consumer passes; private Gasification lookup still fails before tests. Renderer #5090, Rust #5097 and impact completion #5088 merged. Inventory PR #5103 publishes `ace9a007b`; 410 reviewed candidates retain publication blockers. UD #9787 / PR #9804 remains held: remote `b8da0c024` restores compatible pins but bypasses claim-preservation at the update call site. Tested local `6235789dc` is preserved; no shared-branch overwrite or remote qualification.
...
- T5 reuses merged ingestion #5084. Numerical boundary repair PR #5106 publishes `3a9362530`: 41 focused tests, 69 Linux and post-main Windows report/API tests, all nine manual gates and normal hooks pass. CI is running; calibration identity, complex FRF, radiation and physical/blinded gates remain open. T6 #5083 merged; #5075 retains strict-wire/evidence gaps. Full synthesis and exact-pin studies remain required.
