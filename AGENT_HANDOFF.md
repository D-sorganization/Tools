...
- T3 #5072: loaded roots published at `f47f64acf`; current private loaded M/G/K operators pass seven new Windows tests and 567 Linux golf/API tests (two optional CAD skips). `LOADED_DYNAMIC_OPERATORS.md` records independent energy/Coriolis controls, guided axial frequency convergence and the unresolved Windows report-sweep timeout. Stability, moving work, loaded grip/head and full FRF remain open; PR not created.
