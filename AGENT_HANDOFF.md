...
## 2026-08-11 Final local #4298 camera/ground composite

- The final local reconstruction keeps PR `#4298` on branch
  `feat/4284-camera-snap-tracking` with declared base
  `feat/4199-wind-workflow` at exact published commit
  `51033696634c624f7c92c3822d98a66875ff90a0`.
- The ancestry is intentionally ordered: published camera child
  `35d04ee4bb4d74045594d3ef0b274e2b09cc5a3b`, then unpublished native
  orthographic-axis polish `37fa7f86dcdd7bf4225da34286e39f16837caba6`,
  combined by intermediate merge `da9ffd820f1596c398d137c84a29bd77e96e5ec5`;
  refreshed flight-ground composite
  `8137621d7c77d9fa6941f5cd51d3fd161ebf4300` is then merged second.
- Stale held candidate `0eb576315498d4576b553e1ccc631345d9f6b23e` is
  not a parent. The only production conflict uses the validated split kinetics
  façade from the refreshed flight-ground parent. Camera state, snap/tracking,
  zoom-safe auto-fit, native preset-axis polish, flight-ground transfer, the
  explicit `CameraCommandId | None` narrowing, and capability-serializer
  string narrowing remain required in the composite.
- SPEC `1.16.26` records this local reconstruction. No remote branch, PR,
  release manifest state, or protected release claim is changed here. Final
  qualification evidence is recorded below before handoff.
- Final local qualification passed 1,109 Python/PyQt/flight/ground tests;
  TypeScript, zero-warning ESLint, 111 React test files with 675 tests, four
  Playwright camera tests across desktop and 520 x 900 at 2x DPR, and the
  194-module Vite production build; Rust formatting, 137 `tools-core` tests,
  12 `swing-core` tests, and warning-denied Clippy; pinned Ruff 0.14.10 on 51
  changed Python paths, pinned MyPy 1.13 on 37 production paths, Bandit on 37
  production paths with no medium/high findings, changed-file 500-line and
  repository 1,200-line budgets, changed-Python/test contracts, manifest/schema,
  docs governance, and whitespace checks. The native GUI tests used Qt's
  offscreen platform; prior native rendered review remains the visual evidence.
...
## 2026-08-11 #4288 receives refreshed exact #4285 parent

- Open PR `#4288` retains its actual branch
  `feat/4269-flight-ground-transfer` and base `feat/4268-ground-contract`.
  Exact published transfer child
  `3e6e418f98856162420dcb3f5f13dedd48d40439` is first and exact refreshed
  ground-contract parent `7a6f31b6cb36e05f903c591e207480d36f491c4d`
  is merged second through a normal no-ff merge.
- The child-owned Python/TypeScript/Rust/PyO3/WASM transfer, signed terminal
  angular state, physical contact evidence, fixtures, and cross-runtime
  contracts remain authoritative. Parent implementation merges without
  conflict; only SPEC, campaign-handoff, and manifest text require additive
  reconciliation.
- Historical PR `#4283` remains merged at reviewed head
  `9bbb98e16e435a0d4c74153b909f2ebfefbbce7a` and merge SHA
  `c1827bbdc50a6e11cc475db2636b4e47a4c15416`; current live carriers remain
  separate ancestry facts. SPEC advances uniquely to `1.16.25`, and the strict
  manifest remains unreleased.
- Local qualification passes 302 focused Python/PyQt tests with six explicit
  optional native-wheel parity skips, covered independently by 137 passing
  `tools-core` Rust library/transfer/wire tests. Rust workspace formatting and
  warning-denying Clippy pass. All 100 focused React tests across 12 files,
  TypeScript, zero-warning ESLint, dependency audit, and the 189-module build
  pass. Pinned Ruff covers all 13 exact-parent Python paths, MyPy covers all
  nine production paths, and Bandit, manifest/schema, docs governance,
  400-line size, changed-policy, minimum-test, exact-base assertion, and diff
  gates pass.
- This candidate is local only. Publication, protected exact-head CI,
  independent review, approval, and release remain open.

## 2026-08-11 #4288 receives exact reconciled #4285 parent

- Live PR evidence confirms #4288 uses branch
  `feat/4269-flight-ground-transfer` over base `feat/4268-ground-contract`; no
  `feat/4270-flight-ground-transfer` ref exists. A normal merge combines exact
  published child `c39297d37a17d5d8d3520ed62c5563cdcc609cab` with exact published
  parent `d36551cead67e2544cf185584bff4d070ee663b3` in child-first order without
  branch rename, rebase, retarget, force-push, or history rewrite.
- All implementation, test, and handoff paths merge automatically. Conflicts
  are limited to `SPEC.md` and manifest carrier-head text, with every history
  retained. Qualified flight-to-ground transfer across Python, TypeScript,
  Rust, PyO3, and WASM remains unchanged while inheriting the current strict
  ground, capability, wind, GUI, solver, and release-authority ancestry.
- `SPEC.md` advances monotonically and uniquely to `1.14.26`. The manifest
  records exact published open heads for #4285 and #4288 while preserving
  historical merged #4283 identity, commit-bound evidence, explicit incomplete
  physics/UI limitations, and unreleased claims.
- Focused local verification is green: pinned Ruff 0.14.10 check/format on all
  18 child and inherited Python paths plus diff hygiene; 138 flight/ground
  contract, parity, GUI, layout, and solver tests; strict manifest validation
  plus eight contract tests; Rust format plus 137 `tools-core` library and
  transfer/wire tests; 25 React tests across three files; and docs governance.
- This merge is local only. Publication, protected exact-head CI, independent
  review, approval, and release remain open.
...
## 2026-08-11 #4285 receives refreshed live capability carrier

- Open PR `#4285` retains branch `feat/4268-ground-contract` and base
  `feat/4197-capability-observer`. Exact published ground-contract child
  `d36551cead67e2544cf185584bff4d070ee663b3` is first and exact refreshed
  live capability carrier `0179abbdd8c0166f125d87e4d6ab47aeca15df40`
  is merged second by a normal no-ff merge.
- The strict UI-neutral `flight-to-ground/v1` schemas, canonical fixture,
  migrations, adapters, explicit dependency, and fail-closed unimplemented-
  physics boundary remain child-owned. Parent implementation merges without
  conflict; only append-only handoff/SPEC histories conflict textually.
- Historical merged PR `#4283` remains recorded at head
  `9bbb98e16e435a0d4c74153b909f2ebfefbbce7a` and merge SHA
  `c1827bbdc50a6e11cc475db2636b4e47a4c15416`. Its live branch ancestry is
  tracked separately and does not reopen or rewrite the PR.
- SPEC advances monotonically from parent `1.16.23` to unique `1.16.24`.
  The strict manifest records verified current published carrier heads while
  retaining its unreleased, unprotected status.
- Local qualification passes 256 focused Python/PyQt tests, 75 focused React
  tests across nine files, React type-check, zero-warning lint, dependency
  audit, and the 188-module production build. Twelve `swing-core` Rust tests,
  workspace formatting, and warning-denying Clippy pass. Pinned Ruff checks and
  formats all 27 exact-parent Python paths; MyPy accepts all 20 production
  paths; Bandit, strict manifest/schema, docs governance, 400-line module
  budget, changed-Python policy, minimum-test, exact-base assertion, and diff
  gates pass.
- This candidate is local only. Publication, protected exact-head CI,
  independent review, approval, normal #4288 propagation, and release remain
  open.

## 2026-08-11 #4285 receives exact live #4283 carrier

- Open PR `#4285` retains branch `feat/4268-ground-contract` and base
  `feat/4197-capability-observer`. A normal merge combines exact published
  child `584e4e53a7990053a346938c4e45538786887ec7` with exact published live
  carrier parent `6d10afb184ff79b11be0bdeb6bc2722b3c7d8912` in child-first order; no
  rebase, retarget, force-push, or history rewrite is used.
- All implementation and test paths merge automatically. Conflicts are limited
  to both append-only handoffs, `SPEC.md`, and manifest carrier-head text; every
  history is retained. The strict `flight-to-ground/v1` schema, fixtures,
  migrations, adapters, dependencies, and fail-closed unimplemented-physics
  boundary remain unchanged while inheriting the current capability/wind stack.
- `SPEC.md` advances monotonically and uniquely to `1.14.25`. The manifest
  records exact published open heads for #4282 and #4285 while preserving
  historical merged #4283 head/merge identity, commit-bound evidence, and
  unreleased claims.
- Focused local verification is green: pinned Ruff 0.14.10 check/format on all
  32 child and inherited Python paths plus diff hygiene; 176 ground-contract,
  parity, catalog, kinetics, GUI, layout, and solver tests; strict manifest
  validation plus eight contract tests; 12 `swing-core` Rust tests; 77 React
  tests across 12 files; and repository docs governance.
- This merge is local only. Publication, protected exact-head CI, independent
  review, approval, normal #4288 propagation, and release remain open.
...
## 2026-08-11 live capability carrier receives refreshed workflow parent

- Historical PR `#4283` remains closed and merged at recorded head
  `9bbb98e16e435a0d4c74153b909f2ebfefbbce7a` and merge SHA
  `c1827bbdc50a6e11cc475db2636b4e47a4c15416`; it is not reopened or rewritten.
- Preserved live carrier branch `feat/4197-capability-observer` starts from exact
  child `6d10afb184ff79b11be0bdeb6bc2722b3c7d8912` and normally merges exact
  refreshed workflow parent `51033696634c624f7c92c3822d98a66875ff90a0`
  second, retaining child-first topology.
- Capability-observer functionality and history remain authoritative while the
  refreshed #4279–#4282 chain is inherited. Implementation and tests merge
  automatically; only append-only handoff/SPEC histories conflict textually.
- SPEC advances from `1.16.22` to unique `1.16.23`. The strict manifest keeps
  historical #4283 identity distinct from current live-branch ancestry and
  remains unreleased.
- Local qualification passes 145 focused Python/PyQt tests and 75 focused
  React tests across nine files, plus React type-check, zero-warning lint, and
  the 188-module production build. Pinned Ruff check/format, MyPy, Bandit,
  strict manifest/schema, docs governance, module-size, minimum-test, and diff
  gates pass. The capability observation JSON boundary now explicitly narrows
  its untyped imported serializer result to the declared `str` contract.
- This candidate is local only. No PR is reopened, and no remote, branch base,
  issue state, protected evidence, approval, or release is changed or claimed.

## 2026-08-11 #4283 carrier receives exact reconciled #4282 parent

- Historical PR `#4283` remains merged at its recorded reviewed head, while
  preserved branch `feat/4197-capability-observer` remains the dependency
  carrier for open draft PR `#4285`. A normal merge combines exact published
  carrier child `d0b827667e61f0583e1dad0b3cbbca6819624d3c` with exact published
  parent `8c6105d865abe130a68cfa35ffe6afb7eef850c8` in child-first order; no PR
  reopening, rebase, retarget, force-push, or history rewrite is used.
- All implementation and test paths merge automatically. Only both append-only
  handoffs and `SPEC.md` conflict textually, and every history is retained. The
  complete capability-observer functionality remains present while inheriting
  the current wind workflow and five-file GUI/solver repair.
- `SPEC.md` advances monotonically and uniquely to `1.14.24`. The campaign
  manifest records exact published #4282 head `8c6105d...` while preserving
  #4283's historical merged head/merge SHA, commit-bound evidence, and all
  unreleased claims.
- Focused local verification is green: pinned Ruff 0.14.10 check/format on the
  five inherited Python files plus diff hygiene; 110 capability, wind, GUI,
  layout, and solver tests; strict manifest validation plus eight contract
  tests; 77 React tests across 12 files; and repository docs governance.
- This carrier merge is local only. Publication, protected exact-head CI,
  dependency propagation into #4285, unresolved-thread checks, and release
  remain open.
...
## 2026-08-11 #4282 receives refreshed exact #4281 parent

- PR `#4282` retains branch `feat/4199-wind-workflow` and base
  `feat/4199-wind-scalar-adapter`. Exact published workflow child
  `8c6105d865abe130a68cfa35ffe6afb7eef850c8` is merged first and exact refreshed
  #4281 parent `73b7f6b45063ec71fa300a39d2185d3f935187b7`
  second through a normal no-ff merge.
- Responsive PyQt6/React wind execution, progress, cancellation, controls,
  scatter, persistence/export, and capability integration remain child-owned.
  All implementation and tests merge automatically while refreshed
  scalar-adapter, variation-export, and workspace ancestry is inherited.
- The append-only root/campaign handoffs and SPEC are reconciled additively;
  module handoff and manifest merge automatically. SPEC advances from
  `1.16.21` to unique `1.16.22`.
- The strict manifest records the currently published #4279–#4282 chain and
  remains explicitly unreleased. It does not self-reference this future local
  merge commit.
...
- Local evidence is green: pinned Ruff 0.14.10 check/format on 57 Python files,
  pinned MyPy 1.13 on 38 production files, Bandit at the medium/high threshold,
  150 focused Python/PyQt tests, 56 focused React tests, TypeScript,
  zero-warning ESLint, the 188-module Vite build, strict manifest/schema,
  documentation governance, 500-line budget, changed-Python policy,
  minimum-test contract, and diff hygiene. Independent review and fresh
  protected exact-head CI remain required.

## 2026-08-11 #4282 receives exact reconciled #4281 parent

- PR `#4282` retains branch `feat/4199-wind-workflow` and base
  `feat/4199-wind-scalar-adapter`. A normal merge combines exact published
  child `3faf9e9493c9ba37168d9f65ab10cafeedc2a72f` with exact published parent
  `e934ba563a79bf95ee892fea8593972c81299eeb`; no rebase, retarget,
  force-push, or parent rewrite is used.
- All implementation and test paths merge automatically. Only the append-only
  root handoff and `SPEC.md` conflict textually, and both histories are retained.
  Responsive PyQt6/React wind execution, progress/cancellation, controls,
  scatter, persistence/export, capability integration, and release authority
  remain unchanged while inheriting the current scalar-adapter ancestry.
- `SPEC.md` advances monotonically and uniquely to `1.14.23`. The campaign
  manifest advances to `as_of` 2026-08-11 and records exact published heads for
  open carriers #4281 and #4282 without rewriting commit-bound test evidence or
  making any protected/main release claim.
- Focused local verification is green: pinned Ruff 0.14.10 check/format on all
  61 child and inherited Python paths plus diff hygiene; 124 focused wind,
  playback, navigation, scalar-adapter, and GUI tests; strict manifest validation
  plus its eight contract tests; and 45 React tests across eight files.
- This merge is local only. Publication, protected exact-head CI, independent
...
## Inherited refreshed #4281 parent history

## 2026-08-11 #4281 receives refreshed exact #4280 parent
...
- PR `#4281` retains branch `feat/4199-wind-scalar-adapter` and base
  ...
  `e934ba563a79bf95ee892fea8593972c81299eeb` is merged first and exact newly
  published #4280 parent `7fb5d7f489db49742b7bc82ef009570ad2502456`
  second through a normal no-ff merge.
- All implementation and test paths merge automatically. The child remains
  authoritative for Python/TypeScript `scalar-ensemble/v1`, deterministic wind
  scenarios, and wind-strategy adaptation while inheriting the parent's current
  variation export and workspace/toolstrip ancestry.
- Only the three append-only handoffs and `SPEC.md` conflict textually; every
  dated record is retained. `SPEC.md` advances from the parent's `1.16.20` to
  unique `1.16.21`. The campaign manifest artifact/checker/test remain absent
  from both sides of this pre-manifest stack and are not recreated.
- This candidate is local only. No branch, PR base, remote, issue state,
  protected evidence, approval, or release claim is changed.
- Local evidence is green: pinned Ruff 0.14.10 check/format across the exact
  five-file Python delta, pinned MyPy 1.13 and Bandit across three production
  files, 101 focused Python/PyQt wind/variation/workspace tests, 18 focused
  React tests, TypeScript, zero-warning ESLint, the 166-module Vite build,
  documentation governance, 500/400-line budgets, changed-Python policy, and
  diff hygiene. Independent review and fresh protected exact-head CI remain
  required.

## 2026-08-11 #4281 receives exact reconciled #4280 parent

- PR `#4281` retains branch `feat/4199-wind-scalar-adapter` and base
  `feat/4144-variation-export-continuation`. A normal merge combines exact
  published child `9321c1d2e091b8c7e5a4a83aa9ad726290e7fb5a` with exact
  published parent `9b45bd5beca38370c1d541f8c488ef0edad08517`; no rebase,
  retarget, force-push, or parent rewrite is used.
- All implementation and test paths merge automatically. Only both append-only
  handoffs and `SPEC.md` conflict textually; every history is preserved. The
  Python/TypeScript `scalar-ensemble/v1` and wind-strategy adapter behavior is
  unchanged while inheriting current variation/export, workspace, kinetics,
  solver, layout, and Qt typing repairs.
- `SPEC.md` advances monotonically and uniquely to `1.14.22`. The later campaign
  manifest is absent from both sides of this pre-manifest stack.
- Focused local verification is green: pinned Ruff 0.14.10 check/format on ten
  child and inherited Python files plus diff hygiene; 21 wind scalar-adapter
  Python tests; 91 variation/workspace Python tests; 232 inherited
  workspace/kinetics/impact Python tests; and 52 React tests across 13 files.
- This merge is local only. Publication, protected exact-head CI, independent
  review, unresolved-thread checks, dependency order, and release remain open.
