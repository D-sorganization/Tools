...
| #4706 | Markerless mocap. M0/M1 PR #4734 is unmerged/conflicting; M2/M3 are local-only sibling stacks. #4714 M4 and #4721 M5 are `blocked_dependency_reconciliation`; protected main has no runtime authority.                 |
...
- #4721 extrinsic/flexible-layout calibration is
  `blocked_dependency_reconciliation` on current-main audit branch
  `reconcile/4721-extrinsic-layout-current-main`. Prior candidate
  `f371ce4f06ba9b904d1719a99221cffab44b4020` is not safe to cherry-pick: it
  bypasses M2/M3, depends on unreleased M4, and lacks complete frame/gauge,
  robust rejection, uncertainty, motion, recalibration, and provenance seams.
  Follow `docs/development/mocap_extrinsic_layout_4721_reconciliation.json`.
