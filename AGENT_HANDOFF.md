...
| #4277 — Within-Player Covariation and Population Meta-Analysis                | Active on `feat/4277-player-covariation`, stacked on the merged Neural Model Lab head of `feat/4226-launch-monitor-player-platform`; adds explicit-identity per-player, centered, pooled, between-player, and cross-player analyses with PyQt6/React parity. The private TrackMan v1 corpus has no trustworthy player ID, so it must not be presented as player-level evidence. |
