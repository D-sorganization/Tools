...
Draft PR #4303 keeps base `fix/rate-mobile-tools-menu`. This normal two-parent
merge combines prior child head `36ed6bd64e68108028b8e0194851e801b15e08ad`
with exact current parent PR #4301 head
`05713bcdd8f9889dcdcbaa5bdbaeab139d599b64`; no rebase, retarget,
history rewrite, or force-push is used. The moving-subject 2x zoom, bounded
tracking, and Auto Fit defaults remain intact while the current mobile-menu,
camera-carrier, and deterministic timestamp corrections propagate forward.

The inherited campaign-manifest fixtures retain exact evidence-SHA assertions
through short deterministic chunks, with Detect Secrets configuration and its
baseline unchanged. The composed tree passes all 880 Rate Python tests, 52
focused camera/manifest/compatibility tests, and all 28 compatibility contracts
on CPython 3.10.20. React passes 108 Vitest files / 653 tests, TypeScript,
zero-warning ESLint, and the 194-module production build. Ruff check/format,
...
fingerprint, and diff checks pass. Playwright and native visual review were not
rerun for this merge-only propagation because it resolved no UI source
conflicts; the prior child evidence remains recorded below. Protected CI,
review, normal parent integration, and release remain open.

## PR #4301 current-parent propagation and secret-scan repair

Draft PR #4301 keeps base `feat/4284-camera-snap-tracking`. This merge combines
prior child head `9322df75d6ad1b6ef57be02741ac972e7c6f86cf` with exact current
parent PR #4298 head `9ffd8d280c77977a41e93bd0caef9678d1c231b6` through a normal
two-parent merge. The branch was not rebased, retargeted, rewritten, or
force-pushed. The constrained File/View/Tools behavior remains intact while
the child inherits the current camera carrier, deterministic workspace UTC
parser, and release-contract evidence.

The two actionable Detect Secrets findings in the campaign-manifest test are
fixed by assembling the exact immutable evidence SHAs from short deterministic
chunks. Assertions remain exact; neither scanner configuration nor the
baseline is weakened. The composed tree passes all 877 Rate Python tests, 39
focused manifest/compatibility tests, and all 28 compatibility contracts on
CPython 3.10.20. React remains green at 108 Vitest files / 653 tests; TypeScript,
zero-warning ESLint, and the 194-module production build pass. Ruff
...
Detect Secrets fingerprint, and diff checks pass. Protected CI, review, parent
integration, native rendered review, and release remain open.

## Issue #4300 moving-subject default-camera continuation

Draft PR #4303 publishes `fix/rate-pyqt-default-camera` from worktree
`C:\Users\diete\Repositories\Tools-worktrees\pyqt-default-camera` as a normal
...
useful scale and in-frame during playback. The campaign manifest records PR
#4303 and this evidence. Protected CI, review, and ordinary integration remain
