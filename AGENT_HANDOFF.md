...
- T3 #5072: published grip-supported balance/operators `eeea63b47` (646 Linux golf/API passes). Private all-node frequency response now passes 50 focused controls, including independent dynamic-root rod/continuum, separate grip reactions, cycle power and rotating loaded balance. Shared clamped/finite-support solver preserves numerical refusal and point-wrench duality. See `GRIPPED_FREQUENCY_RESPONSE.md`; 665 Linux golf/API tests and all nine manual/type/lint gates pass; publication pending. Stability, bandwidth and time evolution remain open; no measured golf/acoustic claim.
