...
Current #5133 publication: 0cd6dce22. All Python shards/aggregates, the
UpstreamDrift consumer, wheel/browser and quality gates pass. Private
Gasification checkout access remains unresolved. A further downstream sweep
found the shared launcher still naming the Function Generator's pre-#5110
module. Its canonical YAML now matches the existing registration; a new
regression fails before the fix, then all 16 Function Generator tests pass.
Consumer metadata/ownership repair and final reviewed pin remain in #9916.
...
- T3 #5072 / review #5130: PR #5133 publishes 0cd6dce22; main 421889407 is integrated. The second published CI repair qualifies momentum and optimizer test oracles while preserving numerical equations and physical feasibility limits. Nine manual gates and root Ruff pass; source/JUnit evidence is in CI_REPAIR_RESULTS.json. Protected review and physical/FRF qualification remain open.
