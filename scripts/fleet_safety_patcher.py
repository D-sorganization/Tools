...
def patch_mypy_agent_guard(repo_path: Path):
    ...
    guard_logic = """
    # LOBOTOMY GUARD: Prevent destructive fixes
    if len(new_lines) < len(lines) * 0.5 and len(lines) > 10:
        if verbose:
            print(f"!!! LOBOTOMY GUARD TRIGGERED for {error.file}: New content is too small. Skipping fix.")
        return False
    """
    
    # We need to find the place where fixes are applied to the file context
    # Usually in the fix functions or the run_agent loop.
    # Looking at my previous view_file, let's insert a generic check in fix_generic_suppression or similar
