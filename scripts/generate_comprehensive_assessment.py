...
def generate_assessments(grades: dict[str, tuple[float, str]], stats: RepoStats):
...
def generate_issues(grades: dict[str, tuple[float, str]]):
    for category, (score, justification) in grades.items():
        if score < 5.0:
            name = CATEGORIES[category]
...
def generate_comprehensive(grades: dict[str, tuple[float, str]]):
    weighted_score = (
