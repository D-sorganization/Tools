...
def fix_remaining_long_lines(file_path: str) -> bool:
    ...
    for i, line in enumerate(lines):
        ...
        if line.strip().startswith('print(') and 'f"' in line:
            ...
            if 'DEBUG:' in line or 'Error' in line or 'Warning' in line:
                ...
                if match:
                    ...
                    for break_point in [': ', ' - ', ', ', ' from ', ' to ', ' with ', ' for ', ' in ', ' of ', ' = ']:
                        if break_point in content_text and len(content_text) > 60:
                            ...
                            if 20 < len(parts[0]) < 70 and len(parts[1]) < 70:
                                new_line1 = line.replace(f'f"{content_text}"', f'f"{parts[0]}{break_point.rstrip()}"')
                    ...
                    for break_point in [': ', ' - ', ', ', ' from ', ' to ', ' with ', ' for ', ' in ', ' of ']:
                        if break_point in content_text:
                            ...
                            if 20 < len(parts[0]) < 70 and len(parts[1]) < 70:
                                ...
                                new_line1 = line.replace(f'{prefix}{content_text}"', f'{prefix}{parts[0]}{break_point.rstrip()}"')
        ...
        # Fix long comments
            ...
            # Break at logical points
            for break_point in [' - ', ': ', ', ', ' and ', ' or ', ' but ', ' with ', ' for ', ' to ', ' from ']:
        ...
        # Fix long string literals in other contexts
            ...
            if any(keyword in line for keyword in ['messagebox.', 'success_message', 'debug_text']):
                indent = len(line) - len(line.lstrip())
                # Find string content
                ...
                if quote_matches:
                    for match in quote_matches:
                        ...
                        if len(content_text) > 60:
                            # Break at logical points
                            for break_point in ['. ', ': ', ', ', ' - ', ' and ', ' or ', ' with ', ' for ', ' to ', ' from ']:
                                if break_point in content_text:
                                    parts = content_text.split(break_point, 1)
                                    if 20 < len(parts[0]) < 70 and len(parts[1]) < 70:
                                        new_line1 = line.replace(f'"{content_text}"', f'"{parts[0]}{break_point.rstrip()}"')
...
def add_basic_type_annotations(file_path: str) -> bool:
    ...
    replacements = [
        ...
        ('self.loaded_data_cache = {}', 'self.loaded_data_cache: dict[str, pd.DataFrame] = {}'),
        ('self.processed_files = {}', 'self.processed_files: dict[str, pd.DataFrame] = {}'),
        ('self.signal_vars = {}', 'self.signal_vars: dict[str, tk.BooleanVar] = {}'),
        ('self.plot_signal_vars = {}', 'self.plot_signal_vars: dict[str, tk.BooleanVar] = {}'),
        ('self.custom_vars_list = []', 'self.custom_vars_list: list[dict[str, Any]] = []'),
        ('self.reference_signal_widgets = {}', 'self.reference_signal_widgets: dict[str, Any] = {}'),
        ...
        ('self.integrator_signal_vars = {}', 'self.integrator_signal_vars: dict[str, tk.BooleanVar] = {}'),
        ('self.deriv_signal_vars = {}', 'self.deriv_signal_vars: dict[str, tk.BooleanVar] = {}'),
        ('self.custom_legend_entries = {}', 'self.custom_legend_entries: dict[str, str] = {}'),
    ]
    ...
    function_fixes = [
        ...
        ('def process_single_csv_file(', 'def process_single_csv_file('),  # Already has types
    ]
...
def main() -> None:
    ...
    file_path = "data_processing/data_processor/python/data_processor/Data_Processor_r0.py"
    path = Path(file_path)
    if path.exists():
