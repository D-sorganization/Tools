...
| 2026-04-12 | 1.1.50  | Replace remaining `print()` calls with `logging` across `src/` modules and disable xvfb pytest plugin to fix CI timeout on headless runners. |
...
| 2026-04-15 | 1.1.50  | Optimized exponential and power regression calculation in `useDataProcessor.ts` by replacing chained array methods with single-pass loops and pre-allocated arrays to eliminate GC overhead. |
...
| 2026-04-18 | 1.1.50  | Added `sr-only` context tags to the video progress text readouts and the pose detection counters to ensure screen readers provide contextual meaning to the numbers. Applied `aria-hidden="true"` to purely decorative pulsing dots. |
