...
- 2026-05-31: feat(ux, #3115) — improve accessibility of the ODE Solver UI by explicitly linking `label`s to `input`s and `textarea`s using `htmlFor` and unique `id`s generated by `React.useId()`. Add `spellcheck="false"` and disabled autocorrect on math text areas to prevent mobile OS interference with equations.
