...
- **2026-08-24**: fix(ci, rust) — Raise the standard Rust quality-gate job budget from 15 to 45 minutes and regression-lock that minimum. The lane compiles default and Python-feature tests, installs and runs `cargo-audit`, builds Python and two WASM artifacts, and executes benchmarks; two otherwise-green PR attempts reached the former job limit while `cargo-audit` was compiling, before the artifact gates could run. Clear the checkout action's repository-scoped GitHub authorization header before `cargo audit` performs its anonymous public RustSec advisory-database fetch, preventing an otherwise healthy audit from failing closed with HTTP 401 while retaining advisory failures as blocking.
