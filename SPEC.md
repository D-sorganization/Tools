...
| 2026-04-13 | 1.1.52  | Replaced chained `.filter().map()` and `.map().filter().map()` operations with single-pass loops utilizing pre-allocated arrays in `useDataProcessor.ts` `calculateTrendline` (exponential and power calculation branches) to prevent intermediate array creation and optimize performance.                                                                                                                                                                                                                                                                                         |
