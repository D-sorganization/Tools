...
The repository caps source files at 400 lines; files at/above 800 lines are
structural debt (at/above 1000 = CRITICAL). `docs/development/SIZE_DEBT_REGISTER.md`
is the ranked, reproducible work queue for these monolith files (issue #3261),
generated by `scripts/generate_size_debt_register.py` (`--write` to refresh,
`--check` to verify freshness). Refactor one file per PR and regenerate so the
count ratchets down. The register is informational, not a blocking CI gate.
