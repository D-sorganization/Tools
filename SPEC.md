...
- Workflow Lint now installs `actionlint` into a runner-local directory added
  through `GITHUB_PATH` instead of moving it into `/usr/local/bin` with `sudo`,
  preserving self-hosted runners that do not allow passwordless sudo while
  keeping the existing actionlint invocation unchanged (#3761).
- CI Standard now keeps the shared apt lock for dependency installation but
  ...
  instead of failing before quality/tests can start (#3761).
...
| ---- | ------- | ------- |
| 2026-06-19 | 1.1.7675 | test(pressure-drop, #3737): expose the singular `PressureDropInput` Pydantic model from `src.pressure_drop_calculator`, preserve construction-time gas-composition validation, and add focused coverage for defaults, missing gas composition, and backend model coexistence. |
