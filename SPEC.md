...
| ---- | ------- | ------- |
| 2026-08-20 | 1.17.53 | ⚡ Bolt: Optimize SVG path rendering in LaunchMonitorLinkedScatter by replacing chained `.map().join()` with single-pass string concatenation, and fix exactReleaseBuild to execute `build:release` so browser qualification release manifests are correctly generated. | #4534 |
