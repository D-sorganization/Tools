...
| ---------- | ------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-05-28 | 1.1.216 | Test infrastructure: registered the `--regenerate-api-baseline` pytest option in root conftest (was referenced but never defined, erroring the sidekick API-stability test) and made its `getoption` defensive (#3097); added an autouse fixture in the sidekick tests conftest stubbing blocking modal `QMessageBox.warning/critical/information/about` so the full suite can no longer hang headlessly (#3096). Removing the hang revealed pre-existing test-isolation failures now observable for the first time.                                                |
| 2026-05-28 | 1.1.215 | Restored sidekick psa_gui.py to its intended thin re-export facade over the .ui package (removed ~990 lines of dead duplicate class bodies whose import-time NameError broke pytest collection for the entire sidekick suite); repointed psa_gui test mock-patch targets to ui.sensitivity_plot/ui.main_window; narrowed a broad except in tools_sidebar/sidebar.py to (JSONDecodeError, TypeError, ValueError).                                                                                                                                                    |
