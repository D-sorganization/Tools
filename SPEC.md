...
| ---- | ------- | ------- |
| 2026-08-26 | 1.18.24 | test(mocap, #4716 TOOLS-M9): add a main-rooted acceptance-only contract that keeps C3D runtime/schema work blocked on protected M0/M1 authority, requires normalized semantic comparison across frames/units/timestamps/confidence/skeleton/events/residuals/analog/force/provenance/unknown metadata, makes the seven-camera overflow unavailable with an external digest-bound loss sidecar, requires nonoptional ezc3d/python-c3d/BTK/C3D.org oracles, and rejects the stale local prototype's competing schema, byte-equality, optional-oracle, first-seven truncation, embedded-only loss, and missing-corpus behaviors. No runtime completion is claimed. |
