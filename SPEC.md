...
| ---- | ------- | ------- |
| 2026-06-15 | 1.1.465 | fix(ci, #3335): install `python-multipart` wherever `ci-standard.yml` installs FastAPI so the URDF viewer upload route can be imported in the Python matrix, and add an ops regression that prevents FastAPI-only CI dependency drift. |
