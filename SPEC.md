...
| 2026-06-10 | 1.1.333 | fix(ci, #3300): merge current `main` into `fix/ci-bugs-3291-3294-3295-3296-3284`, preserve the competing SPEC changelog entries, make the plant simulator and pendulum native wrappers mypy-clean under the changed-file quality gate, and remove workspace-level PyO3 `extension-module` linkage from Rust test builds so `cargo test --features python` can link while maturin drives Python wheel builds through separate `python` and `extension-module` features. |
...
| 2026-06-10 | 1.1.333 | fix(bug/ci, #3291 #3294 #3295): make `start-gaai-daemon.sh` non-destructive by default with project-scoped Claude settings, explicit global opt-in, and no-op dry runs; declare pendulum `Golfer` dynamics native-only with construction-time `RuntimeError` guidance and an explicit workspace exclude for `pendulum-core`; and remove `plant_simulator`'s silent random-data path so `SCADADataset` loads real SQLite `taglog` rows unless synthetic data is explicitly requested. |
...
| 2026-06-09 | 1.1.330 | fix(bug/ci, #3291 #3294 #3295 #3296): make `start-gaai-daemon.sh` non-destructive — it now writes `skipDangerousModePermissionPrompt` to a project-scoped `.claude/settings.local.json` (jq-merge + timestamped backup, never overwrite), requires `--yes` to touch the global `~/.claude/settings.json`, and makes `--dry-run` change nothing; declare the pendulum `Golfer` (8-DOF) dynamics native-only — `Golfer(...)` now raises `RuntimeError` at construction with maturin build guidance instead of returning an object whose every method raises later, the misleading "falling back to NumPy" golfer log is removed, stale closed-issue #1736 references are retargeted to #3294, and `pendulum-core` is given an explicit, commented workspace `exclude`; remove `plant_simulator`'s silent random-data path — `SCADADataset` now loads real `taglog` rows from SQLite and raises `ValueError` on insufficient data unless `allow_synthetic=True` is explicit, and the package is marked experimental; and harden the anti-phantom-merge workflow so the untrusted PR head is checked out only on the `pull_request` trigger (never in the privileged `pull_request_target` context), with an inline no-execute invariant comment. Adds regression tests for the golfer fail-fast and the plant-simulator loader. |
