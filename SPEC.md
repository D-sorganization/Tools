...
| 2026-08-31 | 1.18.95 | perf(p1am): optimize `PowerSupplyTrend` render by eliminating array allocation overhead. Replaced chained array spreads and `.map()` calls with pre-allocated arrays and single-pass iteration, reducing garbage collection stutter. | #4881 |
