...
| ---- | ------- | ------- |
| 2026-06-20 | 1.1.7788 | perf(data-processor, #3682): vectorize the studentized-bootstrap jackknife in `uncertainty_quantification._studentized_interval` by reusing a precomputed leave-one-out index matrix instead of calling `np.delete` inside a double loop, removing the per-element reallocation while keeping results numerically identical (brute-force equality regression added). |
