...
- PyQt6 and React now connect the established strict whole-workspace and
  compositor contracts to visible File commands. New, Open, Save As, strict
  view-layout Import/Export, and Close are live on both surfaces; PyQt6 also
  provides atomic Save-over-current-path and persisted Open Recent.
- A whole-workspace document restores the supported explorer calculation
  slice (impact scenario, club specification, units), primary module
  order/visibility/selection, and the versioned multi-view compositor. Both
  clients completely parse and validate before mutation. Unsupported torque
  profiles or variation plans fail closed rather than being silently dropped.
- Native writes replace the destination atomically. Both clients protect dirty
  sessions with Save/Discard/Cancel or confirmation before New/Open/Close,
  preserve live state after invalid or cancelled reads, and expose actionable
...
- Draft PR #4301 keeps base `feat/4284-camera-snap-tracking` and normally
  merges exact #4299 head `142631a90c008942bad99745e279748a7eda2ffa`
  into original constrained-toolstrip child
  `05713bcdd8f9889dcdcbaa5bdbaeab139d599b64`, with the child as first parent.
  No rebase, retarget, history rewrite, or force-push is permitted.
- File, View, and Tools popovers share one collision clamp that preserves an
  unchanged desktop anchor and native `<details>/<summary>` keyboard and
  accessibility behavior while enforcing a 16 px constrained-screen gutter
  and a viewport-bounded menu width.
- The composed tree inherits the declared four-surface inventory, complete
  camera/playback controls, and repaired flight-to-ground ancestry without
  claiming native rendered qualification, installed-consumer conformance,
  protected release, or completion of #4300/#4284/#4264/#4260.
...
- The standalone PyQt6 and React shells expose matched `Ground Surfaces`
  ...
  material-overlay rows. All v1 material fields, identities, interval bounds,
  precedence, and source revision remain visible and editable.
- Both presentation adapters bind the actual draft to SHA-256 provenance and
  delegate to the strict `ground-regional-material-plan-request/v1` parser.
  They do not duplicate ground physics, weaken validation, or extend the
  frozen flight-to-ground v1 schemas.
- The initial values are visibly illustrative and unvalidated. Validation
...
| 2026-08-11 | 1.14.31 | chore(rate-of-closure, #4261): propagate exact `fix/rate-mobile-tools-menu` parent `c653f9ff9193d6cdb8e11a13ad0001707e468a42` through the runtime-manifest child by an ordinary conflict-free merge; preserve the reviewed contract-only implementation and stack base. |
| 2026-08-11 | 1.14.30 | fix(rate-of-closure, #4261): reject complete placeholder tokens beside every allowed stable-ID separator in mirrored Python and TypeScript validation while preserving legitimate longer substrings; pin the behavior in the shared parity fixture, remove the redundant MyPy reason cast, and preserve the contract-only non-completion boundary. |
| 2026-08-11 | 1.14.29 | fix(rate-of-closure, #4261): complete independent-review repair with exact cross-runtime Unicode White_Space boundary semantics, valid astral/non-BMP text support, unpaired-surrogate rejection, and shared reason-policy fixtures; keep safe-magnitude enforcement local to runtime-manifest options while restoring the established shared Python canonical encoder and capability-observation `1e21` behavior; preserve the contract-only non-completion boundary. |
| 2026-08-11 | 1.14.28 | fix(rate-of-closure, #4261): close independent-review gaps by enforcing the JavaScript safe-magnitude bound for Python floats and the shared canonical encoder, strict SemVer numeric identifiers, and a mirrored deterministic substantive unavailable-reason grammar; add boundary/adversarial Python-TypeScript tests, correct recursive `docs/specs/**/*.md` inventory prose, and preserve the contract-only non-completion boundary. |
| 2026-08-11 | 1.14.27 | feat(rate-of-closure, #4261): add the strict immutable `calculation-runtime-manifest/v1` contract in Python and TypeScript with exact four-surface/build identities, complete impact/flight/ground availability ledgers, model/backend/integrator/schema/frame/unit provenance, unit-explicit finite numerical options, duplicate-field rejection, deterministic shared canonical bytes, and adversarial parity tests; retain a contract-only boundary with no live run, workspace, regional-ground, or UpstreamDrift integration claim. |
