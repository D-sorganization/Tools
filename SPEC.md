...
| 2026-06-02 | 1.1.271 | test(chat,programmatic_pid,humanoid): add functional coverage for three under-tested shared packages, each in a dedicated, fully type-annotated test file to keep the delta-CI mypy surface clean. `chat/tests/test_chat_routing_lifecycle.py` drives the real `create_chat_router` WebSocket dispatch against a concrete `ChatServiceBase` subclass — asserting session lifecycle, conversation-history accumulation, and message routing (send/history/new_session/condense/skill_invoke/request_review/refresh_models/index_codebase) — plus `chat.models` contract objects (#3184). `programmatic_pid/tests/test_pipeline_e2e.py` runs the full spec->generate->render pipeline on the committed biochar golden spec, asserting document/stream/instrument structure and emitted DXF link/text/layer entities (generator 100%, rendering 69%, document 68%) (#3185). `humanoid_character_builder/tests/test_builder_assembly.py` performs a full character build asserting assembled segment set, mass conservation, and per-segment inertia, then parses the emitted URDF asserting link/joint topology, inertial blocks, and visual geometry, plus preset-driven assembly and a preset file round-trip (api 73%, urdf_generator 83%, loader 70%) (#3186). |
