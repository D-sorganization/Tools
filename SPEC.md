...
| 2026-04-18 | 1.1.76  | Added screen-reader-only context to dynamic video progress text and pose detection counters so numeric readouts expose their meaning to assistive technology; decorative pulsing dots are now hidden from screen readers (PR #2138).                                                                                                                                                                                                                                                                                                                                |
...
| 2026-04-21 | 1.1.82  | Optimized AnalyticsSuite `solveLinearSystem` by replacing chained `A.map()` and `[...row]` spread operators with single-pass loops and pre-allocated arrays, eliminating O(N^2) memory allocations. |
...
- `solveLinearSystem` in `AnalyticsSuite` uses pre-allocated arrays and manual looping instead of chained `.map()` and array spread operations to create augmented matrices, mitigating heavy garbage collection and O(N^2) memory allocation overhead on large analytical regression sets.
