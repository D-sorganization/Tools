...
## 2024-07-28 (Bolt): Optimize min/max calculation for trend axes

- Replaced `Math.min(...values)` and `Math.max(...values)` with a single-pass `for` loop in `src/p1am_control_system/frontend/src/lib/trendAxis.ts`.
- Improved execution time and eliminated 'Maximum call stack size exceeded' errors when rendering trend charts with large datasets.
