...
### 2026-08-31 Plot Canvas Performance Optimization

Version 1.18.95 optimizes the canvas rendering hot path in `PlotCanvasCard.tsx` by replacing intermediate `.forEach` iterations with standard `for` loops, eliminating closure allocation overhead when iterating over large datasets.
