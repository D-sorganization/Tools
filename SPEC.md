...
| ---- | ------- | ------- |
| 2026-09-03 | 1.18.126 | fix(readme, metadata contract): restore the Python-version contract strings the #4742 landing-page rebuild dropped — the badge URL regains the literal `python-3.11+` path segment and the Installation prerequisites state "Version **3.11+** required", satisfying `tests/unit/test_project_metadata_contracts.py::test_readme_python_badge_matches_package_requires_python` (red on main; no 3.10 claims introduced). |
| 2026-09-03 | 1.18.125 | fix(types): satisfy repo-wide pre-push mypy in the swing_sim impact files merged from #4945 — wrap the Any-returning float expressions in `swing_sim/impact/models.py` (`_compute_effective_club_mass`), `impact_interval/tests/test_solver.py` (`_reduced_mass`), and `swing_sim/putting/stroke_interchange.py` (`_validated_triplet` now builds the tuple explicitly). Behavior is unchanged (verified by the 10 impact-interval solver tests); the module-inventory swing_sim shard is regenerated. |
| 2026-09-03 | 1.18.124 | fix(a11y): add explicit `id` and `htmlFor` label associations to inputs in `ControlDashboard` and `TagInspector` to improve screen reader accessibility. |
