...
| 2026-06-02 | 1.1.273 | test(codemap): add focused headless coverage for the Markdown tree-sitter extractor, including ATX heading extraction from byte input, long heading truncation, unavailable-parser fallback, raw heading fallback text, and blank heading skipping, raising `src/shared/python/codemap/_lang_markdown.py` focused coverage from 0.00% to 100.00% without changing production behavior. |
