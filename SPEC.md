...
| 2026-06-01 | 1.1.280 | fix(sidekick,ui,theme): delete the dead `process_calculators/syngas_compression/` placeholder subpackage (empty `__all__=[]` stubs) so `SyngasCompressionEngine` has exactly one canonical definition in `syngas_compression_engine.py`, guarded by an AST import-graph dedup test (#3183); guard the PyQt6-dependent imports in `ui/__init__.py` with `try/except ImportError` (mirroring `theme/__init__`) so the `ui` package imports cleanly in headless environments and exposes the Qt widgets as `None` when PyQt6 is absent, with a subprocess headless-import regression test (#3187); and add a drift-guard test asserting the hand-maintained `_HARDCODED_BUILTIN_THEMES`/`_HARDCODED_CHART_COLORS` fallbacks stay equal to the canonical `theme-definitions/themes.json`, keeping `themes.json` the single source of truth (#3188). Also regenerated `tests/sidekick_api_baseline.json` to record the `ui/tools_sidebar/*` modules added by #3147 and drop the deleted syngas stubs. |
