...
| ---- | ------- | ------- |
| 2026-08-26 | 1.18.24 | fix(ci, #4706): include UpstreamDrift's canonical `src/bunkershot3d/postproc/wrench_trace.py` in the narrow non-cone downstream sparse checkout and pin the exact path with a deterministic workflow contract; keep consumer qualification explicitly serial with `pytest -n 0` without changing runner routing. |
