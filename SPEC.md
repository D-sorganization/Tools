...
| ---- | ------- | ------- |
| 2026-08-22 | 1.17.89 | docs(rate-of-closure, #4142/#4433): reconcile both bounded handoffs after protected PR #4646 merged. The records pin its merge commit as an ancestor of current remote `main` and preserve the distinction between passing PR-head checks and incomplete post-merge trusted evidence: React stopped before browser execution on a transient shared-runner apt lock, and dependent PyQt failed closed on the absent React artifact. A single post-campaign rerun, requirement-level audit, and immutable UpstreamDrift consumer pin/parity slice remain. No runtime, baseline, or scientific authority changes. |
