...
| Date       | PR         | Changes    |
| ---------- | ---------- | ---------- |
| 2026-09-10 | #5169 | Estimate global metric scale from identified ruler endpoints and established camera geometry; preserve source evidence, rotations and lens profiles. Validate independent held-out lengths, raw reprojection, parallax, zoom identity and anchored projection invariance. Physical accuracy and engineering-manual publication remain unapproved. Preserve main 28de742e2 chart-bound fixes, regenerate the combined provider inventory, and register only the three additive Sidekick API modules with every existing entry unchanged. Preserve concurrent scheduled-agent integration 4c5010e55 and its chart fixes during successor turnover. |
