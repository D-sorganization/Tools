...
| ---- | ------- | ------- |
| 2026-08-15 | 1.5.8 | fix(tests): stop `tests/unit/codemap/conftest.py` skipping the entire test suite. Its `pytest_collection_modifyitems` hook received the whole session's item list — not just the items beneath it — so whenever `tests/unit/codemap` was collected without the optional `codemap` extra installed, every test in the run was marked skipped and pytest still exited 0. Replaced with a directory-scoped autouse fixture; added `tests/architecture/test_conftest_hook_scoping.py` (an AST guard forbidding the hook in nested conftests plus a subprocess canary) to the always-on CI core test set. Locally `pytest tests/` goes from 8411 skipped / 0 passed to 8099 passed / 91 skipped. |
