...
| 2026-04-15 | 1.1.51  | Optimized exponential and power regression calculation in `useDataProcessor.ts` by replacing chained array methods with single-pass loops and pre-allocated arrays to eliminate GC overhead. |
...
| 2026-04-18 | 1.1.51  | Added `sr-only` context tags to the video progress text readouts and the pose detection counters to ensure screen readers provide contextual meaning to the numbers. Applied `aria-hidden="true"` to purely decorative pulsing dots. |
| 2026-04-18 | 1.1.51  | Gracefully skipped tests requiring numpy and pandas to allow the core test suite to run in CI environments without those heavy data science dependencies installed. |
