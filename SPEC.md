...
| ---- | ------- | ------- |
| 2026-08-14 | 1.5.7 | ci(ops): add `tests/ops/` to the always-on `core_tests` set — those tests assert on `.github/workflows/*.yml`, so editing a workflow never enlisted them, and three had been red since `quality-gate` moved to a hosted runner and left the self-hosted tool-cache steps behind. Retarget the stale assertions at the mechanism rather than a job-name list: the persistent-tool-cache and cache-clean/restore contracts now key on the jobs that opt into the shared fleet tool cache (and assert such a job is self-hosted, and that the cleaner's version argument tracks that job's own `setup-python` request), while `quality-gate`'s pip-isolation contract asserts the `$RUNNER_TEMP/ci-venv` + `PYTHONNOUSERSITE` isolation actually in force. |
| 2026-08-14 | 1.5.7 | fix(imports): repair every dead first-party import in `src/` and add `tests/architecture/test_import_resolvability.py` to keep them out. `mypy.ini`'s global `ignore_missing_imports = True` is the sole suppressor of `import-not-found` (`--follow-imports=skip` is not), so imports naming modules that never existed survived indefinitely: `signal_toolkit.polynomial_generator` silently fell back to bare logging (`shared.python.logging_config` → `shared.python.logging_pkg.logging_config`), and `model_generation.humanoid` / `model_generation.mesh` wrapped their re-exports in a single `try/except ImportError: pass` that one missing module aborted wholesale — leaving all 34 and 14 exported names dead behind `# mypy: ignore-errors`. Both facades now import from the modules that actually define those names, with the pragmas removed. The guard resolves by filesystem path walk against declared roots rather than `importlib.util.find_spec`, which false-positives when a parent package raises on import; its only exemption is `shared.python.launcher_embed`, which UpstreamDrift supplies when it hosts the migrated `data_explorer` embed adapter. |
...
| 2026-08-14 | 1.5.6 | fix(architecture): make the `tests/architecture/` boundary guards always-on in the `tests` job's `core_tests` set (whole directory, so new guards are covered on arrival) instead of running only when their own file changed — a blind spot that hid four guards failing against `main`. Land the fixes those guards demand: canonicalize the six `src/shared/python/sidekick/{__main__,standalone/*}.py` modules off the deprecated bare `sidekick.`/`theme.`/`ai.` alias spellings onto relative and `shared.python.*` imports; repoint `test_gh1696_god_modules.py`'s `pressure_drop_calculator` path constants at the canonical `sidekick` tree left behind by the `upstream_drift_tools` rename; and cache `_shared_python_roots()`, whose per-import-node directory rescan made the guard take ~67 s against a 60 s pytest timeout. |
