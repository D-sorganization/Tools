...
| Unified Tools Sidebar    | `src/shared/python/upstream_drift_tools/ui/tools_sidebar/`                             | Optional Qt dock widget contract for downstream host applications, including project-scoped file browsing, workspace registry/state persistence, reusable Sidekick design tokens, stable stylesheet selectors, embedded Sidekick runtime widgets for chat status, workspace Python execution, symbolic calculator evaluation, project notes, and unit conversion, plus configurable tabbed utility surfaces for host apps |
