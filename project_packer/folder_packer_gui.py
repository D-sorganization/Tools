...
class FolderPackerGUI:
    ...
    def __init__(self, root):
        ...
        try:
            ...
            try:
                ...
                ctypes.windll.shell32.SetCurrentProcessExplicitAppUserModelID("FolderPacker.Tool.1.0")
            except Exception as e:
        ...
        self.file_extensions = {
            '.py', '.html', '.css', '.js', '.m', '.txt', '.json', '.xml',
            '.cpp', '.java', '.c', '.h', '.hpp', '.php', '.rb', '.go',
            '.rs', '.swift', '.kt', '.r', '.sql', '.sh', '.bat', '.yml',
            '.yaml', '.md', '.rst', '.tex', '.vue', '.jsx', '.tsx', '.ts'
        }
        
        # Define common archive/backup folder patterns
        self.common_exclude_patterns = [
            '*_archive', '*Archive', '_Archive', '*_backup', '*Backup', '_Backup',
            '*_old', '*_OLD', '*.old', '.git', '__pycache__',
    ...
    def setup_ui(self):
        ...
        style.configure('Title.TLabel', font=('Segoe UI', 22, 'bold'), foreground=self.colors['text_dark'], background=self.colors['background'])
        style.configure('Section.TLabelframe', background=self.colors['background'], borderwidth=0)
        style.configure('Section.TLabelframe.Label', font=('Segoe UI', 12, 'bold'), foreground=self.colors['primary'], background=self.colors['background'])
        style.configure('TButton', font=('Segoe UI', 12), padding=8)
        style.configure('TLabel', font=('Segoe UI', 11), background=self.colors['background'], foreground=self.colors['text_dark'])
        style.configure('TCheckbutton', font=('Segoe UI', 11), background=self.colors['background'], foreground=self.colors['text_dark'])
        style.configure('TNotebook', background=self.colors['background'], borderwidth=0, tabmargins=[2, 2, 2, 0])
        style.configure('TNotebook.Tab', font=('Segoe UI', 11), padding=[12, 6], background=self.colors['background'], foreground=self.colors['primary'], borderwidth=0)
        style.map('TNotebook.Tab',
                  background=[('selected', self.colors['legal_pad_yellow']), ('!selected', self.colors['background'])],
                  foreground=[('selected', self.colors['primary']), ('!selected', self.colors['primary'])])
        ...
        # Title
        title_label = ttk.Label(main_container, text="Folder Packer / Unpacker Tool", style='Title.TLabel', anchor='center')
        ...
        status_bar = tk.Label(self.root, textvariable=self.status_var, anchor='w',
                             ...
                             font=('Segoe UI', 11), relief='flat', bd=1, padx=12, pady=6)
    ...
    def setup_main_tab(self, parent):
        ...
        self.pack_button = ttk.Button(button_frame, text="Select Project to Pack", command=self.pack_folder_clicked, width=22)
        self.pack_button.grid(row=0, column=0, padx=24)
        self.unpack_button = ttk.Button(button_frame, text="Select Project to Unpack", command=self.unpack_file_clicked, width=22)
        self.unpack_button.grid(row=0, column=1, padx=24)

        # Output/Progress area
        output_frame = ttk.LabelFrame(main_frame, text="Progress & Output", style='Section.TLabelframe', padding=12)
        output_frame.pack(fill='both', expand=True, pady=(18, 0), padx=8)
        self.output_text = scrolledtext.ScrolledText(output_frame, wrap=tk.WORD, width=90, height=16,
                                                    bg=self.colors['background'], fg=self.colors['text_dark'],
                                                    font=('Consolas', 12), insertbackground=self.colors['primary'],
                                                    selectbackground=self.colors['secondary'], borderwidth=1, relief='solid')
    ...
    def setup_instructions_tab(self, parent):
        ...
        instructions_label = ttk.Label(instructions_frame, text="Instructions", style='Section.TLabelframe.Label', anchor='w')
        instructions_label.pack(anchor='w', pady=(0, 12))
        instructions_text = ttk.Label(instructions_frame, text=instructions, wraplength=800, justify=tk.LEFT, style='TLabel')
    ...
    def setup_exclusions_tab(self, parent):
        ...
        # Instructions
        instructions = ttk.Label(exclusions_frame, 
        ...
        left_frame = ttk.LabelFrame(columns, text="Common Exclude Patterns", style='Section.TLabelframe', padding=14)
        ...
        for i, pattern in enumerate(self.common_exclude_patterns):
            var = tk.BooleanVar(value=pattern in ['*_archive', '*Archive', '_Archive', '*_backup', '*Backup', '_Backup'])
            self.pattern_vars[pattern] = var
            cb = ttk.Checkbutton(checkbox_frame, text=pattern, variable=var, style='TCheckbutton')
            cb.grid(row=i, column=0, sticky='w', padx=4, pady=2)
        
        # Right: Custom exclusions
        right_frame = ttk.LabelFrame(columns, text="Custom Folder Exclusions", style='Section.TLabelframe', padding=14)
        ...
        self.exclusion_listbox = tk.Listbox(listbox_frame, yscrollcommand=scrollbar.set, height=10,
                                           bg=self.colors['background'], fg=self.colors['text_dark'],
                                           selectbackground=self.colors['secondary'],
                                           selectforeground=self.colors['text_dark'],
                                           font=('Segoe UI', 11), borderwidth=1, relief='solid')
        ...
        add_folder_btn = ttk.Button(button_frame, text="Add Folder", command=self.add_excluded_folder)
        add_folder_btn.pack(side=tk.LEFT, padx=(0, 8))
        add_pattern_btn = ttk.Button(button_frame, text="Add Pattern", command=self.add_excluded_pattern)
        add_pattern_btn.pack(side=tk.LEFT, padx=(0, 8))
        remove_btn = ttk.Button(button_frame, text="Remove Selected", command=self.remove_excluded)
        remove_btn.pack(side=tk.LEFT)
        # Info label
        info_label = ttk.Label(exclusions_frame, 
    ...
    def add_excluded_pattern(self):
        ...
        pattern = simpledialog.askstring("Add Pattern", 
    ...
    def pack_folder_clicked(self):
        ...
        source_folder = filedialog.askdirectory(
            title="Select Project Folder to Pack",
        ...
        thread = threading.Thread(target=self.pack_folder, 
                                args=(source_folder, output_file))
        thread.start()
    ...
    def pack_folder(self, source_folder, output_file):
        ...
        try:
            ...
            if excluded_count > 0:
                self.log_message(f"Excluded {excluded_count} files based on exclusion patterns")
            
            if not files_to_pack:
                self.log_message("No supported files found after applying exclusions!", "WARNING")
            ...
            with open(output_file, 'w', encoding='utf-8') as f:
                ...
                f.write(f"%%%%%% Pack Time: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} %%%%%%\n")
                f.write(f"%%%%%% Total Files: {len(files_to_pack)} %%%%%%\n")
                f.write(f"%%%%%% Files Excluded: {excluded_count} %%%%%%\n")
                if exclusions:
                    f.write(f"%%%%%% Active Exclusions: {', '.join(sorted(exclusions))} %%%%%%\n")
                f.write("%%%%%% Supported Extensions: " + ", ".join(sorted(self.file_extensions)) + " %%%%%%\n\n")
                    ...
                    try:
                        ...
                        try:
                            ...
                            try:
                                ...
                                if not content.endswith('\n'):
                                    f.write('\n')
                                self.log_message(f"  Note: Used latin-1 encoding for {relative_path_str}", "WARNING")
                            except Exception as e:
                                f.write(f"****** ERROR: Could not read file content. Error: {str(e)} ******\n")
    ...
    def unpack_file_clicked(self):
        ...
        thread = threading.Thread(target=self.unpack_file, args=(input_file, output_folder, backup))
        thread.start()
    ...
    def unpack_file(self, input_file, output_folder, backup=False):
        ...
        try:
            ...
            while True:
                ...
                end_match = re.search(end_pattern.replace('(.*?)', re.escape(file_path)), content[start_pos:])
                if not end_match:
                    self.log_message(f"Warning: No end delimiter found for {file_path}", "WARNING")
            ...
            if not file_blocks:
                ...
                messagebox.showerror("Unpack Error", "No valid file blocks found in the packed file.")
                return
            ...
            for file_path, file_content in file_blocks:
                try:
                    ...
                    if "****** ERROR: Could not read file content" in file_content:
                        self.log_message(f"  Skipping file with read error: {file_path}", "WARNING")
                        error_count += 1
                        continue
                    ...
                    self.log_message(f"  Error unpacking {file_path}: {str(e)}", "ERROR")
                    error_count += 1
            self.log_message(f"\nUnpacking complete!")
            self.log_message(f"Successfully unpacked: {unpacked_count} files")
            if error_count > 0:
                self.log_message(f"Errors encountered: {error_count} files", "WARNING")
            messagebox.showinfo("Unpacking Complete", 
...
def main():
    ...
    try:
        ...
        ctypes.windll.shell32.SetCurrentProcessExplicitAppUserModelID("FolderPacker.Tool.v1.1")
    except Exception:
