...
% Compatible with MATLAB and GNU Octave.
    ...
    [pass_count, fail_count] = test_reference_frame_operations(pass_count, fail_count);
...
function [pass_count, fail_count] = test_reference_frame_operations(pass_count, fail_count)
    fprintf("Running reference-frame operation tests...\n");

    % Twist frame conversion
    out_twist = rotation_converter.reference_frame_conversion( ...
        'twist_frame_conversion', ...
        'transform', eye(4), ...
        'twist', [0 0 1 0.5 0 0]);
    [pass_count, fail_count] = check_true( ...
        max(abs(out_twist.results.output_twist - [0 0 1 0.5 0 0])) < 1e-12, ...
        "reference-frame twist identity conversion", pass_count, fail_count);

    % Homogeneous transform builder
    out_tf = rotation_converter.reference_frame_conversion( ...
        'homogeneous_transform', ...
        'rotation_matrix', eye(3), ...
        'translation', [1 -2 3]);
    T = out_tf.results.homogeneous_transform;
    T_inv = out_tf.results.inverse_transform;
    [pass_count, fail_count] = check_true( ...
        max(abs(T * T_inv - eye(4)), [], "all") < 1e-12, ...
        "reference-frame homogeneous inverse", pass_count, fail_count);

    % so(3) <-> SO(3)
    out_so3 = rotation_converter.reference_frame_conversion( ...
        'so3_so3_maps', ...
        'so3_vector', [0.1 -0.2 0.3]);
    [pass_count, fail_count] = check_true( ...
        max(abs(out_so3.results.so3_vee_vector - [0.1 -0.2 0.3])) < 1e-12, ...
        "reference-frame hat/vee roundtrip", pass_count, fail_count);
end
