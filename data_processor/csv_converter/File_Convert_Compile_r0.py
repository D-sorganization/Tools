...
class ColumnSelectionDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        self.column_list.itemChanged.connect(self._on_item_changed)  # Connect the signal
...
class ConversionWorker(QThread):
    ...
    def __init__(self, csv_files: List[str], output_path: str, 
                 combine_files: bool = True, chunk_size: int = 10000, 
                 auto_convert: bool = True, selected_columns: Set[str] = None):
        super().__init__()
            ...
            try:
                ...
                for chunk in chunk_iter:
                    ...
                    if self.selected_columns:
                        ...
                        columns_to_keep = list(available_columns.intersection(self.selected_columns))
                        if columns_to_keep:
                            chunk = chunk[columns_to_keep]
                        else:
                            self.error_occurred.emit(f"No selected columns found in {Path(csv_file).name}")
                            continue
            ...
            try:
                ...
                if self.selected_columns:
                    ...
                    if columns_to_keep:
                        ...
                        self.error_occurred.emit(f"No selected columns found in {csv_file}")
                        continue
...
class CSVToParquetConverter(QWidget):
    ...
    def _create_top_section(self):
        ...
        self.select_all_files_checkbox.toggled.connect(self._on_select_all_files_toggled)
        ...
        self.use_all_columns_checkbox = QCheckBox("Use all columns (skip column selection)")
    ...
    def _select_columns(self):
        ...
        try:
            ...
            if self.selected_columns:
                self.column_count_label.setText(f"{len(self.selected_columns)} columns selected")
                self.column_count_label.setStyleSheet("color: green; font-weight: bold;")
                self.log_text.append(f"Selected {len(self.selected_columns)} columns for conversion")
            else:
    ...
    def _execute_conversion(self):
        ...
        self.worker_thread = ConversionWorker(
            ...
            selected_columns=self.selected_columns if not self.use_all_columns_checkbox.isChecked() else None
        )
...
class MainWindow(QMainWindow):
    ...
    def _show_about(self):
        """Show about dialog."""
        QMessageBox.about(
            self, "About CSV to Parquet Converter - Enhanced",
