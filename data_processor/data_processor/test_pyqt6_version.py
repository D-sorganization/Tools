...
def test_file_format_detection():
    ...
    for expected_format, file_path in test_files.items():
        ...
        if detected_format == expected_format:
            ...
            print(f"✗ {expected_format.upper()} format detection: FAILED (detected: {detected_format})")
...
def test_data_reading_writing():
    ...
    for format_type in formats_to_test:
        try:
            ...
            # Compare
            if len(test_data) == len(read_data) and len(test_data.columns) == len(read_data.columns):
...
def test_application_creation():
    ...
    try:
        ...
        if window is not None:
            ...
            # Check tab names
            expected_tabs = ["Processing", "Plotting & Analysis", "Plots List", 
                           "Format Converter", "Folder Tool", "DAT File Import", "Help"]
            ...
            for i, expected_tab in enumerate(expected_tabs):
                if i < window.tab_widget.count():
                    ...
                    if actual_tab == expected_tab:
                        ...
                        print(f"  - Tab {i+1}: {actual_tab} (expected: {expected_tab}) ✗")
                else:
...
def test_thread_classes():
    ...
    from Data_Processor_PyQt6 import ProcessingThread, ConversionThread, FolderProcessingThread
