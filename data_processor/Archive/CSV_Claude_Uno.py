...
class EnhancedPlantDataProcessor(ctk.CTk):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if PARQUET_AVAILABLE:
            self.export_formats.extend(["Parquet (Separate Files)", "Parquet (Compiled)"])
        if FEATHER_AVAILABLE:
            self.export_formats.extend(["Feather (Separate Files)", "Feather (Compiled)"])
        if HDF5_AVAILABLE:
        ...
        self.create_time_range_extractor_tab(self.main_tab_view.tab("Time Range Extractor"))
        ...
        self.status_label.configure(text="Ready. Enhanced processor with ML format support loaded.")
    ...
    def create_format_converter_tab(self, parent_tab):
        ...
        ctk.CTkLabel(control_frame, text="Format Converter", font=ctk.CTkFont(size=18, weight="bold")).grid(row=0, column=0, columnspan=3, padx=10, pady=10)
        
        # Input format selection
        ctk.CTkLabel(control_frame, text="Input Format:").grid(row=1, column=0, padx=10, pady=5, sticky="w")
        self.input_format_var = ctk.StringVar(value="CSV")
        input_formats = ["CSV", "Excel", "Parquet", "Feather", "HDF5", "MAT"]
        self.input_format_menu = ctk.CTkOptionMenu(control_frame, variable=self.input_format_var, values=input_formats)
        self.input_format_menu.grid(row=1, column=1, padx=10, pady=5, sticky="ew")
        
        # Output format selection
        ctk.CTkLabel(control_frame, text="Output Format:").grid(row=2, column=0, padx=10, pady=5, sticky="w")
        self.output_format_var = ctk.StringVar(value="Parquet")
        output_formats = ["CSV", "Excel", "Parquet", "Feather", "HDF5", "JSON"]
        self.output_format_menu = ctk.CTkOptionMenu(control_frame, variable=self.output_format_var, values=output_formats)
        ...
        ctk.CTkButton(button_frame, text="Select Files", command=self.select_conversion_files).pack(pady=2)
        ctk.CTkButton(button_frame, text="Convert", command=self.convert_formats).pack(pady=2)
        ctk.CTkButton(button_frame, text="Help", command=self._show_converter_help).pack(pady=2)
        ...
        # File list
        self.conversion_file_list = ctk.CTkScrollableFrame(main_frame, label_text="Files to Convert")
        ...
        ctk.CTkLabel(options_frame, text="Conversion Options", font=ctk.CTkFont(weight="bold")).pack(padx=10, pady=10)
        
        # Compression options
        compression_frame = ctk.CTkFrame(options_frame)
        compression_frame.pack(fill="x", padx=10, pady=5)
        ctk.CTkLabel(compression_frame, text="Compression:").pack(anchor="w", padx=10, pady=5)
        self.compression_var = ctk.StringVar(value="snappy")
        ctk.CTkOptionMenu(compression_frame, variable=self.compression_var, 
                         values=["none", "snappy", "gzip", "brotli"]).pack(fill="x", padx=10, pady=5)
        ...
        ctk.CTkCheckBox(batch_frame, text="Combine all files into single output", 
                       variable=self.combine_files_var).pack(anchor="w", padx=10, pady=5)
        ...
        ctk.CTkCheckBox(memory_frame, text="Use chunk processing for large files", 
                       variable=self.chunk_processing_var).pack(anchor="w", padx=10, pady=5)
    ...
    def create_time_range_extractor_tab(self, parent_tab):
        ...
        ctk.CTkLabel(control_frame, text="Time Range Extractor", font=ctk.CTkFont(size=18, weight="bold")).grid(row=0, column=0, columnspan=3, padx=10, pady=10)
        
        ctk.CTkButton(control_frame, text="Select Files", command=self.select_extraction_files).grid(row=1, column=0, padx=10, pady=5)
        ctk.CTkButton(control_frame, text="Extract Ranges", command=self.extract_time_ranges).grid(row=1, column=1, padx=10, pady=5)
        ctk.CTkButton(control_frame, text="Help", width=70, command=self._show_extractor_help).grid(row=1, column=2, padx=10, pady=5)
        ...
        # File list
        self.extraction_file_list = ctk.CTkScrollableFrame(main_frame, label_text="Files for Time Range Extraction")
        ...
        ctk.CTkLabel(range_config_frame, text="Time Range Configuration", font=ctk.CTkFont(weight="bold")).pack(padx=10, pady=10)
        
        # Date range
        date_frame = ctk.CTkFrame(range_config_frame)
        date_frame.pack(fill="x", padx=10, pady=5)
        ctk.CTkLabel(date_frame, text="Start Date (YYYY-MM-DD):").pack(anchor="w", padx=10, pady=2)
        ...
        ctk.CTkLabel(date_frame, text="End Date (YYYY-MM-DD):").pack(anchor="w", padx=10, pady=2)
        ...
        ctk.CTkLabel(time_frame, text="Start Time (HH:MM:SS):").pack(anchor="w", padx=10, pady=2)
        self.start_time_entry = ctk.CTkEntry(time_frame, placeholder_text="00:00:00")
        self.start_time_entry.pack(fill="x", padx=10, pady=2)
        
        ctk.CTkLabel(time_frame, text="End Time (HH:MM:SS):").pack(anchor="w", padx=10, pady=2)
        ...
        ctk.CTkLabel(output_frame, text="Output Format:").pack(anchor="w", padx=10, pady=2)
        self.extraction_format_var = ctk.StringVar(value="CSV")
        ctk.CTkOptionMenu(output_frame, variable=self.extraction_format_var, 
                         values=["CSV", "Parquet", "Feather", "HDF5", "Excel"]).pack(fill="x", padx=10, pady=2)
        ...
        ctk.CTkCheckBox(resample_frame, text="Resample data", variable=self.resample_extraction_var).pack(anchor="w", padx=10, pady=2)
        self.resample_frequency_entry = ctk.CTkEntry(resample_frame, placeholder_text="10S")
    ...
    def create_plotting_tab(self, parent_tab):
        ...
        plot_left_panel = ctk.CTkScrollableFrame(plot_main_frame, label_text="Plotting Controls", width=350)
        ...
        self.plot_signal_frame = ctk.CTkScrollableFrame(signal_frame, label_text="Signals to Plot", height=200)
    ...
    def create_dat_import_tab(self, parent_tab):
        ...
        ctk.CTkLabel(header_frame, text="DAT File Import & Processing", 
                    font=ctk.CTkFont(size=18, weight="bold")).pack(side="left")
        ctk.CTkButton(header_frame, text="Help", width=70, command=self._show_dat_help).pack(side="right")
        ...
        ctk.CTkButton(file_frame, text="Select Tag File", command=self._select_tag_file).grid(row=1, column=0, padx=10, pady=5)
        ...
        ctk.CTkButton(file_frame, text="Preview", command=self._preview_tag_file).grid(row=1, column=2, padx=10, pady=5)

        ctk.CTkLabel(file_frame, text="Step 2: Select Data File", font=ctk.CTkFont(weight="bold")).grid(row=2, column=0, columnspan=3, padx=10, pady=(10,5), sticky="w")
        ctk.CTkButton(file_frame, text="Select Data File", command=self._select_dat_file).grid(row=3, column=0, padx=10, pady=5)
        self.dat_file_label = ctk.CTkLabel(file_frame, text="No file selected", anchor="w")
        self.dat_file_label.grid(row=3, column=1, columnspan=2, padx=10, pady=5, sticky="ew")

        # Timestamp configuration
        timestamp_frame = ctk.CTkFrame(main_frame)
        timestamp_frame.pack(fill="x", padx=10, pady=10)
        
        ctk.CTkLabel(timestamp_frame, text="Step 3: Configure Timestamps", font=ctk.CTkFont(weight="bold")).pack(anchor="w", padx=10, pady=5)
        ...
        self.sample_unit_menu = ctk.CTkOptionMenu(sample_frame, values=["seconds", "minutes", "hours"])
        ...
        ctk.CTkLabel(manual_frame, text="Start Time (if not in filename):").pack(side="left")
        self.manual_start_time = ctk.CTkEntry(manual_frame, placeholder_text="YYYY-MM-DD HH:MM:SS")
        self.manual_start_time.pack(side="left", fill="x", expand=True, padx=5)

        # Tag selection
        self.dat_tags_frame = ctk.CTkScrollableFrame(main_frame, label_text="Step 4: Select Tags", height=200)
        ...
        ctk.CTkLabel(processing_frame, text="Step 5: Processing Options", font=ctk.CTkFont(weight="bold")).pack(anchor="w", padx=10, pady=5)
        ...
        self.dat_reduction_entry = ctk.CTkEntry(options_inner, width=100, placeholder_text="1")
        self.dat_reduction_entry.pack(side="left", padx=5)
        
        ctk.CTkLabel(options_inner, text="Output Format:").pack(side="left", padx=(20,5))
        self.dat_output_format = ctk.StringVar(value="CSV")
        ctk.CTkOptionMenu(options_inner, variable=self.dat_output_format, 
                         values=["CSV", "Parquet", "Feather", "HDF5"]).pack(side="left")

        # Convert button
        self.convert_dat_button = ctk.CTkButton(main_frame, text="Step 6: Convert and Load", 
                                               height=40, command=self._run_dat_conversion)
    ...
    def create_status_bar(self):
        ...
        self.progressbar = ctk.CTkProgressBar(status_frame, orientation="horizontal", width=200)
    ...
    def populate_setup_sub_tab(self, tab):
        ...
        ctk.CTkLabel(file_frame, text="File Selection", font=ctk.CTkFont(weight="bold")).grid(row=0, column=0, padx=10, pady=(10, 5), sticky="w")
        ctk.CTkButton(file_frame, text="Select Input Files", command=self.select_files).grid(row=1, column=0, padx=10, pady=5, sticky="ew")
        ...
        self.output_label = ctk.CTkLabel(file_frame, text=f"Output: {self.output_directory}", wraplength=300, justify="left")
        ...
        self.export_type_menu = ctk.CTkOptionMenu(export_frame, variable=self.export_type_var, values=self.export_formats)
    ...
    def populate_processing_sub_tab(self, tab):
        ...
        ctk.CTkOptionMenu(filter_frame, variable=self.filter_type_var, values=self.filter_names).grid(row=1, column=1, padx=10, pady=5, sticky="ew")

    def populate_custom_var_sub_tab(self, tab):
        """Populates the custom variables sub-tab."""
        tab.grid_columnconfigure(0, weight=1)
        
        ctk.CTkLabel(tab, text="Custom Variables", font=ctk.CTkFont(weight="bold")).grid(row=0, column=0, padx=10, pady=10, sticky="w")
        ...
        ctk.CTkLabel(var_frame, text="Variable Name:").grid(row=0, column=0, padx=10, pady=5, sticky="w")
        self.custom_var_name_entry = ctk.CTkEntry(var_frame)
        self.custom_var_name_entry.grid(row=0, column=1, padx=10, pady=5, sticky="ew")
        
        ctk.CTkLabel(var_frame, text="Formula:").grid(row=1, column=0, padx=10, pady=5, sticky="w")
        self.custom_var_formula_entry = ctk.CTkEntry(var_frame)
        self.custom_var_formula_entry.grid(row=1, column=1, padx=10, pady=5, sticky="ew")
        
        ctk.CTkButton(var_frame, text="Add Variable", command=self._add_custom_variable).grid(row=2, column=0, columnspan=2, padx=10, pady=10)
    ...
    def _update_conversion_file_list(self):
        ...
        for file_path in self.conversion_files:
            ctk.CTkLabel(self.conversion_file_list, text=os.path.basename(file_path)).pack(anchor="w", padx=5, pady=2)
    ...
    def convert_formats(self):
        ...
        try:
            ...
            compression = self.compression_var.get() if self.compression_var.get() != "none" else None
            use_chunks = self.chunk_processing_var.get()
            
            if combine_files:
                self._convert_files_combined(self.conversion_files, input_format, output_format, 
                                           output_dir, compression, use_chunks)
            else:
                self._convert_files_separate(self.conversion_files, input_format, output_format, 
                                           output_dir, compression, use_chunks)
            
            messagebox.showinfo("Success", f"Successfully converted {len(self.conversion_files)} files to {output_format.upper()}")
    ...
    def _convert_files_separate(self, files, input_format, output_format, output_dir, compression, use_chunks):
        ...
        for i, file_path in enumerate(files):
            ...
            output_path = os.path.join(output_dir, f"{base_name}.{self._get_file_extension(output_format)}")
            
            self._save_data_by_format(df, output_path, output_format, compression)

    def _convert_files_combined(self, files, input_format, output_format, output_dir, compression, use_chunks):
        """Convert and combine multiple files into one."""
        combined_data = []
        
        for i, file_path in enumerate(files):
            self.status_label.configure(text=f"Loading {os.path.basename(file_path)}...")
            self.progressbar.set((i + 1) / (len(files) * 2))  # Half progress for loading
            self.update_idletasks()
        ...
        # Save combined data
        output_path = os.path.join(output_dir, f"combined_data.{self._get_file_extension(output_format)}")
    ...
    def _load_data_by_format(self, file_path, format_type, use_chunks=False):
        ...
        if format_type == "csv":
            ...
            if not HDF5_AVAILABLE:
                raise ImportError("HDF5 support not available. Install tables and h5py.")
            ...
            data_dict = {k: v.flatten() for k, v in mat_data.items() if not k.startswith('__')}
    ...
    def _save_data_by_format(self, df, file_path, format_type, compression=None):
        ...
        if format_type == "csv":
            ...
            if not HDF5_AVAILABLE:
                raise ImportError("HDF5 support not available. Install tables and h5py.")
            df.to_hdf(file_path, key='data', mode='w', complib='zlib' if compression else None)
    ...
    def select_extraction_files(self):
        ...
        files = filedialog.askopenfilenames(
            ...
            filetypes=[("All supported", "*.csv;*.parquet;*.feather;*.h5"), 
                      ("CSV files", "*.csv"), ("Parquet files", "*.parquet"), 
                      ("Feather files", "*.feather"), ("HDF5 files", "*.h5")]
        )
    ...
    def _update_extraction_file_list(self):
        ...
        for file_path in self.extraction_files:
            ctk.CTkLabel(self.extraction_file_list, text=os.path.basename(file_path)).pack(anchor="w", padx=5, pady=2)
    ...
    def extract_time_ranges(self):
        ...
        try:
            ...
            output_dir = filedialog.askdirectory(title="Select Output Directory for Extracted Files")
            if not output_dir:
                return
            ...
            for i, file_path in enumerate(self.extraction_files):
                self.status_label.configure(text=f"Extracting from {os.path.basename(file_path)}...")
                ...
                # Extract time range
                df_filtered = df[(df[time_col] >= start_datetime) & (df[time_col] <= end_datetime)]
                ...
                if resample and resample_freq:
                    df_filtered = df_filtered.set_index(time_col).resample(resample_freq).mean().reset_index()
            ...
            messagebox.showinfo("Success", f"Successfully extracted time ranges from {extracted_count} files.")
    ...
    def _run_dat_conversion(self):
        ...
        try:
            ...
            all_tags = [tag.strip() for tag in all_tags if isinstance(tag, str) and tag.strip()]
            
            # Get selected tags
            selected_tags = [tag for tag, data in self.dat_tag_vars.items() if data['var'].get()]
            ...
            # Generate timestamps
            start_time = self._extract_timestamp_from_filename() or self._get_manual_timestamp()
            if not start_time:
                messagebox.showerror("Error", "Could not determine start time. Please enter manually.")
                return
            ...
            save_path = filedialog.asksaveasfilename(
                ...
                filetypes=[(f"{output_format.upper()} files", f"*.{self._get_file_extension(output_format)}")]
            )
            ...
            if save_path:
                ...
                messagebox.showinfo("Success", f"DAT file converted and saved as {output_format.upper()}")
                self.status_label.configure(text=f"DAT conversion completed: {os.path.basename(save_path)}")
    ...
    def _update_file_list_ui(self):
        ...
        if self.input_file_paths:
            ...
            self.status_label.configure(text=f"Loaded {len(self.input_file_paths)} files.")
        else:
            ctk.CTkLabel(self.file_list_frame, text="No files selected.").pack(padx=5, pady=5)
    ...
    def update_plot(self):
        ...
        self.plot_ax.clear()
        self.plot_ax.text(0.5, 0.5, "Plotting functionality would be implemented here", 
    ...
    def _add_custom_variable(self):
        ...
        if not var_name or not formula:
            messagebox.showwarning("Warning", "Please enter both variable name and formula.")
            return
