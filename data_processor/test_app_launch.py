...
try:
    ...
    if has_canvas and has_ax:
        ...
        try:
            app.plot_ax.clear()
            app.plot_ax.text(0.5, 0.5, "Test from external script", ha='center', va='center')
            ...
            if hasattr(app, 'plot_file_menu'):
                ...
                try:
                    ...
                    if hasattr(app, 'plot_signal_vars') and app.plot_signal_vars:
                        selected_count = sum(1 for data in app.plot_signal_vars.values() if data['var'].get())
                        print(f"✓ plot_signal_vars created with {len(app.plot_signal_vars)} signals")
