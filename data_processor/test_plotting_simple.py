...
def test_processed_files_initialization():
    ...
    assert hasattr(app, 'processed_files'), "processed_files attribute should be initialized"
    assert isinstance(app.processed_files, dict), "processed_files should be a dictionary"
...
def test_process_files_stores_data():
    ...
    # Mock signal_vars
    app.signal_vars = {
        'Time': {'var': type('MockVar', (), {'get': lambda *args: True})(),
                'checkbox': type('MockCheckbox', (), {'configure': lambda **kwargs: None})()},
        'Signal1': {'var': type('MockVar', (), {'get': lambda *args: True})(),
                   'checkbox': type('MockCheckbox', (), {'configure': lambda **kwargs: None})()},
        'Signal2': {'var': type('MockVar', (), {'get': lambda *args: True})(),
                   'checkbox': type('MockCheckbox', (), {'configure': lambda **kwargs: None})()}
    }
    ...
    # Mock messagebox
    app.messagebox = type('MockMessagebox', (), {'showinfo': lambda *args, **kwargs: None})()
    ...
    assert isinstance(app.processed_files['test1.csv'], pd.DataFrame), "Stored data should be DataFrame"
    assert isinstance(app.processed_files['test2.csv'], pd.DataFrame), "Stored data should be DataFrame"
...
def test_plot_file_menu_update_logic():
    ...
    assert menu_values == expected_values, f"Expected {expected_values}, got {menu_values}"
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("2. ✓ Modified process_files to store processed data in self.processed_files")
