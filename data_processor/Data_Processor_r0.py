...
class CSVProcessorApp(ctk.CTk):
    ...
    def populate_setup_sub_tab(self, tab):
        ...
        self.signal_list_status_label.grid(row=3, column=0, columnspan=3, padx=10, pady=(5, 10), sticky="w")
    ...
    def _filter_plot_signals(self, event=None):
        ...
        limited_signals = set()
        if hasattr(self, 'limit_plot_signals_var') and self.limit_plot_signals_var.get():
            limited_signals = {s for s, data in self.signal_vars.items() if data['var'].get()}
        ...
        for signal, data in self.plot_signal_vars.items():
            ...
            allowed_signal = not hasattr(self, 'limit_plot_signals_var') or not self.limit_plot_signals_var.get() or signal in limited_signals
...
• The status label below shows current signal list status
            ...
            if bulk_mode and total_files > 1:
                ...
                if first_file_only:
                    ...
                    if total_files > 100:
                        ...
                        self.update_status("Bulk mode: Reading headers from first file only...", show_progress=True, progress_value=0.1, progress_text="Reading file headers...")
                    ...
                    if total_files > 100:
                        ...
                        self.update_status("Bulk mode: Reading headers from sample files...", show_progress=True, progress_value=0.1, progress_text="Reading file headers...")
    ...
    def _update_processing_dependent_signals(self):
        ...
        time_columns = {'local_time', 'utc_time', 'time', 'timestamp', 'date', 'datetime'}
        non_time_signals = [s for s in selected_signals if s.lower() not in time_columns]
    ...
    def _display_signals_batch(self, signals_batch, start_index=0, auto_select=True):
        ...
        print(f"DEBUG: Displaying batch of {len(signals_batch)} signals starting at index {start_index}, auto_select={auto_select}")
        ...
        try:
            ...
            for col in processed_df.columns:
                if col != time_col:
                    ...
                    print(f"  Column {col}: {before_numeric} -> {after_numeric} valid values")
            ...
            if trim_date or trim_start or trim_end:
                print(f"  Applying time trimming: date={trim_date}, start={trim_start}, end={trim_end}")
                try:
            ...
            print(f"  SUCCESS: Returning processed DataFrame with shape {processed_df.shape}")
            return processed_df
    ...
    def _export_csv_separate(self, processed_files):
        ...
        if exported_count > 0:
            ...
            messagebox.showinfo("Export Success", f"Exported {exported_count} files to {self.output_directory}")
        else:
    ...
    def _combine_multiple_files(self, processed_files):
        ...
        print(f"Time range: {combined_df[time_col].min()} to {combined_df[time_col].max()}")
    ...
    def create_plotting_tab(self, tab):
        ...
        def create_plot_left_content(left_panel):
            ...
            limit_plot_checkbox.grid(row=1, column=0, columnspan=2, sticky="w", padx=5, pady=2)
            
            ctk.CTkButton(plot_signal_select_frame, text="All", command=self._plot_select_all).grid(row=2, column=0, sticky="ew", padx=2, pady=5)
            ctk.CTkButton(plot_signal_select_frame, text="None", command=self._plot_select_none).grid(row=2, column=1, sticky="ew", padx=2, pady=5)
            ctk.CTkButton(plot_signal_select_frame, text="Show Selected", command=self._show_selected_signals).grid(row=2, column=2, sticky="ew", padx=2, pady=5)
            ctk.CTkButton(plot_signal_select_frame, text="X", width=28, command=self._plot_clear_search).grid(row=2, column=3, sticky="w", padx=2, pady=5)
    ...
    def update_status(self, message, show_progress=False, progress_value=0, progress_text=""):
