...
try:
    ...
    selected_signals = [s for s, data in app.plot_signal_vars.items() if data['var'].get()]
...
try:
    ...
    if hasattr(app, 'plot_ax') and len(app.plot_ax.lines) > 0:
        ...
        print("⚠ No plot lines created (this might be expected if no signals were selected)")
...
try:
    ...
    numeric_cols = [col for col in df.columns[1:3] if df[col].dtype in ['float64', 'int64']]
...
critical_methods = [
    'get_data_for_plotting', 'update_plot', 'on_plot_file_select',
    '_ensure_data_loaded', '_apply_plot_filter'
]
...
critical_attributes = [
    'processed_files', 'loaded_data_cache', 'input_file_paths'
]
...
print(f"Signal variables created: {len(app.plot_signal_vars) if hasattr(app, 'plot_signal_vars') else 0}")
