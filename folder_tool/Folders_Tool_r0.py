...
class FolderProcessorApp:
    ...
    def __init__(self, root_window):
        ...
        try:
            ...
            try:
                ...
                ctypes.windll.shell32.SetCurrentProcessExplicitAppUserModelID("FolderFix.Tool.2.0")
            ...
            if os.path.exists(ico_path):
                ...
                if os.path.exists(png_path):
                    ...
                    print("No icon files found (paper_plane_icon.ico or paper_plane_icon.png)")
    ...
    def create_source_widgets(self, parent):
        ...
        source_scrollbar = ttk.Scrollbar(listbox_frame, orient="vertical", command=self.source_listbox.yview)
        ...
        self.source_info_label = ttk.Label(self.source_frame, text="", foreground="blue")
    ...
    def create_filtering_widgets(self, parent):
        filter_frame = ttk.LabelFrame(parent, text="3. File Filtering Options", padding="10")
        filter_frame.pack(fill=tk.X, pady=5)
        
        # File extensions filter
        ext_frame = ttk.Frame(filter_frame)
        ext_frame.pack(fill=tk.X, pady=2)
        ttk.Label(ext_frame, text="Include only extensions (comma-separated):").pack(side=tk.LEFT)
        ttk.Entry(ext_frame, textvariable=self.filter_extensions, width=30).pack(side=tk.RIGHT)
        ttk.Label(filter_frame, text="Example: .jpg,.png,.pdf (leave empty for all files)", 
                 foreground="grey").pack(anchor=tk.W)
        
        # File size filters
        size_frame = ttk.Frame(filter_frame)
        size_frame.pack(fill=tk.X, pady=5)
        ttk.Label(size_frame, text="Min size (MB):").pack(side=tk.LEFT)
        ttk.Entry(size_frame, textvariable=self.min_file_size, width=10).pack(side=tk.LEFT, padx=5)
        ttk.Label(size_frame, text="Max size (MB):").pack(side=tk.LEFT, padx=(10,0))
        ttk.Entry(size_frame, textvariable=self.max_file_size, width=10).pack(side=tk.LEFT, padx=5)

    def create_preprocessing_widgets(self, parent):
        self.pre_process_frame = ttk.LabelFrame(parent, text="4. Pre-processing Options (On Source)", padding="10")
        self.pre_process_frame.pack(fill=tk.X, pady=5)
        
        ttk.Checkbutton(self.pre_process_frame, text="Bulk extract archives (.zip, .rar, .7z)", 
                       variable=self.unzip_var).pack(anchor=tk.W)
        ttk.Checkbutton(self.pre_process_frame, text="Safe extraction (verify before deleting originals)", 
                       variable=self.safe_extract_var).pack(anchor=tk.W, padx=(20,0))

    def create_main_operation_widgets(self, parent):
        self.mode_frame = ttk.LabelFrame(parent, text="5. Choose Main Operation", padding="10")
        self.mode_frame.pack(fill=tk.X, pady=5)
        
        ttk.Radiobutton(self.mode_frame, text="Combine & Copy", variable=self.operation_mode, 
                       value="combine", command=self.on_mode_change).pack(anchor=tk.W)
        ttk.Radiobutton(self.mode_frame, text="Flatten & Tidy", variable=self.operation_mode, 
                       value="flatten", command=self.on_mode_change).pack(anchor=tk.W)
        ttk.Radiobutton(self.mode_frame, text="Copy & Prune Empty Folders", variable=self.operation_mode, 
                       value="prune", command=self.on_mode_change).pack(anchor=tk.W)
        ttk.Radiobutton(self.mode_frame, text="Deduplicate Files (In-Place)", variable=self.operation_mode, 
                       value="deduplicate", command=self.on_mode_change).pack(anchor=tk.W)
        ttk.Radiobutton(self.mode_frame, text="Analyze & Report Only", variable=self.operation_mode, 
                       value="analyze", command=self.on_mode_change).pack(anchor=tk.W)
        
        self.mode_description = ttk.Label(self.mode_frame, text="", wraplength=600, justify=tk.LEFT)
        self.mode_description.pack(fill=tk.X, pady=(5,0))

    def create_organization_widgets(self, parent):
        org_frame = ttk.LabelFrame(parent, text="6. File Organization Options", padding="10")
        org_frame.pack(fill=tk.X, pady=5)
        
        ttk.Checkbutton(org_frame, text="Organize files by type (create subfolders)", 
                       variable=self.organize_by_type_var).pack(anchor=tk.W)
        ttk.Checkbutton(org_frame, text="Organize files by date (YYYY/MM folders)", 
                       variable=self.organize_by_date_var).pack(anchor=tk.W)

    def create_postprocessing_widgets(self, parent):
        self.post_process_frame = ttk.LabelFrame(parent, text="7. Post-processing Options (On Destination)", padding="10")
        self.post_process_frame.pack(fill=tk.X, pady=5)
        
        ttk.Checkbutton(self.post_process_frame, text="Deduplicate renamed files in destination folder after copy", 
                       variable=self.deduplicate_var).pack(anchor=tk.W)

    def create_output_options_widgets(self, parent):
        output_frame = ttk.LabelFrame(parent, text="8. Output Options", padding="10")
        output_frame.pack(fill=tk.X, pady=5)
        
        ttk.Checkbutton(output_frame, text="Create ZIP archive of final result", 
                       variable=self.zip_output_var).pack(anchor=tk.W)

    def create_advanced_options_widgets(self, parent):
        advanced_frame = ttk.LabelFrame(parent, text="9. Advanced Options", padding="10")
        advanced_frame.pack(fill=tk.X, pady=5)
        
        ttk.Checkbutton(advanced_frame, text="Preview mode (show what would be done without executing)", 
                       variable=self.preview_mode_var).pack(anchor=tk.W)
        ttk.Checkbutton(advanced_frame, text="Create backup before processing", 
    ...
    def create_run_button(self, parent):
        ...
        self.run_button = ttk.Button(button_frame, text="Run Process", command=self.run_processing_threaded, 
                                   style="Accent.TButton")
        self.run_button.pack(side=tk.LEFT, expand=True, fill=tk.X, padx=(0,5), ipady=10)
    ...
    def on_mode_change(self):
        ...
        frames_to_toggle = [self.dest_frame, self.pre_process_frame, self.post_process_frame]
    ...
    def safe_extract_archive(self, archive_path):
        ...
        try:
            ...
            return True, f"Successfully extracted and deleted '{os.path.basename(archive_path)}'"
    ...
    def create_backup(self):
        ...
        backup_base = os.path.join(os.path.dirname(self.source_folders[0]), 
        ...
        for i, folder in enumerate(self.source_folders):
            ...
            self.update_progress(progress, f"Backing up folder {i+1}/{len(self.source_folders)}")
    ...
    def run_processing(self):
        ...
        if mode == 'analyze':
            ...
            try:
                ...
                if report:
                    ...
                    messagebox.showinfo("Analysis Complete", "Analysis report generated successfully!")
            except Exception as e:
        ...
        if self.unzip_var.get():
            try:
                ...
                if self.cancel_operation:
                    return
                if not messagebox.askyesno("Pre-processing Complete", 
                                         f"Bulk Extraction Complete!\n\n" + "\n".join(unzip_log) + 
                                         "\n\nDo you want to proceed?"):
                    return
            except Exception as e:
                messagebox.showerror("Error", f"An error occurred during bulk unzip: {e}")
                return
        ...
        try:
            ...
            messagebox.showerror("Error", f"An error occurred during the main operation: {e}")
            return
    ...
    def validate_inputs(self, check_destination=True):
        ...
        if check_destination:
            ...
            if any(src == self.dest_folder for src in self.source_folders):
                messagebox.showerror("Error", "The destination folder cannot be a source folder.")
                return False
        return True
    ...
    def _bulk_unzip_enhanced(self):
        ...
        for i, archive_path in enumerate(archives):
            ...
            self.update_progress(20 + (i / len(archives)) * 10, 
            ...
            for root, dirs, files in os.walk(src):
                for file in files:
                    ...
                    if final_dest_path != dest_path:
                        log.append(f"Renamed: '{file}' to '{os.path.basename(final_dest_path)}'")
                        renamed_count += 1
                    ...
                    if processed_files % 10 == 0:  # Update progress every 10 files
                        ...
                        self.update_progress(progress, f"Processed {processed_files}/{total_files} files")
                    ...
                    for file_path in files:
                        if file_path != file_to_keep:
                            try:
                                if not self.preview_mode_var.get():
                                    os.remove(file_path)
                                log.append(f"  - {'WOULD DELETE' if self.preview_mode_var.get() else 'DELETED'}: '{os.path.basename(file_path)}'")
                                deleted_count += 1
                            except OSError as e:
                                log.append(f"  - FAILED to delete '{os.path.basename(file_path)}': {e}")
