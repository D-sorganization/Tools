...
## 2025-01-31 - Avoid spread operator with Math.min/Math.max on large arrays
**Learning:** In JavaScript/TypeScript, using `Math.min(...values)` or `Math.max(...values)` on large arrays (like chart data points) is significantly slower and can throw a 'Maximum call stack size exceeded' RangeError due to intermediate memory allocation.
**Action:** Use a single-pass `for` loop to compute the min and max for large arrays to improve performance and prevent stack overflows.
