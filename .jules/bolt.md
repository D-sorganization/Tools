...
## 2024-06-25 - Avoid spreading large arrays into Math.min/Math.max
**Learning:** When calculating min/max over large datasets in React components (like chart axes), using `Math.min(...activeValues)` can cause "Maximum call stack size exceeded" errors if the array is too large. Furthermore, combining this with `.flatMap` and `.map` creates unnecessary intermediate arrays and causes excessive garbage collection.
**Action:** Replace `Math.min(...array)` and chained array iterators with explicit, single-pass `for` loops wrapped in `useMemo` to safely and quickly calculate min/max over large datasets without exceeding the call stack or creating intermediate arrays.
