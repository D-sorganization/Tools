...
## 2024-06-14 - Pytest-qt in CI
**Learning:** Running PyQt tests in CI or headless environments using `pytest-qt` fails with a fatal abort if a display server is not available.
**Action:** Always set the environment variable `QT_QPA_PLATFORM=offscreen` when running PyQt6 tests.
