...
## 2026-08-04 - Avoid rust-toolchain curl failures
**Learning:** Installing `dtolnay/rust-toolchain` can fail with `curl: (56) Recv failure: Connection reset by peer` due to spurious network errors during heavy toolchain downloads on local runners.
**Action:** Add `CARGO_NET_RETRY: "10"` and `CARGO_NET_TIMEOUT: "120"` to the workflow `env` to improve resilience against transient network resets during cargo operations.
