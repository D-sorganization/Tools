...
## 2025-05-23 - [Calculator Loading State]
**Learning:** High-latency actions (like CAS calculations) without button feedback can lead to user confusion and double-submission. Users expect immediate visual acknowledgment (text change or spinner) even for short delays.
**Action:** Always disable the primary action button and update its label (e.g., "Processing...") during async operations, then restore state and focus in a `finally` block to ensure recovery even on error.
