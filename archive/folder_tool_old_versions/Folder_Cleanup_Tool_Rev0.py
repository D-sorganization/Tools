...
class FolderProcessorApp:
    ...
    def create_main_operation_widgets(self, parent):
        ...
        self.mode_description = ttk.Label(
            self.mode_frame, text="", wraplength=500, justify=tk.LEFT
        )
    ...
    def create_postprocessing_widgets(self, parent):
        self.post_process_frame = ttk.LabelFrame(
            parent, text="5. Post-processing Options (On Destination)", padding="10"
        )
    ...
    def create_run_button(self, parent):
        ...
        ).pack(fill=tk.X, ipady=10, pady=(10, 5))
        style = ttk.Style()
    ...
    def on_mode_change(self):
        ...
        if mode == "combine":
            self.mode_description.config(
                text="Copies all files from source folders into the single destination folder."
            )
        elif mode == "flatten":
            self.mode_description.config(
                text="Finds deeply nested folders and copies them to the top level of the destination."
            )
        elif mode == "prune":
            self.mode_description.config(
                text="Copies source folders to the destination, preserving structure but skipping empty sub-folders."
            )
        elif mode == "deduplicate":
            self.mode_description.config(
                text="Deletes renamed duplicates like 'file (1).txt' within the source folder(s), keeping the newest version."
            )
    ...
    def run_processing(self):
        ...
        try:
            main_op_log = []
            if mode == "combine":
                main_op_log = self._combine_folders()
            elif mode == "flatten":
    ...
    def _perform_deduplication(self, target_folder):
        ...
        summary = [
            ...
            f"Deleted a total of {deleted_count} files.",
        ] + log[:20]
        if len(log) > 20:
    ...
    def _bulk_unzip(self):
        ...
        for archive_path in archives:
            ...
            try:
                ...
                log.append(f"Extracted and deleted '{os.path.basename(archive_path)}'")
                extracted_count += 1
            except Exception as e:
                log.append(f"FAILED to extract '{os.path.basename(archive_path)}': {e}")
        return [
            f"Processed {len(archives)} archive(s). Successfully extracted: {extracted_count}"
        ] + log[1:]

    def _combine_folders(self):
        log, file_count, renamed_count = [], 0, 0
        os.makedirs(self.dest_folder, exist_ok=True)
        for src in self.source_folders:
            for dp, _, fn in os.walk(src):
                for f in fn:
                    sp, dp = os.path.join(dp, f), os.path.join(self.dest_folder, f)
                    fdp = self._get_unique_path(dp)
                    if fdp != dp:
                        log.append(f"Renamed: '{f}' to '{os.path.basename(fdp)}'")
                        renamed_count += 1
                    try:
                        shutil.copy2(sp, fdp)
                        file_count += 1
    ...
    def _flatten_folders(self):
        ...
        for src in self.source_folders:
            ...
            while True:
                try:
                    children = os.listdir(cp)
                except FileNotFoundError:
                    cp = None
                    break
                if any(os.path.isfile(os.path.join(cp, c)) for c in children):
                    break
                subdirs = [d for d in children if os.path.isdir(os.path.join(cp, d))]
                if len(subdirs) == 1:
                    cp = os.path.join(cp, subdirs[0])
                else:
                    break
            if cp and os.path.exists(cp):
                fdp = self._get_unique_path(
                    os.path.join(self.dest_folder, os.path.basename(cp))
                )
                if fdp != os.path.join(self.dest_folder, os.path.basename(cp)):
                    log.append(
                        f"Renamed folder: '{os.path.basename(cp)}' to '{os.path.basename(fdp)}'"
                    )
                try:
                    shutil.copytree(cp, fdp)
                    log.append(f"Copied '{os.path.basename(cp)}'")
                    moved_count += 1
                except Exception as e:
                    log.append(f"ERROR copying '{os.path.basename(cp)}': {e}")
            else:
                log.append(
                    f"Skipped '{os.path.basename(src)}': No content folder found."
                )
    ...
    def _prune_empty_folders(self):
        log, fc, pf = [], 0, 0
        for src in self.source_folders:
            dbp = self._get_unique_path(
                os.path.join(self.dest_folder, os.path.basename(src))
            )
            if dbp != os.path.join(self.dest_folder, os.path.basename(src)):
                log.append(
                    f"Renamed base folder: '{os.path.basename(src)}' to '{os.path.basename(dbp)}'"
                )
            if not any(fn for _, _, fn in os.walk(src)):
                log.append(f"Skipped '{os.path.basename(src)}' as it's empty.")
                continue
            pf += 1
            for dp, _, fn in os.walk(src):
                if not fn:
                ...
                for f in fn:
                    ...
                    dfp, ddff = os.path.join(dbp, rp), os.path.dirname(
                        os.path.join(dbp, rp)
                    )
                    try:
                        os.makedirs(ddff, exist_ok=True)
                        shutil.copy2(sp, dfp)
                        fc += 1
                    except Exception as e:
                        log.append(f"ERROR copying '{f}': {e}")
