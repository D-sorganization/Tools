...
class CSVProcessorApp(ctk.CTk):
            ...
            if signal in df.columns and signal != time_col:
                ...
                if method == "Trapezoidal":
                    ...
                    cumulative = np.cumsum(
                        0.5
                        * (signal_data.iloc[:-1].values + signal_data.iloc[1:].values)
                        * dt.iloc[1:].values
                    )
                    ...
                    if len(signal_data) % 2 == 0:
                        cumulative = np.cumsum(
                            (
                                signal_data.iloc[::2].values
                                + 4 * signal_data.iloc[1::2].values
                                + signal_data.iloc[2::2].values
                            )
                            * dt.iloc[::2].values
                            / 3
                        )
    ...
    def process_files(self):
        ...
        settings = {
            ...
            "hampel_window": (
                int(self.hampel_window_entry.get())
                if self.hampel_window_entry.get()
                else 7
            ),
            "hampel_threshold": (
                float(self.hampel_threshold_entry.get())
                if self.hampel_threshold_entry.get()
                else 3.0
            ),
            "zscore_threshold": (
                float(self.zscore_threshold_entry.get())
                if self.zscore_threshold_entry.get()
                else 3.0
            ),
            "zscore_method": (
        ...
        try:
            ...
            if trim_date or trim_start or trim_end:
                try:
                    ...
                    if processed_df.empty:
                        print(
                            f"Warning: Time trimming resulted in empty dataset for {os.path.basename(file_path)}"
                        )
                        return None

                except Exception as e:
                    print(
                        f"Error applying time trimming to {os.path.basename(file_path)}: {e}"
                    )
                ...
                for col in numeric_cols:
                    ...
                    if filter_type == "Moving Average":
                        ...
                        try:
                            ...
                            processed_df.loc[outliers, signal] = median_filtered.loc[
                                outliers
                            ]
                        except ImportError:
    ...
    def _export_csv_separate(self, processed_files):
        ...
        if exported_count > 0:
            messagebox.showinfo(
                "Success", f"Exported {exported_count} files to {self.output_directory}"
            )
        else:
    ...
    def _export_csv_compiled(self, processed_files):
        ...
        if compiled_dfs:
            ...
            output_path = os.path.join(
                self.output_directory, "compiled_processed_data.csv"
            )
    ...
    def _export_mat_compiled(self, processed_files):
        ...
        if compiled_dfs:
            ...
            output_path = os.path.join(
                self.output_directory, "compiled_processed_data.mat"
            )
    ...
    def create_plotting_tab(self, tab):
        ...
        def create_plot_left_content(left_panel):
            ...
            self.plot_start_time_entry = ctk.CTkEntry(
                time_range_frame, placeholder_text="e.g., 09:30:00"
            )
            self.plot_start_time_entry.grid(
            ...
            self.plot_end_time_entry = ctk.CTkEntry(
        ...
        if not signals_to_plot:
            ...
            for i, signal in enumerate(signals_to_plot):
                ...
                if show_both and plot_filter != "None":
                    ...
                    raw_label = (
                        f"{self.custom_legend_entries.get(signal, signal)} (Raw)"
                    )
                    self.plot_ax.plot(
                        plot_df[x_axis_col],
                        plot_df[signal],
                    ...
                    filtered_label = (
                        f"{self.custom_legend_entries.get(signal, signal)} (Filtered)"
                    )
                    self.plot_ax.plot(
    ...
    def _add_trendline(self, df, signal, x_axis_col):
        ...
        try:
            if trend_type == "Linear":
                ...
                trend = np.poly1d(coeffs)
                trendline = trend(x_numeric)
                equation = f"Polynomial (order {order}): " + " + ".join(
                    [f"{coeffs[i]:.4f}x^{order-i}" for i in range(order + 1)]
                )
...
• Close all braces properly
    ...
    def save_signal_list(self):
        ...
        try:
            ...
            if file_path:
                ...
                messagebox.showinfo(
                    "Success",
                    f"Signal list '{signal_list_name}' saved successfully!\n\nSaved signals: {len(selected_signals)}",
                )
                self.status_label.configure(
    ...
    def load_signal_list(self):
        ...
        try:
            ...
            messagebox.showinfo(
                "Success",
                f"Signal list '{self.saved_signal_list_name}' loaded and applied successfully!\n\nSignals: {len(self.saved_signal_list)}",
            )
            self.status_label.configure(
...
For additional support or feature requests, please refer to the application documentation or contact the development team.
    ...
    def _save_current_plot_config(self):
        ...
        plot_config = {
            ...
            "start_time": (
                self.plot_start_time_entry.get()
                if hasattr(self, "plot_start_time_entry")
                else ""
            ),
            "end_time": (
                self.plot_end_time_entry.get()
                if hasattr(self, "plot_end_time_entry")
                else ""
            ),
            "color_scheme": (
    ...
    def _apply_plot_config(self, plot_config):
        ...
        if "trendline_signal" in plot_config and hasattr(self, "trendline_signal_var"):
            self.trendline_signal_var.set(
                plot_config.get("trendline_signal", "Select signal...")
            )
    ...
    def _generate_plot_preview(self):
        ...
        try:
            ...
            if df is None:
                for file_path, data in self.processed_files.items():
                    if (
                        file_name in os.path.basename(file_path)
                        or os.path.basename(file_path) in file_name
                    ):
                        df = data
                        break
    ...
    def _on_plot_setting_change(self, *args):
        ...
        if hasattr(self, "plot_signal_vars") and any(
            data["var"].get() for data in self.plot_signal_vars.values()
        ):
