        ...
        if filter_config.get("enabled", False):
            ...
            for col in df.select_dtypes(include=[np.number]).columns:
                if filter_type == "Moving Average":
                    ...
                    df[col] = pd.Series(
                        medfilt(df[col].fillna(0), kernel_size=kernel_size)
                    )
                    ...
                    try:
                        ...
                        df[col] = pd.Series(
                            filtfilt(b, a, df[col].fillna(method="ffill"))
                        )
                    except:
                        pass
                    ...
                    try:
                        df[col] = pd.Series(
                            savgol_filter(
                                df[col].fillna(method="ffill"), window, poly_order
                            )
                        )
                    except:
                        pass
                                ...
                                try:
                                    if method == "Spline (Acausal)":
                                        spline = UnivariateSpline(
                                            time_numeric, df[signal], s=0, k=3
                                        )
                                    ...
                                    print(
                                        f"Derivative error for {signal}, order {order}: {e}"
                                    )
...
class CSVProcessorUnifiedApp(ctk.CTk):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.layout_config_file = os.path.join(
            os.path.expanduser("~"), ".csv_processor_unified_layout.json"
        )
        ...
        self.status_label.configure(
            text="Unified CSV Processor Ready - All features available"
        )
    ...
    def create_setup_and_process_tab(self, tab):
        ...
        self.compilation_checkbox = ctk.CTkCheckBox(
            compilation_frame,
            text="Compile multiple files into single output",
            variable=self.compilation_enabled,
        )
        self.compilation_checkbox.grid(row=1, column=0, columnspan=2, padx=10, pady=5)

        ctk.CTkLabel(compilation_frame, text="Merge Method:").grid(
            row=2, column=0, padx=10, pady=5, sticky="w"
        )
        self.merge_method_var = ctk.StringVar(value="Concatenate")
        merge_help_button = ctk.CTkButton(
            compilation_frame,
            text="❓",
            command=self._show_merge_methods_help,
            width=30,
        )
        ...
        merge_menu = ctk.CTkOptionMenu(
            compilation_frame,
            variable=self.merge_method_var,
        ...
        ctk.CTkLabel(compilation_frame, text="Time Range Trimming:").grid(
            row=4, column=0, padx=10, pady=5, sticky="w"
        )
        self.time_filtering_checkbox = ctk.CTkCheckBox(
            compilation_frame,
            text="Enable time range filtering",
            variable=self.time_filtering_enabled,
        )
        self.time_filtering_checkbox.grid(
            row=5, column=0, columnspan=2, padx=10, pady=5
        )

        ctk.CTkLabel(compilation_frame, text="Time Column:").grid(
            row=6, column=0, padx=10, pady=5, sticky="w"
        )
        self.time_column_var = ctk.StringVar(value="Time")
        self.time_column_entry = ctk.CTkEntry(
            compilation_frame, textvariable=self.time_column_var
        )
        self.time_column_entry.grid(row=6, column=1, padx=10, pady=5, sticky="ew")

        ctk.CTkLabel(compilation_frame, text="Start Time:").grid(
            row=7, column=0, padx=10, pady=5, sticky="w"
        )
        self.start_time_entry = ctk.CTkEntry(
            compilation_frame, placeholder_text="YYYY-MM-DD HH:MM:SS"
        )
        self.start_time_entry.grid(row=7, column=1, padx=10, pady=5, sticky="ew")

        ctk.CTkLabel(compilation_frame, text="End Time:").grid(
        ...
        ctk.CTkLabel(
            processing_frame,
            text="Advanced Processing Options",
            font=ctk.CTkFont(weight="bold"),
        ).grid(row=0, column=0, columnspan=2, padx=10, pady=(10, 5), sticky="w")

        # Filtering options
        ctk.CTkLabel(processing_frame, text="Signal Filtering:").grid(
            row=1, column=0, padx=10, pady=5, sticky="w"
        )
        self.filter_enabled_var = tk.BooleanVar(value=False)
        self.filter_checkbox = ctk.CTkCheckBox(
            processing_frame, text="Enable filtering", variable=self.filter_enabled_var
        )
        self.filter_checkbox.grid(row=1, column=1, padx=10, pady=5, sticky="w")

        ctk.CTkLabel(processing_frame, text="Filter Type:").grid(
            row=2, column=0, padx=10, pady=5, sticky="w"
        )
        self.filter_type_var = ctk.StringVar(value="None")
        self.filter_menu = ctk.CTkOptionMenu(
            processing_frame, variable=self.filter_type_var, values=self.filter_names
        )
        self.filter_menu.grid(row=2, column=1, padx=10, pady=5, sticky="ew")

        ctk.CTkLabel(processing_frame, text="Filter Window:").grid(
        ...
        # Integration options
        ctk.CTkLabel(processing_frame, text="Integration:").grid(
            row=4, column=0, padx=10, pady=5, sticky="w"
        )
        self.integration_enabled_var = tk.BooleanVar(value=False)
        self.integration_checkbox = ctk.CTkCheckBox(
            processing_frame,
            text="Enable integration",
            variable=self.integration_enabled_var,
        )
        self.integration_checkbox.grid(row=4, column=1, padx=10, pady=5, sticky="w")

        ctk.CTkLabel(processing_frame, text="Integration Method:").grid(
            row=5, column=0, padx=10, pady=5, sticky="w"
        )
        self.integration_method_var = ctk.StringVar(value="Trapezoidal")
        self.integration_menu = ctk.CTkOptionMenu(
            processing_frame,
            variable=self.integration_method_var,
            values=["Trapezoidal", "Simpson's", "Cumulative Sum"],
        )
        self.integration_menu.grid(row=5, column=1, padx=10, pady=5, sticky="ew")

        # Differentiation options
        ctk.CTkLabel(processing_frame, text="Differentiation:").grid(
            row=6, column=0, padx=10, pady=5, sticky="w"
        )
        self.differentiation_enabled_var = tk.BooleanVar(value=False)
        self.differentiation_checkbox = ctk.CTkCheckBox(
            processing_frame,
            text="Enable differentiation",
            variable=self.differentiation_enabled_var,
        )
        self.differentiation_checkbox.grid(row=6, column=1, padx=10, pady=5, sticky="w")

        ctk.CTkLabel(processing_frame, text="Differentiation Method:").grid(
            row=7, column=0, padx=10, pady=5, sticky="w"
        )
        self.differentiation_method_var = ctk.StringVar(value="Spline (Acausal)")
        self.differentiation_menu = ctk.CTkOptionMenu(
            processing_frame,
            variable=self.differentiation_method_var,
        ...
        deriv_orders_frame.grid(
            row=8, column=0, columnspan=2, padx=10, pady=5, sticky="ew"
        )

        ctk.CTkLabel(deriv_orders_frame, text="Derivative Orders:").grid(
        ...
        for i in range(1, 5):
            checkbox = ctk.CTkCheckBox(
                ...
                text=(
                    f"{i}st"
                    if i == 1
                    else f"{i}nd" if i == 2 else f"{i}rd" if i == 3 else f"{i}th"
                ),
        ...
        # Resampling options
        ctk.CTkLabel(processing_frame, text="Resampling:").grid(
            row=9, column=0, padx=10, pady=5, sticky="w"
        )
        self.resampling_enabled_var = tk.BooleanVar(value=False)
        self.resampling_checkbox = ctk.CTkCheckBox(
            processing_frame,
            text="Enable resampling",
            variable=self.resampling_enabled_var,
        )
        self.resampling_checkbox.grid(row=9, column=1, padx=10, pady=5, sticky="w")

        ctk.CTkLabel(processing_frame, text="Resample Frequency:").grid(
            row=10, column=0, padx=10, pady=5, sticky="w"
        )
        self.resample_frequency_entry = ctk.CTkEntry(
            processing_frame, placeholder_text="1min"
        )
        self.resample_frequency_entry.grid(
            row=10, column=1, padx=10, pady=5, sticky="ew"
        )
        ...
        ctk.CTkLabel(
            signal_frame, text="Available Signals", font=ctk.CTkFont(weight="bold")
        ).grid(row=0, column=0, padx=10, pady=(10, 5), sticky="w")
        ...
        self.signal_search_entry = ctk.CTkEntry(
            signal_controls, placeholder_text="Search signals..."
        )
        ...
        ctk.CTkButton(
            signal_buttons, text="All", command=self._select_all_signals, width=50
        ).grid(row=0, column=0, padx=2)
        ctk.CTkButton(
            signal_buttons, text="None", command=self._select_no_signals, width=50
        ...
        ctk.CTkLabel(
            process_frame, text="Processing & Export", font=ctk.CTkFont(weight="bold")
        ).grid(row=0, column=0, padx=10, pady=(10, 5), sticky="w")
    ...
    def create_plotting_tab(self, tab):
        ...
        ctk.CTkLabel(
            file_select_frame, text="File to Plot", font=ctk.CTkFont(weight="bold")
        ).grid(row=0, column=0, pady=5)

        self.plot_file_var = ctk.StringVar(value="Select a file...")
        self.plot_file_menu = ctk.CTkOptionMenu(
            file_select_frame,
            variable=self.plot_file_var,
            values=["Select a file..."],
            command=self._on_plot_file_select,
        )
        ...
        ctk.CTkLabel(axis_frame, text="X-Axis", font=ctk.CTkFont(weight="bold")).grid(
            row=0, column=0, pady=5
        )
        self.x_axis_var = ctk.StringVar(value="Index")
        self.x_axis_menu = ctk.CTkOptionMenu(
            axis_frame, variable=self.x_axis_var, values=["Index"]
        )
        ...
        ctk.CTkLabel(
            signals_frame, text="Signals to Plot", font=ctk.CTkFont(weight="bold")
        ).grid(row=0, column=0, pady=5)

        self.plot_signal_search = ctk.CTkEntry(
            signals_frame, placeholder_text="Search signals..."
        )
        ...
        ctk.CTkLabel(
            plot_controls_frame, text="Plot Controls", font=ctk.CTkFont(weight="bold")
        ).grid(row=0, column=0, pady=5)

        ctk.CTkButton(
            plot_controls_frame, text="Update Plot", command=self._update_plot
        ).grid(row=1, column=0, padx=5, pady=5, sticky="ew")
        ctk.CTkButton(
            plot_controls_frame, text="Clear Plot", command=self._clear_plot
        ...
        ctk.CTkLabel(
            export_controls_frame, text="Export Chart", font=ctk.CTkFont(weight="bold")
        ).grid(row=0, column=0, pady=5)
        ctk.CTkButton(
            export_controls_frame, text="Save as PNG", command=self._export_plot_png
        ).grid(row=1, column=0, padx=5, pady=2, sticky="ew")
        ctk.CTkButton(
            export_controls_frame, text="Save as PDF", command=self._export_plot_pdf
    ...
    def create_custom_vars_tab(self, tab):
        ...
        ctk.CTkLabel(
            inst_frame,
            text="Custom Variable Calculator",
            font=ctk.CTkFont(weight="bold"),
        ).grid(row=0, column=0, padx=10, pady=(10, 5), sticky="w")
        ...
        ctk.CTkLabel(
            create_frame, text="Add Custom Variable", font=ctk.CTkFont(weight="bold")
        ).grid(row=0, column=0, columnspan=2, padx=10, pady=(10, 5), sticky="w")

        ctk.CTkLabel(create_frame, text="Variable Name:").grid(
            row=1, column=0, padx=10, pady=5, sticky="w"
        )
        self.custom_var_name_entry = ctk.CTkEntry(
            create_frame, placeholder_text="MyVariable"
        )
        self.custom_var_name_entry.grid(row=1, column=1, padx=10, pady=5, sticky="ew")

        ctk.CTkLabel(create_frame, text="Formula:").grid(
            row=2, column=0, padx=10, pady=5, sticky="w"
        )
        self.custom_var_formula_entry = ctk.CTkEntry(
            create_frame, placeholder_text="[Column1] + [Column2]"
        )
        ...
        ctk.CTkButton(
            button_frame, text="Add Variable", command=self._add_custom_variable
        ).grid(row=0, column=0, padx=5, pady=5, sticky="ew")
        ctk.CTkButton(
            button_frame, text="Clear All", command=self._clear_custom_variables
        ...
        ctk.CTkLabel(
            list_frame, text="Current Custom Variables", font=ctk.CTkFont(weight="bold")
        ).grid(row=0, column=0, padx=10, pady=(10, 5), sticky="w")

        self.custom_vars_display = ctk.CTkTextbox(list_frame)
        self.custom_vars_display.grid(
    ...
    def create_plots_list_tab(self, tab):
        ...
        ctk.CTkLabel(
            title_frame,
            text="Saved Plot Configurations",
        ...
        ctk.CTkButton(
            controls_frame,
            text="Save Current Plot Config",
            command=self._save_plot_config,
        ).grid(row=0, column=0, padx=5, pady=10, sticky="ew")
        ctk.CTkButton(
            controls_frame, text="Load Plot Config", command=self._load_plot_config
        ).grid(row=0, column=1, padx=5, pady=10, sticky="ew")
        ctk.CTkButton(
            controls_frame, text="Delete Plot Config", command=self._delete_plot_config
...
After installation, restart the application to access this feature."""
            ...
            msg_label = ctk.CTkLabel(
                msg_frame, text=msg_text, justify="center", font=ctk.CTkFont(size=14)
            )
        ...
        ctk.CTkLabel(
            tag_frame,
            text="Step 1: Tag File Selection",
            font=ctk.CTkFont(weight="bold"),
        ).grid(row=0, column=0, columnspan=2, padx=10, pady=(10, 5), sticky="w")

        ctk.CTkButton(
            tag_frame, text="Select Tag File (.dbf)", command=self._select_tag_file
        ...
        ctk.CTkButton(
            tag_frame, text="Preview Tag File", command=self._preview_tag_file
        ...
        ctk.CTkLabel(
            data_frame,
            text="Step 2: Data File Selection",
            font=ctk.CTkFont(weight="bold"),
        ).grid(row=0, column=0, columnspan=2, padx=10, pady=(10, 5), sticky="w")

        ctk.CTkButton(
            data_frame, text="Select Data File (.dat)", command=self._select_data_file
        ).grid(row=1, column=0, padx=10, pady=5, sticky="w")
        ...
        ctk.CTkLabel(
            process_frame,
            text="Step 4: Processing Options",
            font=ctk.CTkFont(weight="bold"),
        ).grid(row=0, column=0, columnspan=2, padx=10, pady=(10, 5), sticky="w")

        ctk.CTkLabel(process_frame, text="Data Reduction Factor:").grid(
            row=1, column=0, padx=10, pady=5, sticky="w"
        )
        self.dat_reduction_entry = ctk.CTkEntry(
            process_frame, placeholder_text="1 (no reduction)"
        )
        ...
        self.convert_dat_button = ctk.CTkButton(
            ...
            command=self._convert_dat_file,
            height=35,
        )
        ...
        ctk.CTkLabel(
            log_frame, text="Conversion Log", font=ctk.CTkFont(weight="bold")
        ).grid(row=0, column=0, padx=10, pady=(10, 5), sticky="w")
    ...
    def _update_signal_list(self):
        ...
        for i, signal in enumerate(sorted(all_signals)):
            ...
            checkbox = ctk.CTkCheckBox(
                self.signal_scroll_frame, text=signal, variable=var
            )
            checkbox.grid(row=i, column=0, padx=5, pady=2, sticky="w")

            self.signal_vars[signal] = {"var": var, "widget": checkbox}
    ...
    def process_files(self):
        ...
        threading.Thread(
            target=self._process_files_worker, args=(selected_signals,), daemon=True
        ).start()
    ...
    def _process_files_worker(self, selected_signals):
        ...
        try:
            ...
            self.after(
                0, lambda: self.status_label.configure(text="Processing complete.")
            )
            ...
            self.after(
                0, lambda: self.status_label.configure(text="Processing failed.")
            )
        finally:
            self.after(
                0,
    ...
    def _prepare_processing_settings(self, selected_signals):
        ...
        if self.time_filtering_enabled.get():
            settings["time_filtering"] = {
                ...
                "end_time": (
                    self.end_time_entry.get() if self.end_time_entry.get() else None
                ),
            }
            ...
            if time_col and all(time_col in df.columns for df in compiled_data):
                ...
                if common_times:
                    ...
                    combined_df = pd.concat(
                        filtered_data, ignore_index=True, sort=False
                    )
                else:
                    # No common times, fallback to concatenate
                    combined_df = pd.concat(
                        compiled_data, ignore_index=True, sort=False
                    )
            else:
                # Fallback to concatenate
                combined_df = pd.concat(compiled_data, ignore_index=True, sort=False)
            ...
            for i, future in enumerate(as_completed(future_to_file)):
                ...
                try:
                    ...
                    if result.get("success", False):
                        ...
                        print(
                            f"Error processing {file_path}: {result.get('error', 'Unknown error')}"
                        )
...
"""
    ...
    def _update_plot_signals(self, columns):
        ...
        for i, col in enumerate(columns):
            ...
            self.plot_signal_vars[col] = {"var": var, "widget": checkbox}
                ...
                if signal in df.columns:
                    ...
                    if plot_type in ["Scatter", "Both"]:
                        self.plot_ax.scatter(
                            x_data,
                            y_data,
                            label=signal if plot_type == "Scatter" else None,
                            color=color,
                            alpha=0.6,
                            s=10,
                        )
    ...
    def _add_custom_variable(self):
        ...
        messagebox.showinfo(
            "Success", f"Custom variable '{var_name}' added successfully."
        )
    ...
    def _convert_dat_file(self):
        ...
        self.dat_log_textbox.configure(state="normal")
        self.dat_log_textbox.delete("1.0", "end")
        self.dat_log_textbox.insert(
            "1.0", f"Starting conversion for {len(selected_tags)} selected tags...\n"
        )
        ...
        try:
            ...
            time_data = pd.date_range(
                start="2024-01-01", periods=num_samples, freq="10S"
            )
            ...
            if reduction_str:
                try:
                    ...
                    if reduction_factor > 1:
                        ...
                        self.dat_log_textbox.insert(
                            "end", f"Applied reduction factor of {reduction_factor}\n"
                        )
                except ValueError:
                    self.dat_log_textbox.insert(
                        "end", "Invalid reduction factor, skipping.\n"
                    )

            # Save converted file
            output_filename = (
                os.path.splitext(os.path.basename(self.dat_import_data_file_path))[0]
                + "_converted.csv"
            )
            ...
            messagebox.showinfo(
                "Success", "DAT file converted and loaded successfully!"
            )
            ...
            self.dat_log_textbox.configure(state="disabled")
            self.convert_dat_button.configure(
                state="normal", text="Step 5: Convert and Load File"
            )
...
requires no Python installation on target computers.
    ...
    def _save_plot_config(self):
        ...
        messagebox.showinfo(
            "Info", "Plot configuration saving - feature in development."
        )

    def _load_plot_config(self):
        """Load a saved plot configuration."""
        messagebox.showinfo(
            "Info", "Plot configuration loading - feature in development."
        )

    def _delete_plot_config(self):
        """Delete a saved plot configuration."""
        messagebox.showinfo(
            "Info", "Plot configuration deletion - feature in development."
        )
