...
class CSVProcessorApp(ctk.CTk):
    ...
    def create_setup_and_process_tab(self, parent_tab):
        ...
        splitter_frame = self._create_splitter(
            parent_tab, left_panel, right_panel, "setup_left_width", 450
        )
    ...
    def create_plotting_tab(self, tab):
        ...
        self._create_splitter(
            plot_main_frame, left_panel, right_panel, "plotting_left_width", 350
        )
    ...
    def populate_processing_sub_tab(self, tab):
        ...
        resample_frame.grid_rowconfigure(0, weight=1)
        ctk.CTkLabel(
        ...
        integrator_frame.grid(row=6, column=0, padx=10, pady=10, sticky="new")
        integrator_frame.grid_columnconfigure(1, weight=1)
        integrator_frame.grid_rowconfigure(0, weight=1)
        ctk.CTkLabel(
        ...
        for signal in signals_to_integrate:
            ...
            if method == "Trapezoidal":
                ...
                cumulative = np.cumsum(
                    (signal_data.iloc[:-1] + signal_data.iloc[1:])
                    * time_diffs.iloc[1:]
                    / 2
                )
                    ...
                    for i in range(2, len(signal_data), 2):
                        if i < len(signal_data) - 1:
                            ...
                            cumulative[i] = (
                                cumulative[i - 2]
                                + (
                                    signal_data.iloc[i - 2]
                                    + 4 * signal_data.iloc[i - 1]
                                    + signal_data.iloc[i]
                                )
                                * dt
                                / 3
                ...
                if method == "Spline (Acausal)":
                    ...
                    try:
                        spline = UnivariateSpline(
                            signal_data.index, signal_data, s=0, k=3
                        )
                    ...
                    if window < 3:
                        window = 3
                    derivative_values = _poly_derivative(
                        signal_data, window, order + 2, order, 1.0
                    )
    ...
    def populate_custom_var_sub_tab(self, tab):
        ...
        ctk.CTkLabel(
            tab, text="Current Custom Variables:", font=ctk.CTkFont(weight="bold")
    ...
    def _create_bw_param_frame(self, parent):
        ...
        ctk.CTkLabel(row1, text="Filter Order:", width=110, anchor="w").grid(
        ...
        ctk.CTkLabel(row2, text="Cutoff Freq (Hz):", width=110, anchor="w").grid(
            row=0, column=0, sticky="w"
        )
    ...
    def _create_median_param_frame(self, parent):
        ...
        ctk.CTkLabel(inner_frame, text="Kernel Size:", width=110, anchor="w").grid(
            row=0, column=0, sticky="w"
        )
    ...
    def _create_savgol_param_frame(self, parent):
        ...
        row1.grid(row=0, column=0, columnspan=2, padx=10, pady=5, sticky="ew")
        row1.grid_columnconfigure(1, weight=1)
        ctk.CTkLabel(row1, text="Window Len:", width=110, anchor="w").grid(
            row=0, column=0, sticky="w"
        )
        ...
        row2.grid(row=1, column=0, columnspan=2, padx=10, pady=5, sticky="ew")
        row2.grid_columnconfigure(1, weight=1)
        ctk.CTkLabel(row2, text="Poly Order:", width=110, anchor="w").grid(
            row=0, column=0, sticky="w"
        )
    ...
    def create_plots_list_tab(self, tab):
        ...
        # Create splitter between left and right panels
        self._create_splitter(
            main_frame, left_panel, right_panel, "plots_list_left_width", 300
        )
        ...
        ctk.CTkButton(
            list_header, text="+", width=30, command=self._add_new_plot_config
        ).pack(side="right")
        ...
        ctk.CTkButton(
            list_buttons, text="Load Selected", command=self._load_plot_config
        ).pack(side="left", padx=2)
        ctk.CTkButton(
            list_buttons, text="Delete Selected", command=self._delete_plot_config
        ).pack(side="left", padx=2)
        ...
        ctk.CTkLabel(
            config_header,
            text="Plot Configuration Editor",
            font=ctk.CTkFont(weight="bold"),
        ).pack(side="left")
        ctk.CTkButton(
            config_header, text="Save Config", command=self._save_plot_config
        ).pack(side="right")
        ...
        # Plot name
        ctk.CTkLabel(config_editor, text="Plot Name:").pack(
            anchor="w", padx=10, pady=(10, 5)
        )
        self.plot_config_name_entry = ctk.CTkEntry(
            config_editor, placeholder_text="Enter plot name..."
        )
        self.plot_config_name_entry.pack(fill="x", padx=10, pady=(0, 10))

        # File selection
        ctk.CTkLabel(config_editor, text="File to Plot:").pack(
            anchor="w", padx=10, pady=(10, 5)
        )
        self.plot_config_file_menu = ctk.CTkOptionMenu(
            config_editor, values=["Select a file..."]
        )
        self.plot_config_file_menu.pack(fill="x", padx=10, pady=(0, 10))

        # X-axis selection
        ctk.CTkLabel(config_editor, text="X-Axis:").pack(
            anchor="w", padx=10, pady=(10, 5)
        )
        self.plot_config_xaxis_menu = ctk.CTkOptionMenu(
            config_editor, values=["default time"]
        )
        ...
        self.plot_config_search_entry = ctk.CTkEntry(
            signals_frame, placeholder_text="Search signals..."
        )
        self.plot_config_search_entry.pack(fill="x", padx=10, pady=5)
        self.plot_config_search_entry.bind(
            "<KeyRelease>", self._filter_plot_config_signals
        )
        ...
        ctk.CTkButton(
            signal_buttons, text="All", command=self._plot_config_select_all
        ).pack(side="left", padx=2)
        ctk.CTkButton(
            signal_buttons, text="None", command=self._plot_config_select_none
        ).pack(side="left", padx=2)
        ctk.CTkButton(
            signal_buttons, text="X", width=28, command=self._plot_config_clear_search
        ).pack(side="right", padx=2)

        # Signals list
        self.plot_config_signals_frame = ctk.CTkScrollableFrame(
            signals_frame, height=150
        )
        ...
        ctk.CTkLabel(
            time_range_frame, text="Time Range:", font=ctk.CTkFont(weight="bold")
        ).pack(anchor="w", padx=10, pady=5)

        # Start time
        ctk.CTkLabel(time_range_frame, text="Start Time (HH:MM:SS):").pack(
            anchor="w", padx=10
        )
        self.plot_config_start_entry = ctk.CTkEntry(
            time_range_frame, placeholder_text="e.g., 09:30:00"
        )
        self.plot_config_start_entry.pack(fill="x", padx=10, pady=(0, 5))

        # End time
        ctk.CTkLabel(time_range_frame, text="End Time (HH:MM:SS):").pack(
            anchor="w", padx=10
        )
        self.plot_config_end_entry = ctk.CTkEntry(
        ...
        ctk.CTkButton(
            action_frame, text="Generate Plot", command=self._generate_plot_from_config
        ).pack(side="left", padx=5)
        ctk.CTkButton(
            action_frame, text="Export Plot", command=self._export_plot_from_config
        ).pack(side="left", padx=5)
    ...
    def create_dat_import_tab(self, parent_tab):
        ...
        help_button = ctk.CTkButton(
            top_frame, text="Help", width=70, command=self._show_dat_help
        )
        ...
        self.dat_tag_search_entry = ctk.CTkEntry(
            tag_controls_frame, placeholder_text="Search tags..."
        )
        ...
        ctk.CTkButton(
            tag_controls_frame, text="All", width=60, command=self._dat_select_all
        ).grid(row=0, column=1, padx=5, pady=5)
        ctk.CTkButton(
            tag_controls_frame, text="None", width=60, command=self._dat_select_none
        ).grid(row=0, column=2, padx=5, pady=5)
        ctk.CTkButton(
            tag_controls_frame,
            text="Show Selected",
            width=120,
            command=self._dat_show_selected,
        ).grid(row=0, column=3, padx=5, pady=5)
        ctk.CTkButton(
            tag_controls_frame, text="X", width=28, command=self._dat_clear_search
        ...
        ctk.CTkLabel(
            reduction_frame, text="Step 5 (Optional): Data Reduction Factor"
        ).pack(side="left", padx=10)
        self.dat_reduction_entry = ctk.CTkEntry(
            reduction_frame, width=100, placeholder_text="e.g., 10"
        )
        ...
        self.convert_dat_button = ctk.CTkButton(
            top_frame,
            text="Step 6: Convert and Load File",
            height=40,
        ...
        copy_log_button = ctk.CTkButton(
            log_frame, text="Copy Log to Clipboard", command=self._copy_log_to_clipboard
        )
    ...
    def _update_file_list_ui(self):
        ...
        if self.input_file_paths:
            ...
            if hasattr(self, "plot_config_file_menu"):
                ...
                if (
                    file_names
                    and self.plot_config_file_menu.get() == "Select a file..."
                ):
    ...
    def _run_dat_conversion(self):
        ...
        selected_tags = [
            tag
            for tag, data_dict in self.dat_tag_vars.items()
            if data_dict["var"].get()
        ]
        if not selected_tags:
            messagebox.showwarning(
                "No Tags Selected",
                "Please select at least one tag to include in the output.",
            )
            return
        ...
        if df is None:
            ...
            self.convert_dat_button.configure(
                state="normal", text="Step 6: Convert and Load File"
            )
            return
        ...
        try:
            ...
            if not match:
                raise ValueError(
                    "Could not find a 'YYYY MM DD HHMM' date pattern in the data filename."
                )
            ...
            time_index = pd.to_datetime(
                pd.date_range(start=start_time, periods=num_rows, freq=sample_period)
            )
            ...
            self.dat_log_textbox.insert(
                "end", "- Timestamp, Date, and Time columns generated successfully.\n"
            )

        except Exception as e:
            messagebox.showerror(
                "Timestamp Error",
                f"Could not generate timestamps.\nPlease check the filename format and sample rate.\n\nError: {e}",
            )
            self.convert_dat_button.configure(
                state="normal", text="Step 6: Convert and Load File"
            )
        ...
        df = df[
            ["Timestamp", "Date", "Time"] + selected_tags
        ]  # Keep new time columns + selected tags
        self.dat_log_textbox.insert(
            "end", f"- Original dimensions: {len(df)} rows, {len(df.columns)} columns\n"
        )
        ...
        try:
            ...
            if factor_str and int(factor_str) > 1:
                ...
                self.dat_log_textbox.insert(
                    "end",
                    f"- Downsampled by factor of {reduction_factor}. New row count: {len(df)}\n",
                )
        except (ValueError, TypeError):
            self.dat_log_textbox.insert(
                "end", "- Invalid reduction factor. Skipping downsampling.\n"
            )

        # --- SAVE AND LOAD ---
        try:
            dat_filename_base = os.path.splitext(
                os.path.basename(self.dat_import_data_file_path)
            )[0]
            save_path = filedialog.asksaveasfilename(
            ...
            if save_path:
                ...
                messagebox.showinfo(
                    "Success", "Successfully processed and loaded the DAT file."
                )
            else:
            ...
            messagebox.showerror(
                "File Save Error",
                f"Could not save the processed CSV file.\n\nError: {e}",
            )
        finally:
...
requires no Python installation on target computers.
    ...
    def process_files(self):
        ...
        resample_rule = (
            f"{self.resample_value_entry.get()}{self.resample_unit_menu.get()}"
            if self.resample_var.get()
            else None
        )
        ...
        with ProcessPoolExecutor() as executor:
            ...
            futures = {
                executor.submit(process_single_csv_file, path, settings): path
            ...
            for i, future in enumerate(as_completed(futures)):
                ...
                self.status_label.configure(
                    text=f"Processing [{i+1}/{num_files}]: {os.path.basename(file_path)}"
                )
    ...
    def _process_single_file(self, file_path, selected_signals):
        ...
        signals_to_integrate = [
            s for s, data in self.integrator_signal_vars.items() if data["var"].get()
        ]
        ...
        if signals_to_differentiate:
            ...
            processed_df = self._apply_differentiation(
                processed_df, time_col, signals_to_differentiate, differentiation_method
            )
    ...
    def _update_plots_list_display(self):
        ...
        if not self.plots_list:
            ...
            for i, config in enumerate(self.plots_list, 1):
                ...
                self.plots_listbox.insert(
                    "end",
                    f"   Signals: {', '.join(config['signals'][:3])}{'...' if len(config['signals']) > 3 else ''}\n",
                )
                self.plots_listbox.insert(
                    "end",
                    f"   Time Range: {config['start_time']} - {config['end_time']}\n\n",
                )
    ...
    def _load_plot_config(self):
        ...
        if not self.plots_list:
            messagebox.showwarning(
                "No Configurations", "No saved plot configurations to load."
            )
            return
    ...
    def _update_plot_config_signals_list(self):
        ...
        for signal in numeric_cols:
            var = tk.BooleanVar(
                value=signal in self.current_plot_config.get("signals", [])
            )
            cb = ctk.CTkCheckBox(
                self.plot_config_signals_frame, text=signal, variable=var
            )
            cb.pack(anchor="w", padx=5, pady=2)
    ...
    def _save_plot_config(self):
        ...
        if not name:
            messagebox.showwarning(
                "Missing Name", "Please enter a name for the plot configuration."
            )
            return
        ...
        config = {
            ...
            "signals": [
                s
                for s, data in self.plot_config_signal_vars.items()
                if data["var"].get()
            ],
        ...
        self._update_plots_list_display()
        messagebox.showinfo(
            "Success", f"Plot configuration '{name}' saved successfully."
        )
    ...
    def _delete_plot_config(self):
        ...
        if not self.plots_list:
            messagebox.showwarning(
                "No Configurations", "No saved plot configurations to delete."
            )
            return
        ...
        self._update_plots_list_display()
        messagebox.showinfo(
            "Deleted", f"Plot configuration '{deleted_config['name']}' deleted."
        )
    ...
    def _generate_plot_from_config(self):
        ...
        if not self.current_plot_config:
            messagebox.showwarning(
                "No Configuration", "Please create or load a plot configuration first."
            )
            return
...
"""
    ...
    def _create_splitter(
        self, parent, left_widget, right_widget, splitter_key, default_left_width
    ):
    ...
    def _drag_splitter(self, event, splitter_key):
        ...
        if splitter_key in self.splitters and self.splitters[splitter_key].get(
            ...
            max_width = (
                splitter_info["frame"].winfo_width() - 300
            )  # Leave some space for right panel
