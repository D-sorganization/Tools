...
class CSVProcessorApp(ctk.CTk):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # --- Create Main UI ---
        self.main_tab_view = ctk.CTkTabview(self); self.main_tab_view.grid(row=0, column=0, padx=10, pady=10, sticky="nsew")
        self.main_tab_view.add("1. Setup & Process"); self.main_tab_view.add("2. Plotting & Analysis")
    ...
    def create_setup_and_process_tab(self, parent_tab):
        ...
        left_panel = ctk.CTkFrame(parent_tab, width=350); left_panel.grid(row=0, column=0, padx=10, pady=10, sticky="nsew"); left_panel.grid_rowconfigure(1, weight=1)
        ...
        processing_tab_view = ctk.CTkTabview(left_panel); processing_tab_view.grid(row=1, column=0, padx=10, pady=10, sticky="nsew")
        ...
        self.process_button = ctk.CTkButton(left_panel, text="Process & Batch Export Files", height=40, command=self.process_files); self.process_button.grid(row=2, column=0, padx=10, pady=10, sticky="ew")
        right_panel = ctk.CTkFrame(parent_tab); right_panel.grid(row=0, column=1, padx=(0, 10), pady=10, sticky="nsew"); right_panel.grid_rowconfigure(2, weight=1); right_panel.grid_columnconfigure(0, weight=1)
        self.file_list_frame = ctk.CTkScrollableFrame(right_panel, label_text="Selected Input Files", height=120); self.file_list_frame.grid(row=0, column=0, padx=10, pady=(0, 10), sticky="new")
        self.initial_file_label = ctk.CTkLabel(self.file_list_frame, text="Files you select will be listed here."); self.initial_file_label.pack(padx=5, pady=5)
        signal_control_frame = ctk.CTkFrame(right_panel); signal_control_frame.grid(row=1, column=0, padx=10, pady=0, sticky="ew"); signal_control_frame.grid_columnconfigure(0, weight=1) 
        self.search_entry = ctk.CTkEntry(signal_control_frame, placeholder_text="Search for signals..."); self.search_entry.grid(row=0, column=0, padx=(0, 5), sticky="ew"); self.search_entry.bind("<KeyRelease>", self._filter_signals)
        self.clear_search_button = ctk.CTkButton(signal_control_frame, text="X", width=28, command=self._clear_search); self.clear_search_button.grid(row=0, column=1, padx=5)
        ...
        self.signal_list_frame = ctk.CTkScrollableFrame(right_panel, label_text="Available Signals to Process"); self.signal_list_frame.grid(row=2, column=0, padx=10, pady=(5, 10), sticky="nsew"); self.signal_list_frame.grid_columnconfigure(0, weight=1)
    ...
    def populate_setup_sub_tab(self, tab):
        ...
        self.sort_col_menu = ctk.CTkOptionMenu(export_frame, values=["default (no sort)"]); self.sort_col_menu.grid(row=2, column=1, padx=10, pady=5, sticky="ew")
    ...
    def create_plotting_tab(self, tab):
        ...
        plot_left_panel_outer = ctk.CTkFrame(plot_main_frame, width=350); plot_left_panel_outer.grid(row=0, column=0, padx=10, pady=10, sticky="nsew"); plot_left_panel_outer.grid_rowconfigure(0, weight=1); plot_left_panel_outer.grid_propagate(False)
        plot_left_panel = ctk.CTkScrollableFrame(plot_left_panel_outer, label_text="Plotting Controls", height=600); plot_left_panel.grid(row=0, column=0, sticky="nsew", padx=0, pady=0); plot_left_panel_outer.grid_columnconfigure(0, weight=1)
          # FEATURE 3: Enhanced signal selection controls for plotting
        plot_signal_select_frame = ctk.CTkFrame(plot_left_panel); plot_signal_select_frame.pack(fill="x", expand=False, pady=5)
        plot_signal_select_frame.grid_columnconfigure(0, weight=1)
        self.plot_search_entry = ctk.CTkEntry(plot_signal_select_frame, placeholder_text="Search plot signals..."); self.plot_search_entry.grid(row=0, column=0, columnspan=4, sticky="ew", padx=5, pady=5); self.plot_search_entry.bind("<KeyRelease>", self._filter_plot_signals)
        ...
        self.plot_signal_frame = ctk.CTkScrollableFrame(plot_left_panel, label_text="Signals to Plot", height=150); self.plot_signal_frame.pack(expand=True, fill="both", padx=5, pady=5)

        appearance_frame = ctk.CTkFrame(plot_left_panel); appearance_frame.pack(fill="x", expand=False, pady=5)
        ...
        self.plot_type_var = ctk.StringVar(value="Line with Markers"); ctk.CTkOptionMenu(appearance_frame, variable=self.plot_type_var, values=["Line with Markers", "Line Only", "Markers Only (Scatter)"]).pack(fill="x", padx=10, pady=5)
        self.plot_title_entry = ctk.CTkEntry(appearance_frame, placeholder_text="Plot Title"); self.plot_title_entry.pack(fill="x", padx=10, pady=5)
        self.plot_xlabel_entry = ctk.CTkEntry(appearance_frame, placeholder_text="X-Axis Label"); self.plot_xlabel_entry.pack(fill="x", padx=10, pady=5)
        self.plot_ylabel_entry = ctk.CTkEntry(appearance_frame, placeholder_text="Y-Axis Label"); self.plot_ylabel_entry.pack(fill="x", padx=10, pady=5)

        plot_filter_frame = ctk.CTkFrame(plot_left_panel); plot_filter_frame.pack(fill="x", expand=False, pady=5)
        ...
        self.plot_filter_menu = ctk.CTkOptionMenu(plot_left_panel, variable=self.plot_filter_type, values=self.filter_names, command=self._update_plot_filter_ui); self.plot_filter_menu.pack(fill="x", padx=5, pady=5)
        ...
        trend_frame = ctk.CTkFrame(plot_left_panel); trend_frame.pack(fill="x", expand=False, pady=5)
        ...
        self.trendline_type_var = ctk.StringVar(value="None"); ctk.CTkOptionMenu(trend_frame, variable=self.trendline_type_var, values=["None", "Linear", "Exponential", "Power", "Polynomial"]).pack(fill="x", padx=10, pady=5)
        self.poly_order_entry = ctk.CTkEntry(trend_frame, placeholder_text="Polynomial Order (2-6)"); self.poly_order_entry.pack(fill="x", padx=10, pady=5)
        self.trendline_textbox = ctk.CTkTextbox(trend_frame, height=70); self.trendline_textbox.pack(fill="x", expand=True, padx=10, pady=5)

        ctk.CTkButton(plot_left_panel, text="Update Plot", height=35, command=self.update_plot).pack(fill="x", padx=5, pady=10)

        # FEATURE 11: Chart export options
        export_chart_frame = ctk.CTkFrame(plot_left_panel); export_chart_frame.pack(fill="x", expand=False, pady=5)
        ...
        # Plot Range Controls
        plot_range_frame = ctk.CTkFrame(plot_left_panel); plot_range_frame.pack(fill="x", expand=False, pady=5)
        ctk.CTkLabel(plot_range_frame, text="Plot Time Range", font=ctk.CTkFont(weight="bold")).pack(anchor="w", padx=10, pady=5)
        ctk.CTkLabel(plot_range_frame, text="Start Time (HH:MM:SS):").pack(fill="x", padx=10); self.plot_start_entry = ctk.CTkEntry(plot_range_frame, placeholder_text="e.g., 09:30:00"); self.plot_start_entry.pack(fill="x", padx=10, pady=(0,5))
        ctk.CTkLabel(plot_range_frame, text="End Time (HH:MM:SS):").pack(fill="x", padx=10); self.plot_end_entry = ctk.CTkEntry(plot_range_frame, placeholder_text="e.g., 17:00:00"); self.plot_end_entry.pack(fill="x", padx=10, pady=(0,5))
        ...
        trim_frame = ctk.CTkFrame(plot_left_panel); trim_frame.pack(fill="x", expand=False, pady=5)
        ctk.CTkLabel(trim_frame, text="Trim & Export", font=ctk.CTkFont(weight="bold")).pack(anchor="w", padx=10, pady=5)
        self.trim_date_range_label = ctk.CTkLabel(trim_frame, text="Data on date:", text_color="gray"); self.trim_date_range_label.pack(fill="x", padx=10)
        ...
        ctk.CTkLabel(trim_frame, text="End Time (HH:MM:SS):").pack(fill="x", padx=10); self.trim_end_entry = ctk.CTkEntry(trim_frame, placeholder_text="e.g., 17:00:00"); self.trim_end_entry.pack(fill="x", padx=10, pady=(0,5))
...
requires no Python installation on target computers.
    ...
    def process_files(self):
        ...
        try:
            ...
            messagebox.showinfo("Success", f"Successfully processed and saved {len(self.input_file_paths)} file(s).")
