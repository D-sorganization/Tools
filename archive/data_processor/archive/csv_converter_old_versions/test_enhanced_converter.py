...
from csv_to_parquet_converter_enhanced import (ColumnSelectionDialog,
...
def create_test_csv_files(num_files=3, rows_per_file=100, num_columns=50):
    ...
    print(
        f"Creating {num_files} test CSV files with {num_columns} columns each in {temp_dir}"
    )
        ...
        for col in columns:
            if "id" in col.lower():
                ...
                data[col] = pd.date_range(
                    "2024-01-01", periods=rows_per_file, freq="1S"
                )
            elif (
                "value" in col.lower()
                or "force" in col.lower()
                or "measurement" in col.lower()
            ):
...
def test_column_selection_dialog():
    ...
    search_term = "input"
    matching_columns = [
        col for col in test_columns if search_term.lower() in col.lower()
    ]
...
def test_large_column_handling():
    ...
    search_term = "input"
    matching_columns = [
        col for col in large_columns if search_term.lower() in col.lower()
    ]
    search_time = time.time() - start_time

    print(
        f"Search for '{search_term}' found {len(matching_columns)} matches in {search_time:.4f} seconds"
    )
    ...
    print(
        f"Case-sensitive search for '{search_term}' found {len(matching_columns)} matches in {search_time:.4f} seconds"
    )
