...
class CSVProcessorApp(ctk.CTk):
    ...
    def create_plots_list_tab(self, tab):
        ...
        ctk.CTkLabel(top_frame, text="Plots List Manager", font=ctk.CTkFont(size=16, weight="bold")).grid(row=0, column=0, columnspan=2, padx=10, pady=10, sticky="w")
    ...
    def create_dat_import_tab(self, parent_tab):
        ...
        if not HAS_DBF_SUPPORT:
            ...
            ctk.CTkLabel(warning_frame, text="DAT Import Not Available", font=ctk.CTkFont(size=16, weight="bold")).pack(pady=20)
            ctk.CTkLabel(warning_frame, text="The 'simpledbf' package is required for DAT file import.", wraplength=400).pack(pady=10)
            ctk.CTkLabel(warning_frame, text="Install it with: pip install simpledbf", font=ctk.CTkFont(family="monospace")).pack(pady=10)
            return
        ...
        # Help button
        help_button = ctk.CTkButton(main_frame, text="Help", width=70, command=self._show_dat_help)
        ...
        # Convert button
        self.convert_dat_button = ctk.CTkButton(main_frame, text="Step 5: Convert and Load File", height=40, command=self._run_dat_conversion)
    ...
    def _create_splitter(self, parent, left_content_creator, right_content_creator, splitter_key, default_left_width):
        """
    ...
    def _create_ma_param_frame(self, parent, time_units):
        ...
        ctk.CTkLabel(frame, text="Time Window:").grid(row=0, column=0, padx=5, pady=5, sticky="w")
    ...
    def _create_bw_param_frame(self, parent):
        ...
        ctk.CTkLabel(frame, text="Filter Order:").grid(row=0, column=0, padx=5, pady=5, sticky="w")
        entry_ord = ctk.CTkEntry(frame, placeholder_text="e.g., 3")
        entry_ord.grid(row=0, column=1, padx=5, pady=5, sticky="ew")
        
        ctk.CTkLabel(frame, text="Cutoff Freq (Hz):").grid(row=1, column=0, padx=5, pady=5, sticky="w")
    ...
    def _create_median_param_frame(self, parent):
        ...
        ctk.CTkLabel(frame, text="Kernel Size:").grid(row=0, column=0, padx=5, pady=5, sticky="w")
    ...
    def _create_savgol_param_frame(self, parent):
        ...
        ctk.CTkLabel(frame, text="Window Length:").grid(row=0, column=0, padx=5, pady=5, sticky="w")
        entry_win = ctk.CTkEntry(frame, placeholder_text="Odd integer, e.g., 11")
        entry_win.grid(row=0, column=1, padx=5, pady=5, sticky="ew")
        
        ctk.CTkLabel(frame, text="Poly Order:").grid(row=1, column=0, padx=5, pady=5, sticky="w")
    ...
    def _update_plot_filter_ui(self, choice):
        ...
        elif choice in ["Butterworth Low-pass", "Butterworth High-pass"] and hasattr(self, 'plot_bw_frame'):
    ...
    def _update_custom_vars_listbox(self):
        ...
        if not self.custom_vars_list:
            ...
            for i, (var_name, formula) in enumerate(self.custom_vars_list):
                self.custom_vars_listbox.insert("end", f"{i+1}. {var_name} = {formula}\n")
    ...
    def _apply_differentiation(self, df, time_col, signals_to_differentiate, method="Spline (Acausal)"):
        """Apply differentiation to selected signals."""
        ...
        # Convert time column to numeric for differentiation
        time_numeric = pd.to_numeric(df[time_col], errors='coerce')
        if time_numeric.isna().all():
            print("Warning: Could not convert time column to numeric for differentiation")
            return df
        
        # Calculate time step
        delta_x = time_numeric.diff().median()
        if pd.isna(delta_x) or delta_x == 0:
            print("Warning: Invalid time step for differentiation")
            return df
            ...
            for order in selected_orders:
                try:
                    if method == "Spline (Acausal)":
                        ...
                        spline = UnivariateSpline(time_numeric, signal_data, s=0, k=3)
                        derivative = spline.derivative(n=order)(time_numeric)
                    elif method == "Rolling Polynomial (Causal)":
                        # Use rolling polynomial fit for causal derivatives
                        derivative = _poly_derivative(signal_data, window=20, poly_order=3, 
                                                    deriv_order=order, delta_x=delta_x)
                    else:
                        continue
                    
                    # Create derivative column name
                    deriv_col_name = f"d{order}_{signal}"
                    df[deriv_col_name] = derivative
                    
                except Exception as e:
                    print(f"Error calculating {order}th derivative for {signal}: {e}")
                    continue
    ...
    def _apply_integration(self, df, time_col, signals_to_integrate, method="Trapezoidal"):
        ...
        # Convert time column to numeric for integration
        time_numeric = pd.to_numeric(df[time_col], errors='coerce')
        if time_numeric.isna().all():
            print("Warning: Could not convert time column to numeric for integration")
            return df
        ...
        for signal in signals_to_integrate:
            ...
            try:
                if method == "Trapezoidal":
                    ...
                    cumulative = np.cumsum(np.trapz(signal_data, time_numeric))
                elif method == "Rectangular":
                    # Use rectangular rule for integration
                    dt = time_numeric.diff().fillna(0)
                    cumulative = np.cumsum(signal_data * dt)
                elif method == "Simpson":
                    # Use Simpson's rule for integration
                    cumulative = np.cumsum(signal_data * time_numeric.diff().fillna(0))
                else:
                    continue
                ...
                print(f"Error calculating cumulative for {signal}: {e}")
                continue
    ...
    def process_files(self):
        ...
        try:
            ...
            if export_type in ["CSV (Separate Files)", "Excel (Separate Files)", "MAT (Separate Files)"]:
            ...
            messagebox.showinfo("Success", f"Processing complete! Files saved to:\n{self.output_directory}")
    ...
    def _export_chart_excel(self):
        ...
        try:
            ...
            if save_path:
                ...
                self.status_label.configure(text=f"Excel export: {os.path.basename(save_path)}")
    ...
    def _select_tag_file(self):
        ...
        filepath = filedialog.askopenfilename(
            ...
            filetypes=(("All files", "*.*"), ("Data files", "*.dat"), ("DBF files", "*.dbf"))
        )
        if filepath:
    ...
    def _preview_tag_file(self):
        ...
        if not HAS_DBF_SUPPORT:
            messagebox.showerror("Error", "simpledbf package not available. Cannot read DBF files.")
            return

        tags = self._get_tags_from_file()
        if tags:
            messagebox.showinfo("Tag Preview", f"Found {len(tags)} tags in the file.\nFirst 10 tags: {', '.join(tags[:10])}")
        else:
    ...
    def _run_dat_conversion(self):
        ...
        messagebox.showinfo("DAT Conversion", "DAT conversion is a simplified implementation. For full functionality, please use the original version with all dependencies.")
    ...
    def _update_plots_list_display(self):
        ...
        if not self.plots_list:
            ...
            for i, config in enumerate(self.plots_list, 1):
                self.plots_listbox.insert("end", f"{i}. {config.get('name', 'Unnamed')}\n")
