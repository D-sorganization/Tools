...
class CSVProcessorApp(ctk.CTk):
    ...
    def create_plotting_tab(self, tab):
        ...
        plot_left_panel_outer.grid(row=0, column=0, padx=10, pady=10, sticky="ns")
        plot_left_panel_outer.grid_propagate(False)
        plot_left_panel_outer.grid_rowconfigure(
            1, weight=1
        )  # The scrollable area will expand
        plot_left_panel_outer.grid_columnconfigure(0, weight=1)

        # --- NEW: Help Button moved here for visibility ---
        help_button = ctk.CTkButton(
            plot_left_panel_outer, text="Help", width=70, command=self._show_plot_help
        )
        ...
        button_bar_frame = ctk.CTkFrame(
            plot_signal_select_frame, fg_color="transparent"
        )
        button_bar_frame.pack(fill="x", padx=5, pady=(0, 5))

        ctk.CTkButton(button_bar_frame, text="All", command=self._plot_select_all).pack(
            side="left", fill="x", expand=True, padx=2
        )
        ctk.CTkButton(
            button_bar_frame, text="None", command=self._plot_select_none
        ).pack(side="left", fill="x", expand=True, padx=2)
        ctk.CTkButton(
            button_bar_frame, text="Show Selected", command=self._show_selected_signals
        ).pack(side="left", fill="x", expand=True, padx=2)
        ctk.CTkButton(
            button_bar_frame, text="X", width=28, command=self._plot_clear_search
    ...
    def _run_dat_conversion(self):
        ...
        self.dat_log_textbox.insert(
            "end",
            f"- Original dimensions: {full_df.shape[0]} rows, {full_df.shape[1]} columns\n",
        )
        self.dat_log_textbox.insert(
            "end", f"- Filtered to {df.shape[1]} selected columns\n"
        )
        ...
        try:
            ...
            temp_csv_path = os.path.join(
                output_dir, f"{dat_filename_base}_processed.csv"
            )
