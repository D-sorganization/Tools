    ...
    def read_file(file_path: str, format_type: str, **kwargs) -> pd.DataFrame:
        ...
        try:
            if format_type == "csv":
                ...
                if table_name:
                    return pd.read_sql(
                        f"SELECT * FROM {table_name}", f"sqlite:///{file_path}"
                    )
                else:
                    ...
                    if len(tables) > 0:
                        return pd.read_sql(
                            f"SELECT * FROM {tables.iloc[0]['name']}",
                            f"sqlite:///{file_path}",
                        )
                    else:
    ...
    def write_file(
        ...
        try:
            if format_type == "csv":
                ...
                if file_path.endswith(".npz"):
                    np.savez_compressed(
                        file_path, **{col: df[col].values for col in df.columns}
                    )
                else:
...
class ConversionWorker(QThread):
    ...
    def _convert_to_single_file(self):
        ...
        for i, input_file in enumerate(self.input_files):
            ...
            try:
                self.status_updated.emit(
                    f"Processing {Path(input_file).name} ({i+1}/{total_files})"
                )
                ...
                if self.selected_columns:
                    ...
                    selected_available = self.selected_columns.intersection(
                        available_columns
                    )
                    if selected_available:
                        ...
                        self.status_updated.emit(
                            f"Warning: No selected columns found in {input_file}"
                        )
                        continue
        ...
        if self.split_config.enabled:
            ...
            DataWriter.write_file(
                final_df, self.output_path, self.output_format, **self.format_options
            )
    ...
    def _convert_to_multiple_files(self):
        ...
        for i, input_file in enumerate(self.input_files):
            ...
            try:
                self.status_updated.emit(
                    f"Converting {Path(input_file).name} ({i+1}/{total_files})"
                )
                ...
                # Write the file
                DataWriter.write_file(
                    df, str(output_file), self.output_format, **self.format_options
                )
    ...
    def _convert_in_batches(self):
        ...
        self.status_updated.emit(
            f"Processing {total_files} files in {total_batches} batches of {self.batch_size} files each"
        )
        ...
        for batch_num in range(total_batches):
            ...
            for i, input_file in enumerate(batch_files):
                ...
                try:
                    self.status_updated.emit(
                        f"Reading {Path(input_file).name} (batch {batch_num + 1})"
                    )
            ...
            batch_filename = (
                f"{output_path.stem}_batch_{batch_num + 1:03d}{output_path.suffix}"
            )
            batch_output_path = output_path.parent / batch_filename

            # Write the batch file
            self.status_updated.emit(
                f"Writing batch {batch_num + 1} to {batch_filename}"
            )
            DataWriter.write_file(
                batch_df,
    ...
    def _split_by_rows(self, df: pd.DataFrame, base_path: Path) -> List[str]:
        ...
        for i in range(num_files):
            ...
            # Write chunk
            self.status_updated.emit(
                f"Writing split file {i + 1}/{num_files}: {filename.name}"
            )
            self._write_chunk(
                chunk_df, output_file, i == 0
    ...
    def _split_by_size(self, df: pd.DataFrame, base_path: Path) -> List[str]:
        ...
        with tempfile.NamedTemporaryFile(
            suffix=f".{self.output_format}", delete=True
        ) as temp_file:
    ...
    def _split_by_time(self, df: pd.DataFrame, base_path: Path) -> List[str]:
        ...
        if not pd.api.types.is_datetime64_any_dtype(df[time_column]):
            try:
                ...
                raise ValueError(
                    f"Could not convert '{time_column}' to datetime: {str(e)}"
                )
        ...
        for i, (start_time, end_time) in enumerate(intervals):
            ...
            mask = (df_sorted[time_column] >= start_time) & (
                df_sorted[time_column] < end_time
            )
            ...
            filename = self._generate_filename(
                base_path, i + 1, len(intervals), time_str
            )
            output_file = str(filename)

            # Write chunk
            self.status_updated.emit(
                f"Writing time split {i + 1}/{len(intervals)}: {filename.name}"
            )
            self._write_chunk(
                chunk_df, output_file, True
            )  # Each time split gets its own header
        ...
        for index, row in df.iterrows():
            ...
            try:
                ...
                if should_split and current_chunk:
                    ...
                    self.status_updated.emit(
                        f"Writing custom split {file_index}: {filename.name}"
                    )
    ...
    def _write_chunk(
        self, df: pd.DataFrame, output_file: str, include_header: bool = True
    ):
...
class UniversalFileConverter(QWidget):
    ...
    def _create_bottom_section(self):
        ...
        self.batch_processing_checkbox = QCheckBox(
            "Create multiple smaller output files (memory management)"
        )
        self.batch_processing_checkbox.toggled.connect(
            self._on_batch_processing_toggled
        )
    ...
    def _browse_folder(self):
        ...
        folder_path = QFileDialog.getExistingDirectory(
            self, "Select Folder with Files to Convert"
        )
        if folder_path:
    ...
    def _scan_files(self, folder_path: str):
        ...
        if found_files:
            ...
            QMessageBox.information(
                self, "No Files Found", f"No supported files found in {folder_path}"
            )
    ...
    def _select_columns(self):
        ...
        try:
            ...
            if not input_format:
                QMessageBox.warning(
                    self, "Format Error", f"Could not detect format for {first_file}"
                )
                return
    ...
    def _update_convert_button(self):
        ...
        can_convert = (
            len(self.input_files) > 0 and self.output_path_edit.text().strip() != ""
        )
...
class MainWindow(QMainWindow):
    ...
    def _show_about(self):
        ...
        QMessageBox.about(
            self,
            "About Universal File Format Converter",
            """
...
class FileSplittingDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        self.enable_splitting_checkbox.toggled.connect(
            self._on_enable_splitting_toggled
        )
        ...
        self.filename_pattern_edit = QLineEdit(
            "{base_name}_part_{part_number:04d}{extension}"
        )
    ...
    def _update_preview(self):
        ...
        if current_tab == 0:  # Rows tab
            ...
            if column:
                estimated_files = (
                    f"Split by {interval:.1f} hour intervals using '{column}' column"
                )
            else:
    ...
    def _accept(self):
        ...
        if current_tab == 0:  # Rows
            ...
            if not self.split_config.time_column:
                QMessageBox.warning(
                    self, "Missing Time Column", "Please specify a time column name."
                )
                return

        else:  # Custom
            self.split_config.method = SplitMethod.CUSTOM
            self.split_config.custom_condition = (
                self.custom_condition_edit.toPlainText().strip()
            )

            if not self.split_config.custom_condition:
                QMessageBox.warning(
                    self,
                    "Missing Custom Condition",
