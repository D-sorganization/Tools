...
class TestSkillRunner:
    ...
    def test_precondition_failure_returns_structured_result(self) -> None:
        runner = _build_runner(_OkSkill)
        result = asyncio.run(
            runner.run(
    ...
    def test_postcondition_failure_returns_structured_result(self) -> None:
        runner = _build_runner(_BadPostSkill)
        result = asyncio.run(
            runner.run(SkillInvocation(skill_id="t.badpost", args={}, request_id="r4"))
        )
        assert result.success is False
        assert result.error is not None
        kinds = [event["kind"] for event in result.audit_trail]
    ...
    def test_body_exception_returns_structured_execution_error(self) -> None:
    ...
    def test_raises_directly_when_skill_id_unknown(self) -> None:
        runner = _build_runner(_OkSkill)
        with pytest.raises(SkillNotFoundError):
            asyncio.run(
                runner.run(SkillInvocation(skill_id="nope", args={}, request_id="r6"))
            )

    def test_precondition_error_type_can_be_raised(self) -> None:
        # SkillPreconditionError exists and is the runner-internal raise type.
        err = SkillPreconditionError("nope")
        assert str(err) == "nope"

    def test_postcondition_error_type_can_be_raised(self) -> None:
        err = SkillPostconditionError("nope")
        assert str(err) == "nope"

    def test_timeout_error_type_can_be_raised(self) -> None:
        err = SkillTimeoutError("nope")
        assert str(err) == "nope"

    def test_execution_error_type_can_be_raised(self) -> None:
