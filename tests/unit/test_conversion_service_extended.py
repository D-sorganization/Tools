...
class TestConversionServiceBasic:
    ...
    def test_mass_kg_to_lb(self) -> None:
        val = convert(1.0, "kg", "lb")
        assert val == pytest.approx(2.20462, rel=1e-4)

    def test_volume_m3_to_L(self) -> None:
        val = convert(1.0, "m3", "L")
        assert val == pytest.approx(1000.0, rel=1e-4)

    def test_energy_J_to_kJ(self) -> None:
        val = convert(1000.0, "J", "kJ")
        assert val == pytest.approx(1.0, rel=1e-4)

    def test_identity_conversion(self) -> None:
        """Converting a unit to itself must return the same value."""
        val = convert(42.0, "kg", "kg")
        assert val == pytest.approx(42.0)

    def test_kelvin_to_celsius(self) -> None:
        val = convert(273.15, "K", "C")
        assert val == pytest.approx(0.0, abs=0.01)

    def test_rankine_to_kelvin(self) -> None:
        val = convert(491.67, "R", "K")
...
class TestConversionTableConsistency:
    ...
    def test_mass_conversion_factors_positive(self) -> None:
        """All mass flow conversion factors must be positive."""
        for unit, factor in MASS_FLOW_CONVERSIONS.items():
            assert factor > 0, f"Factor for {unit} is not positive: {factor}"

    def test_molar_conversion_factors_positive(self) -> None:
        for unit, factor in MOLAR_FLOW_CONVERSIONS.items():
            assert factor > 0, f"Factor for {unit} is not positive: {factor}"

    def test_volumetric_conversion_factors_positive(self) -> None:
        for unit, factor in VOLUMETRIC_FLOW_CONVERSIONS_TO_M3_S.items():
            assert factor > 0, f"Factor for {unit} is not positive: {factor}"

    def test_base_unit_is_unity(self) -> None:
