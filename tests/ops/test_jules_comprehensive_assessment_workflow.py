...
_COMPREHENSIVE_WORKFLOW = Path(".github/workflows/Jules-Comprehensive-Assessment.yml")
_COMPLETIST_WORKFLOW = Path(".github/workflows/Jules-Completist.yml")
...
def test_comprehensive_assessment_timeout_covers_jules_poll_budget() -> None:
    workflow = _workflow_text(_COMPREHENSIVE_WORKFLOW)
...
def test_completist_timeout_covers_jules_poll_budget() -> None:
    workflow = _workflow_text(_COMPLETIST_WORKFLOW)
    timeout_match = re.search(
        r"audit:\n\s+timeout-minutes: (?P<minutes>\d+)",
        workflow,
    )
