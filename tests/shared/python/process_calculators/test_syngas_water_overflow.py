...
class TestPerformanceBenchmarks:
    ...
    def test_safe_exp_overhead_negligible(self) -> None:
        """_safe_exp overhead vs math.exp should be < 20x for 100k calls.
        ...
        Uses best-of-3 repetitions to prevent CI runner scheduling jitter
        under multi-worker test execution.
        """
        # Keep within [-700, 700] so both math.exp and _safe_exp take the same path
        values = [float(x) for x in np.linspace(-700, 700, 100_000)]

        def _best_time(func: Callable[[float], float]) -> float:
            best = float("inf")
            for _ in range(3):
        ...
        baseline = _best_time(math.exp)
        safe_time = _best_time(_safe_exp)
