...
def test_import_data_explorer_service_does_not_pull_in_pandas(tmp_path: Path) -> None:
    ...
    script = textwrap.dedent(
        ...
        if "pandas" in sys.modules:
            print("FAIL: pandas is in sys.modules after import", flush=True)
            sys.exit(1)
        print("OK", flush=True)
        """
    )
...
def test_count_delimited_rows_cancel_returns_early(tmp_path: Path) -> None:
    """Setting cancel_event before calling must cause an immediate early exit."""
    csv_path = tmp_path / "large.csv"
    _write_csv(csv_path, 50_000)

    cancel = threading.Event()
    cancel.set()  # set before the call — should exit on the first check

    result = _count_delimited_rows(csv_path, cancel_event=cancel)
    # With cancel already set, we stop as soon as the first interval check fires.
    # The result must be strictly less than 50 000 (the full count).
...
def test_count_delimited_rows_cancel_mid_way(tmp_path: Path) -> None:
    ...
    t.start()
    # Signal cancellation shortly after the thread starts.
    cancel.set()
    t.join(timeout=10)
...
def test_count_delimited_rows_progress_callback_fires(tmp_path: Path) -> None:
    ...
    # Each reported count must be a multiple of 10 000
