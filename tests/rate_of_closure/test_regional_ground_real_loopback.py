...
# Bounds here only keep the suite snappy -- no test asserts timeout behaviour.
# The original 15 s poll and 1 s request budgets were tight enough to fail under
# `-n auto`, where a dozen workers each spawn a real authority subprocess: the
# job completes, but the client gives up first and reports `poll_timeout`. The
# production default is 300 s, so these stay well inside it while surviving load.
...
def test_default_environment_factory_is_qualified_and_executes(tmp_path: Path) -> None:
    ...
    try:
        transport = LoopbackAuthorityHttpTransport(runtime, timeout_s=10.0)
...
def test_real_qualified_authority_auth_status_cancel_and_result_contracts(
    ...
    with _authority(_factory("create_preflight_authority_app")) as runtime:
        ...
        wrong = LoopbackAuthorityHttpTransport(wrong_runtime, timeout_s=10.0)
        ...
        capability_transport = LoopbackAuthorityHttpTransport(runtime, timeout_s=10.0)
        ...
        submitter = LoopbackRegionalGroundSubmitter(
            LoopbackAuthorityHttpTransport(runtime, timeout_s=10.0), policy=_POLICY
        )
...
def test_complete_result_survives_hard_authority_process_restart(
    ...
    job = _job()
    submitter = LoopbackRegionalGroundSubmitter(
        LoopbackAuthorityHttpTransport(first, timeout_s=10.0), policy=_POLICY
    )
    ...
    try:
        transport = LoopbackAuthorityHttpTransport(second, timeout_s=10.0)
...
def test_hard_loss_marks_running_job_failed_without_automatic_replay(
    ...
    transport = LoopbackAuthorityHttpTransport(first, timeout_s=10.0)
    job = _job()
    ...
    try:
        recovered_transport = LoopbackAuthorityHttpTransport(second, timeout_s=10.0)
