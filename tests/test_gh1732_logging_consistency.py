...
"""
    ...
    def test_print_in_return_is_flagged(self) -> None:
        """return print(...) — print in return context IS flagged."""
        source = """
def run():
    return print("done")
"""
    ...
    def test_print_in_conditional_is_flagged(self) -> None:
        """print() inside a conditional expression IS flagged."""
        source = """
result = x if x > 0 else print("negative")
"""
        lines = _find_print_calls(source, "<test>")
        assert lines == [2], f"Expected line 2 to be flagged, got {lines}"
