...
def _check_launcher_file(launcher_path: Path) -> None:
    ...
    if not launcher_path.exists():
        _emit_stdout(f"Error: {launcher_path.name} - NOT FOUND")
        return
    ...
    _emit_stdout(f"Found: {launcher_path.name}")
    ...
    try:
        ...
        _emit_stdout(f"   Error: Could not read file: {e}")
    _emit_stdout()
...
def _print_recommendations(current_dir: Path) -> None:
    ...
    if unified_launcher.exists():
        ...
        _emit_stdout("Error: UnifiedToolsLauncher.py not found!")

    _emit_stdout()
    _emit_stdout("Note: 'tools_launcher.py' does not exist.")
    _emit_stdout(
...
def _check_desktop_shortcuts() -> None:
    ...
    if shortcuts:
        ...
        for shortcut in shortcuts:
            _emit_stdout(f"Shortcut: {shortcut.name}")
    else:
