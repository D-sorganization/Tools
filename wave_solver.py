...
def run_cmd(cmd, cwd=None, ignore_err=False):
    ...
    try:
        result = subprocess.run(
            cmd, shell=True, check=not ignore_err,
...
def main():
    ...
    # Fetch issues
    raw_issues = run_cmd("gh issue list --state open --json number,title,body", cwd=repo_path)
    if not raw_issues:
    ...
    for issue in issues_to_fix:
        ...
        run_cmd("git checkout main || git checkout master", cwd=repo_path, ignore_err=True)
        ...
        print("Invoking Claude Code Autonomous Resolver...")
        ...
        try:
            ...
            res = subprocess.run(cmd, cwd=repo_path, shell=True, text=True)
        except Exception as e:
            print(f"Claude invocation failed: {e}")
            continue
            
        print("Claude Code Run Finished. Committing and Pushing PR...")
        
        # Check if anything changed
        status = run_cmd("git status --porcelain", cwd=repo_path)
        if not status:
            print(f"Issue #{num} yielded no changes by Claude, skipping.")
            continue
            
        # Commit & Push
        run_cmd("git add -A", cwd=repo_path)
        run_cmd(f'git commit -m "fix: resolve A-N assessment finding #{num} - {title}" --no-verify', cwd=repo_path)
        run_cmd(f"git push -u origin {branch} --no-verify", cwd=repo_path, ignore_err=True)
        ...
        # Enable Auto-Merge
        run_cmd(f"gh pr merge --squash --auto -d", cwd=repo_path, ignore_err=True)
        
        print(f"Wave deployed for Issue #{num}.")
